...
class Reach(object):
    ...
    def __init__(self, vectors, words, verbose=False, name=""):
        ...
        self.index = AnnoyIndex(self.size)
    ...
    def most_similar(self, w, num=10):
        ...
        """
        return [self.indices[i] for i in self.index.get_nns_by_item(self.words[w], num+1)][1:]

    def nearest_neighbor(self, vector, num=10):

        return [self.indices[i] for i in self.index.get_nns_by_vector(vector, num+1)][1:]
    ...
    def similarity(self, w1, w2):
        ...
        """
        return self.index.get_distance(self.words[w1], self.words[w2])
