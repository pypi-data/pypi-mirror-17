...
class IntelGetter(GenericGetter):
    ...
    def search(self):
        ...
        """
        split_search_term = self.search_term.split(u" ")  # Splits the search term into single words
        ...
        while i < len(split_search_term):
            prepared_search_term += u"%20" + split_search_term[i]
            i += 1

        # Get information from the website
        url = u"http://intel.haruhichan.com/?s=" + prepared_search_term  # Generate the URL
        content = BeautifulSoup(requests.get(url).text, u"html.parser")  # Parse the HTML
        packs = content.select(u"td")  # Only get 'td' elements from the HTML
