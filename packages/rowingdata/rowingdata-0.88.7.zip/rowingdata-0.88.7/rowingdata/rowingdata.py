...
class summarydata:
	...
	try:
	    ...
	    sumdf = pd.read_csv(readFile,delimiter=';')
	self.sumdf = sumdf
...
class ErgDataParser:
	...
	#if (nrsteps>0):
	#    seconds2 = np.cumsum(seconds.diff()[seconds.diff()>0])
	#else:
	#    seconds2 = seconds

	#seconds2 = seconds.iloc[0]+seconds2-seconds2.iloc[0]
...
#	data.sort(['TimeStamp (sec)'],ascending=True)
	data = data.sort_values(by=' ElapsedTime (sec)',ascending=True)
...
class rowingdata:
	...
	mask = (df[' HRCur (bpm)']<=self.rower.ut2)&(df[' Stroke500mPace (sec/500m)']<300)
	df.loc[mask,'hr_ut2'] = df.loc[mask,' HRCur (bpm)']

	mask = (df[' HRCur (bpm)']<=self.rower.ut1)&(df[' HRCur (bpm)']>self.rower.ut2)&(df[' Stroke500mPace (sec/500m)']<300)
	df.loc[mask,'hr_ut1'] = df.loc[mask,' HRCur (bpm)']

	mask = (df[' HRCur (bpm)']<=self.rower.at)&(df[' HRCur (bpm)']>self.rower.ut1)&(df[' Stroke500mPace (sec/500m)']<300)
	df.loc[mask,'hr_at'] = df.loc[mask,' HRCur (bpm)']

	mask = (df[' HRCur (bpm)']<=self.rower.tr)&(df[' HRCur (bpm)']>self.rower.at)&(df[' Stroke500mPace (sec/500m)']<300)
	df.loc[mask,'hr_tr'] = df.loc[mask,' HRCur (bpm)']

	mask = (df[' HRCur (bpm)']<=self.rower.an)&(df[' HRCur (bpm)']>self.rower.tr)&(df[' Stroke500mPace (sec/500m)']<300)
	df.loc[mask,'hr_an'] = df.loc[mask,' HRCur (bpm)']

	mask = (df[' HRCur (bpm)']>self.rower.an)&(df[' Stroke500mPace (sec/500m)']<300)
...
#	df['cum_dist'] = make_cumvalues_rowingdata(df)[0]
#	df['cum_workdist'] = make_cumvalues_rowingdata(df)[2]
	...
	for idx in intervalnrs:
	    ...
	    intervalduration = tdwork['TimeStamp (sec)'].max()-tdwork['TimeStamp (sec)'].min()
