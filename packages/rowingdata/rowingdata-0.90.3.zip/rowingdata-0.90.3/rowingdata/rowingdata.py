...
class rowingdata:
    ...
    def updateintervaldata_old(self,
			   ivalues,
			   iunits,
			   itypes):
	""" Edits the intervaldata. For example a 2x2000m
	...
	startmeters = 0
	timezero = df.ix[0,'TimeStamp (sec)']-df.ix[0,' ElapsedTime (sec)']
	startseconds = timezero
	

	endseconds = startseconds
	endmeters = startmeters

	# test if we have workoutstate & lapidx
	try:
	    test = df[' lapIdx']
	except:
	    df[' lapIdx'] = 0
...
#		print df.loc[mask,'cum_dist'].max()
		...
		endmeters = df.loc[mask,'cum_dist'].max()

		maxinttime = df.loc[mask,'TimeStamp (sec)'].max() - startseconds
#		print maxinttime
...
#	    print startseconds,startmeters,intervalnr,thetype
