...
class rowingdata:
	...
	for i in range(len(ivalues)):
		...
		mask = (df['cum_dist']>startmeters)&(df['cum_dist']<=endmeters)
		df.loc[mask,'lapIdx'] = intervalnr
		df.loc[mask,' WorkoutState'] = workouttype
		endseconds = df.loc[mask,' ElapsedTime (sec)'].max()
		...
		mask = (df['TimeStamp (sec)']-timezero>startseconds)&(df['TimeStamp (sec)']<=endseconds-timezero)
		df.loc[mask,'lapIdx'] = intervalnr
		df.loc[mask,' WorkoutState'] = workouttype
