...
class RowProParser:
	...
	try:
	    ...
	    self.RP_df = pd.read_csv(RPfile,skiprows=skiprows,
					 ...
					 'HR': lambda x: float(x.replace('.','').replace(',','.')),
					 },
				     engine='python',
				     sep=None)	    
...
class rowingdata:
	...
	for idx in intervalnrs:
	    ...
	    tdrest = td[td[' WorkoutState'].isin(workoutstatesrest)]td
...
#	    print startseconds,startmeters,intervalnr,thetype
	...
	for idx in intervalnrs:
	    ...
	    avghrrest = tdrest[' HRCur (bpm)'].mean()
	    maxhrrest = tdrest[' HRCur (bpm)'].max()
	    avgspmrest = tdrest[' Cadence (stokes/min)'].mean()
	    ...
	    restduration = tdrest[' ElapsedTime (sec)'].max()-intervalduration
	    previoustime = 0
