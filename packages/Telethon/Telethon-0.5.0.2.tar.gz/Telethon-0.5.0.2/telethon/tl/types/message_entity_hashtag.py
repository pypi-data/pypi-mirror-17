from telethon.tl.mtproto_request import MTProtoRequest


class MessageEntityHashtag(MTProtoRequest):
    """Class generated by TLObjects' generator. All changes will be ERASED. Original .tl definition below.
    messageEntityHashtag#6f635b0d offset:int length:int = MessageEntity"""

    def __init__(self, offset, length):
        """
        :param offset: Telegram type: «int».
        :param length: Telegram type: «int».
        """
        super().__init__()
        self.constructor_id = 0x6f635b0d

        self.offset = offset
        self.length = length

    def on_send(self, writer):
        writer.write_int(self.constructor_id, signed=False)
        writer.write_int(self.offset)
        writer.write_int(self.length)

    def on_response(self, reader):
        self.offset = reader.read_int()
        self.length = reader.read_int()

    def __repr__(self):
        return 'messageEntityHashtag#6f635b0d offset:int length:int = MessageEntity'

    def __str__(self):
        return '(messageEntityHashtag (ID: 0x6f635b0d) = (offset={}, length={}))'.format(str(self.offset), str(self.length))
