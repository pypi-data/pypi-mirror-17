...
class Dialog(MTProtoRequest):
    ...
    dialog#66ffba14 flags:None peer:Peer top_message:int read_inbox_max_id:int read_outbox_max_id:int unread_count:int notify_settings:PeerNotifySettings pts:flags.0?int draft:flags.1?DraftMessage = Dialog"""

    def __init__(self, peer, top_message, read_inbox_max_id, read_outbox_max_id, unread_count, notify_settings, pts=None, draft=None):
        """
        ...
        """
        super().__init__()
        self.constructor_id = 0x66ffba14
    ...
    def __repr__(self):
        return 'dialog#66ffba14 flags:None peer:Peer top_message:int read_inbox_max_id:int read_outbox_max_id:int unread_count:int notify_settings:PeerNotifySettings pts:flags.0?int draft:flags.1?DraftMessage = Dialog'

    def __str__(self):
        return '(dialog (ID: 0x66ffba14) = (peer={}, top_message={}, read_inbox_max_id={}, read_outbox_max_id={}, unread_count={}, notify_settings={}, pts={}, draft={}))'.format(str(self.peer), str(self.top_message), str(self.read_inbox_max_id), str(self.read_outbox_max_id), str(self.unread_count), str(self.notify_settings), str(self.pts), str(self.draft))
