...
def check_list(videos, name, show_seen=False, reg=None, reverse=False):
    ...
    for index, video in enumerate(videos):
        ...
        cprint('[green]┌-[/green] [bold]{}[/bold]'.format(title))
...
def handle_download(proc, video, try_again=False):
    ...
    height = size.lines
    move = '\033[F'
    arrow = '\033[92m└>\033[0m'
    if try_again:
        text = 'Trying again.'
    else:
        text = 'Starting download.'
    sys.stderr.write('\r{}{} {} Press Ctrl+C to cancel.'.format(move, 
                     arrow, text))
    tmp = b''
    downloaded = False
    while proc.poll() is None:
        ...
        if tmp.endswith(b'\r'):
            ...
            color_progress(perc, '', '',
                           prefix='{} Press Ctrl+C to cancel '.format(arrow),
