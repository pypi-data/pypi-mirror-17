...
class VideoList:
    ...
    def create_list(self, getch=True):
        ...
        if self.vidlist != []:
            ...
            self.screen.addstr(self.height-1, 0, statbar, curses.A_REVERSE)
        if getch:
    ...
    def k_97(self):
        ...
        title = video['title']
        self.data[title] = {'info': {},  'perc': '0%', 'status': 'pending'}
        url = 'https://youtube.com/watch?v={}'.format(video['id'])
        ydl = youtube_dl.YoutubeDL({'logger': VoidLogger(),
    ...
    def k_100(self):
        video = self.vidlist[self.pos+self.off]
        title = video['title']
        self.data[title] = {'perc': '0%', 'status': 'pending', 'info': {}}
