...
(function () {
    ...
    moviesController.$inject = ["$document", "$mdDialog", "$mdPanel", "$sce", "addMovieService", "moviesService"];
    angular
        .module('plugins.movies')
...
$templateCache.put("layout.tmpl.html","<div class=\"header md-whiteframe-4dp\" ng-class=\"menuMini ? \'header-mini\': \'header-full\'\"><div class=\"logo\"><a href=\"#/\"></a></div><tool-bar></tool-bar></div><div layout=\"row\" flex=\"\" ng-class=\"menuMini ? \'nav-menu-mini\': \'nav-menu-full\'\"><side-nav layout=\"column\"></side-nav><ui-view id=\"content\"></ui-view></div>");
...
$templateCache.put("plugins/history/history.tmpl.html","<md-content flex=\"\"><md-input-container><label>Search</label> <input ng-model=\"vm.taskName\" ng-change=\"vm.search()\" ng-model-options=\"{ debounce: 1000 }\"><md-icon class=\"fa fa-lg fa-question-circle\"><md-tooltip md-direction=\"top\">Filtering happens on exact task name!</md-tooltip></md-icon></md-input-container><section ng-repeat=\"(key, value) in vm.entries | groupBy: \'time | limitTo : 10\'\"><md-subheader class=\"md-primary\">{{ key }}</md-subheader><md-list layout-padding=\"\"><md-list-item class=\"md-2-line\" ng-repeat=\"entry in value\"><div class=\"md-list-item-text\"><h3>{{ entry.title }}</h3><p>{{ entry.task }}</p></div></md-list-item></md-list></section></md-content>");
...
$templateCache.put("plugins/seen/seen.tmpl.html","<md-content layout=\"column\" flex=\"\"><seen-entry ng-repeat=\"entry in vm.entries\" entry=\"entry\" delete-entry=\"vm.deleteEntry(entry)\"></seen-entry></md-content>");
...
$templateCache.put("plugins/seen/components/seen-entry/seen-entry.tmpl.html","<md-card><md-card-header><md-card-header-text><span class=\"md-title\">{{vm.entry.title}}</span> <span class=\"md-subhead\" ng-if=\"vm.entry.local\">Local to <b>{{vm.entry.task}}</b></span> <span class=\"md-subhead\" ng-if=\"!vm.entry.local\">Global</span></md-card-header-text></md-card-header><md-card-content><md-list><md-list-item ng-repeat=\"field in ::vm.entry.fields\"><div class=\"md-list-item-text\" layout=\"row\"><p><b>{{field.field_name}}:</b> {{field.value}}</p></div></md-list-item></md-list></md-card-content><md-card-actions><md-button ng-click=\"vm.deleteEntry()\">Delete</md-button></md-card-actions></md-card>");
