...
from rlstm.scores import get_log_sparsity, flatten_and_get_auc
from rlstm.common_util import format_preds, merge_two_dicts
...
class RLSTMShareWeights(object):
    ...
    def get_parent_weights(self, pipeline):
        ...
        if self.parent_weights_idx:
            return weights[self.parent_weights_idx]
        return weights
...
class RLSTMNode(object):
    ...
    def eval(self, tr_x, tr_y, te_x, te_y):
        ...
        # loglikelihoods
        if self.type in ['lr', 'hmm-ffbs', 'hmm-sgd']:
        ...
        # aucs
        tr_pr1_set, tr_pr0_set = pred_fun(trained_weights, tr_x)
        te_pr1_set, te_pr0_set = pred_fun(trained_weights, te_x)

        tr_pr_set = format_preds(tr_pr1_set, tr_pr0_set)
        te_pr_set = format_preds(te_pr1_set, te_pr0_set)

        metrics['{}:tr_auc'.format(self.name)] = \
            flatten_and_get_auc(tr_y, tr_pr_set)
        metrics['{}:te_auc'.format(self.name)] = \
            flatten_and_get_auc(te_y, te_pr_set)
