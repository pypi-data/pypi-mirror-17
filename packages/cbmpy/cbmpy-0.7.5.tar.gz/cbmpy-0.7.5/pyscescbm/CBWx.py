...
Last edit: $Author: bgoli $ ($Id: CBWx.py 305 2015-04-23 15:18:31Z bgoli $)
...
if HAVE_WX:
    class ModelEditor(wx.Frame):
        ...
        def BuildMainPanel(self):
            ...
            self.writeCmd("#\n#############################\n")
            self.writeCmd("import pyscescbm\n")
            self.writeCmd("# cmod = pyscescbm.CBRead.readSBML2FBA(ModelFile, ModelDir)\n")
    ...
    ##                     grid.SetCellValue(r, c, '%s' % self._cmod_.getFluxBoundByReactionID(reactions[r].getPid(), 'upper').getValue())
        ...
        def EVT_BUT_optimise(self, event):
            ...
            self.UpdateModelStatus()
            self.writeCmd("pyscescbm.CBSolver.analyzeModel(cmod)")
            del wait
            event.Skip()
        ...
        def EVT_BUT_minsum(self,event):
            ...
            self.UpdateRGridData()
            self.UpdateModelStatus()
            self.writeCmd("pyscescbm.CBSolver.cplx_MinimizeSumOfAbsFluxes(cmod, selected_reactions=None, pre_opt=True, tol=None, objF2constr=True, rhs_sense='lower', optPercentage=100.0, objective_coefficients={}, return_lp_obj=False)")
            del wait
            event.Skip()
            ##  pyscescbm.CBSolver.cplx_MinimizeSumOfAbsFluxes
