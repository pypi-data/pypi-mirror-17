...
Last edit: $Author: bgoli $ ($Id: _multicoreenvfva.py 305 2015-04-23 15:18:31Z bgoli $)
...
def funcFVA(x, r, tol, rhs_sense, optPercentage, work_dir, debug):
    x = pyscescbm.CBCPLEX.cplx_getModelFromLP(x)
    x.parameters.threads.set(4)
    try:

        fva, fvan = pyscescbm.CBCPLEX.cplx_MultiFluxVariabilityAnalysis(x, r, tol, rhs_sense, optPercentage, work_dir, debug)
...
def multiCoreEnvFVA(MEargs, procs=2, timeout=None):
    ...
    rid = MEargs[1]
    work_dir = MEargs[5]
    PRs = [d_[0] for d_ in list(pyscescbm.CBMultiCore.grouper(int(math.ceil(len(rid)/float(procs))), range(len(rid))))]
