...
Last edit: $Author: bgoli $ ($Id: CBXML.py 376 2015-09-10 14:50:28Z bgoli $)
...
def sbml_readSBML3FBC(fname, work_dir=None, return_sbml_model=False, xoptions={}):
    ...
    # assert FBCplg != None, "\n\nSBML FBC package required, see http://sbml.org/Documents/Specifications/SBML_Level_3/Packages/Flux_Balance_Constraints_%28flux%29 for more details"
    if FBCplg == None:
        print('\nModel is not an SBML3 FBC model. Please try cbm.readCOBRASBML(\'{}\') for models encoded in the COBRA dialect or cbm.readSBML2FBA(\'{}\') for models in FAME format.'.format(fname,fname))
        return None
    ...
    # prototype groups support
    GRP = M.getPlugin(str("groups"))
    ...
    for t_ in uridict:
        if len(uridict[t_]) > 0:
            ...
            if t_ in CBM2BQMAP:
                ...
                cv.setBiologicalQualifierType(CBM2BQMAP[t_])
                #print(CBM2BQMAP[t_])
            else:
                cv = libsbml.CVTerm(libsbml.MODEL_QUALIFIER)
                cv.setModelQualifierType(CBM2BQMMAP[t_])
