...
if MPI.COMM_WORLD.rank == 0:
    ...
    pool.start()
    try:
        xsum = np.concatenate(pool.eval_method("sum", x, (n,), op), axis=0)
        xmul = np.concatenate(pool.eval_method("mul", x, (n,), op), axis=0)
    finally:
        pool.stop()

    pool.start()
    try:
        xfail = np.concatenate(pool.eval_method("sum", None, (n,), op), axis=0)
    except:
