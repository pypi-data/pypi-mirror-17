...
class Endpoint:
    ...
    @property
    def model_name(self):
        return self.inflector.pluralize(self.model._meta.model_name.lower())
    ...
    def _get_field_dict(self, field):
        ...
        rv = {
            'name': name,
            'label': name.title() if name != '__str__' else serializer_instance.Meta.model.__name__,
        ...
        if getattr(field_instance, 'choices', None) is not None:
            if isinstance(field_instance, relations.PrimaryKeyRelatedField):
