...
class CorpusProcessor:
    ...
    def parse_cmd_arguments():
        ...
        parser = configargparse.ArgumentParser(description="Process a text file or a \
            directory tree of files using multiprocess")
        parser.add_argument(
    ...
    def rebuild_tread_parameters(self, thread_config, common, process_parser):
    ...
    def rebuild_common_parameters(self, common, process_parser):
        config_files = [arg_file for arg_file in common.split(self.separator)]
        wrapper_parser = configargparse.ArgumentParser(parents=[process_parser],
                                                       default_config_files=config_files, add_help=False)
        # Only process the config files
        process_arguments = wrapper_parser.parse_args(args="")
    ...
    def process_files(self, file_list, process_arguments, jobs=8, verbose=False):
    ...
    def run_corpus(self, filename=None):
        ...
        if arguments.report:
            ...
            common_arguments = self.rebuild_common_parameters(
                arguments.common, process_parser)
