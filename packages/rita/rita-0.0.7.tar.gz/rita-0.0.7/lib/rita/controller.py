...
from sqlobject import SOIntCol, SOBoolCol, SODateCol, SODateTimeCol
...
_CACHED_HTML_TEMPLATE = {} #キャッシュ済のhtmlテンプレート
...
class BaseController(object):
    ...
    @staticmethod
    def clear_html_templates():
        global _CACHED_HTML_TEMPLATE
        _CACHED_HTML_TEMPLATE = {}
    ...
    def render_string(self, file_name=None):
        ...
        #テンプレートファイルを探す
        if (folder, file_name) not in _CACHED_HTML_TEMPLATE:
            mylookup = None
            if sys.version_info.major >= 3:
            ...
            _CACHED_HTML_TEMPLATE[(folder, file_name)] = mylookup.get_template(file_name)
        return _CACHED_HTML_TEMPLATE[(folder, file_name)].render_unicode(**self.attr)
...
class BaseFormController(BaseController):
    ...
    def prepare_form(self):
        ...
        if target != None:
            for field_name in self.target_field_names:
    ...
    def get_form_state(self):
        ...
        if self.get_param("__mode__") == "delete":
            ...
            if self.is_post_back() and self._validate_hidden_token() and self._validate_form():
                if self.show_input_confirm == False or action == "complete":
                    if cancel_clicked:
                        return FormState.INPUTTING
                    else:
                        return FormState.UPDATE_READY
                else:
                    if cancel_clicked:
                        return FormState.CANCELED
                    else:
    ...
    def get_param(self, name, default=None):
        ...
        if self.target_class != None:
            if name in self.fields:
                column = self.target_class.sqlmeta.columns[name]
    ...
    def get_target(self, use_cache=True):
        ...
        if self.get_primary_key() != None:
            ret = self.target_class.get(self.get_primary_key())
        self._cached_target = ret
        return ret

    def update_target(self, options:dict={}, connection=None):
        def get_value(name):
            column = self.target_class.sqlmeta.columns[name]
            value = self.form[name].value
            if type(column) is SOIntCol:
                value = int(value)
            if type(column) is SOBoolCol:
                value = True if value.lower() in ["1", "true"] else False
            return value                          
        ...
        if pk == None:
            ...
            for name in self.target_field_names:
                tmpdic[name] = get_value(name)
            if connection:
                tmpdic["connection"] = connection
            return self.target_class(**tmpdic)
        else:
            #既存レコード
            target = None
            if connection:
                target = self.target_class.get(pk, connection=connection)
            else:
                target = self.target_class.get(pk)
            for name in self.target_field_names:
                setattr(target, name, get_value(name))
    ...
    def render_form_end(self):
        ...
        if self.get_form_state() == FormState.DELETE_CONFIRM or (self.show_input_confirm == False or self.get_form_state() == FormState.INPUT_CONFIRM):
    ...
    def render_submit(self):
        if self.get_form_state() == FormState.DELETE_CONFIRM:
            return """<input type="submit" class="btn btn-danger" value="削除">"""
        elif self.show_input_confirm == False or self.get_form_state() == FormState.INPUT_CONFIRM:
            return """<input type="submit" class="btn btn-success" value="送信">"""
        else:
            return """<input type="submit" class="btn btn-success" value="入力内容の確認">"""

    def render_cancel(self):
        value = self.attr["_"]("キャンセル")
        if self.get_form_state() == FormState.INPUT_CONFIRM:
            value = self.attr["_"]("入力画面に戻る")
        return """<input type="submit" class="btn btn-default" name="_CANCEL" value="%s">""" % (value)
    #end:フォーム描画用メソッド
    ...
    def _validate_form(self):
        ...
        for field_name in self.fields:
            ...
            column = None
            if self.target_class != None:
                column = self.target_class.sqlmeta.columns[field_name]
            #明示的に設定されたバリデータとDBスキーマでの入力チェック
            for validator in Validator.getDefaultValidators(self.attr["_"], column) + self.fields[field_name].validators:
        ...
        ret = self.validate_form() and ret 
