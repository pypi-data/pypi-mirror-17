...
class TextField(Field):
    ...
    def render(self, name, value, readonly=False, opt=None):
        if readonly:
            ...
            return """<input type="text" class="form-control" id="c_%(name)s" name="%(name)s" value="%(value)s"/>""" % {"name":name, "value":self.html_escape(value)}
...
class TextAreaField(Field):
    ...
    def render(self, name, value, readonly=False, opt=None):
        if readonly:
            ...
            return """<textarea id="c_%(name)s" class="form-control" name="%(name)s">%(value)s</textarea>""" % {"name":name, "value":self.html_escape(value)}
