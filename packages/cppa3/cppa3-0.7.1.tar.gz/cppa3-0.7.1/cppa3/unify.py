...
- Wildcard actions new feature:
Assumption is that at most one is present on each side
initially: if present on one side, must be present on other side too; no other non-wildcard
actions allowed.
potential future enhancement:  if present on one side only, is target for unification
with all actions on other side.

- Wildcard service new feature
...
def unify(acpp, bcpp, nsmap = {}, agreementid = None):

    """
    @param acpp: a CPP
    @param bcpp: another CPP
    @param nsmap: optional dictionary of additional namespaces
    @param agreementid:  optional function to determine the agreement identifier
    @return:
    """
    unifier = CPABuilder(nsmap, agreementid)
...
class CPABuilder():
    ...
    def __init__(self, nsmap = {}, agreementid = None):
        """
        @param nsmap: optional dictionary of additional namespaces
        @param agreementid:  optional function to determine the agreement identifier
        @return:
        ...
        self.agreementid = self._agreementid
        if agreementid != None:
    ...
    def reset_caches(self):
        ...
        # In the future, other data may be cached as well.
    ...
    def unify_profileinfo(self, cpa, acpp, bcpp):
        ...
        agreementid.text = self.agreementid(acpp, bcpp)
    ...
    def _agreementid(self, acpp, bcpp):
        acppid = acpp.xpath('child::cppa:ProfileInfo/cppa:ProfileIdentifier/text()',
                            namespaces=self.NSMAP)[0]
        bcppid = bcpp.xpath('child::cppa:ProfileInfo/cppa:ProfileIdentifier/text()',
                            namespaces=self.NSMAP)[0]
    ...
    def unify_servicebinding_list(self,
        ...
        if (partyrole is None or partyrole == arole) \
            ...
            try:
                ...
                last_exception = None
                for a_servicebinding in a_servicebinding_list:
                    try:
        ...
        try:
            ...
            for a_binding in asendbinding_list:
                ...
                if len(b_binding_list) == 1:
                    ...
                    # Properties
                    self.unify_properties(aid, a_binding, bid, b_binding, actionbinding)
    ...
    def unify_named_channel(self,
        ...
        self.unify_signing_cert_and_anchor(acppid, acpp, bcppid, bcpp, a_channel, b_channel, direction)
        self.unify_encryption_cert_and_anchor(acppid, acpp, bcppid, bcpp, a_channel, b_channel, direction)
        ...
        self.record_dependency(ab_channelid, 'transport', (acppid, a_transport_id, bcppid, b_transport_id))
    ...
    EDIINT
    """
    def unify_ediint_channel(self, acpp, bcpp, abxid, a_channel, b_channel, binding, direction):
        (acppid, axid, bcppid, bxid) = abxid
    ...
    ebMS2
    """
    def unify_ebms2_channel(self, acpp, bcpp, abxid, a_channel, b_channel, binding, direction):
    ...
    Web Services
    """
    def unify_ws_channel(self, acpp, bcpp, abxid, a_channel, b_channel, binding, direction):
        (acppid, axid, bcppid, bxid) = abxid
        self.unify_channel_descriptions(abxid, a_channel, b_channel, binding)
    ...
    Transport Channels
    """
    def unify_transport_channel(self, acpp, bcpp, abxid, a_channel, b_channel, binding, direction):
        (acppid, acid, bcppid, bcid) = abxid
        self.unify_transport_elements(acppid, acpp, bcppid, bcpp, a_channel, b_channel,
                                      abxid, binding, direction)
        self.unify_channel_descriptions(abxid, a_channel, b_channel, binding)
    ...
    ebMS3 and AS4
    """
    def unify_ebms3_channel(self, acpp, bcpp, abxid, a_channel, b_channel, ebmsbinding, direction):
        (acppid, acid, bcppid, bcid) = abxid
        ...
        self.unify_simple_subelement(a_channel, b_channel, ebmsbinding, 'cppa', 'SOAPVersion')
    ...
    def unify_as_response(self, aid, a_binding, bid, b_binding, ab_binding):
        ...
        if xsd_boolean(aval) is xsd_boolean(bval):
            # both are set with a the same Boolean (true or false) value or both are None
            ...
            if len(ax509certL) > 0:
                ...
                for b_anchor_ref in banchor.xpath('cppa:AnchorCertificateRef',
                    ...
                    if check_x509_data_content(signingcertid, ax509rootcert, b_anchor_certid, bcpp):
                        rootfound = True
                if not rootfound:
                    for embedded_cert in banchor.xpath('cppa:Certificate', namespaces=self.NSMAP):
    ...
    def unify_properties(self, aid, a_binding, bid, b_binding, actionbinding):
        ...
        if len(a_property_list) != len(b_property_list):
            ...
            for aprop in a_property_list:
                ...
                bpropl = b_binding.xpath(xpq.format(aname,a_min,a_max),
                                        namespaces=self.NSMAP)
                if len(bpropl) == 0:
                ...
                for counter, achild in enumerate(achildren, 1):
                    ...
                    for bchild in bchildren:
                        try:
                            ...
                            logging.info("Skipping non-matching {} #{}".format(achild.tag, counter))
                        else:
    ...
    def unify_complex_subelement(self,
        ...
        try:
            ...
            elif aelementcount != belementcount:
    ...
    def inline_channel_features(self, cpp):
        for binding_att in [ 'securityBinding',
                             'reliableMessagingBinding']:

            for element in cpp.xpath('//node()[@{}]'.format(binding_att),
                                     namespaces= self.NSMAP):
                logging.info(element)
                binding_element = element.tag
                binding_id = element.get(binding_att)
