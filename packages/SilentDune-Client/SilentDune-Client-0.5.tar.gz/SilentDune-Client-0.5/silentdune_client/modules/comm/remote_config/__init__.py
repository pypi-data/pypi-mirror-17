...
class SilentDuneRemoteConfigModule(BaseModule):
    ...
    def validate_arguments(self, args):
        ...
        if not self._uri.scheme or not self._uri.hostname or self._uri.scheme not in ['http', 'https', 'ftp']:
    ...
    def process_loop(self):
        ...
        if self._startup:
            ...
            # See if we need to ask the firewall to allow all dns queries
            if not self.get_ip_from_hostname(self._uri.hostname, None):
                self._dns_error_count += 1

                # Wait 10 seconds before enabling all DNS access. If auto-discovery module is enabled,
                # DNS should resolve within 10 seconds.
                if self._dns_error_count > 9:
                    ...
                    self._dns_error_count = 0

                return

            else:
                # See if we need to ask the firewall to disable all dns access
                if self._all_dns_access_enabled:
                    self._all_dns_access_enabled = False
                    _logger.debug(
                        '{0}: Asking Firewall Module to disable generic DNS access.'.format(self.get_name()))
    ...
    def get_ip_from_hostname(self, hostname, port):
        ...
        try:
            ...
            _logger.error('{0} unable to resolve remote config hostname'.format(self.get_name()))
            return None
    ...
    def apply_remote_config(self):
        ...
        try:
            rq = requests.get(self._remote_config_url)
        ...
        if self._remote_config_dns_name:
            ...
            if not rc.hash_method or rc.hash_method not in ['sha1', 'sha256', 'sha512']:
                _logger.error('{0}: unsupported hashing algorithm ({1})'.format(self.get_name(), rc.hash_method))
                return

            if rc.hash_method == 'sha1':
                hash_value = hashlib.sha1(rq.text).hexdigest()
            elif rc.hash_method == 'sha256':
                hash_value = hashlib.sha256(rq.text).hexdigest()
            elif rc.hash_method == 'sha512':
                hash_value = hashlib.sha512(rq.text).hexdigest()
        ...
        for rule in rc.rules:
            print("{0}/{1}, {2}, {3}, {4}, {5}".format(rule.host, rule.mask, rule.port, rule.protocol,
                                                       rule.uid, rule.gid))
    ...
    def get_remote_dns_hash(self):
        ...
        # See if we should append hostname to query
        if self._uri.hostname not in query:
            query += '.{0}'.format(self._uri.hostname)
