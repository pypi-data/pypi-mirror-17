...
class MorphologyResolver():
  ...
  def resolve_morph(self, yaml):
    if yaml.get('kind', '') == 'assemblage':
      ...
      if 'build-system' in yaml:
        x = deepcopy(self.defaults['build-systems'][yaml['build-system']])
        x.update(yaml)
        x.pop('build-system')
        return x
      return yaml
...
class Actuator():
  ...
  def __init__(self, directories, aliases, defaults, tree_server=None):
  ...
  def get_tree_for_chunk(self, chunk):
    ...
    if not os.path.exists(gitdir):
      if self._tree_server is not None:
        try:
  ...
  def build_assemblage(self, assemblage):
    ...
    artifact_store = lambda x: os.path.join(self._directories['artifacts'], "%s.%s" % (x['name'], x['cache']))
    ...
    for x in build['contents']:
      ...
      for z in self._defaults.get('build-steps', []):
        for c in x.get(z, []):
          env = self.setup_env(DESTDIR=install,PREFIX=x.get('prefix', '/usr'))
          self.run_sandbox(c, root_fs=sandbox, name=x['name'], env=env, bootstrapped=(x['build-mode'] == 'bootstrap'))
  ...
  def run_sandbox(self, command, root_fs, name, extra_mounts=[], env=None, bootstrapped=False):
    mounts = [(os.path.join(root_fs, m), m, None) for m in extra_mounts]
    writables = ["%s/%s.build" % (root_fs, name),
                 "%s/%s.install" % (root_fs, name)]
    if not bootstrapped:
    ...
    executor=sandboxlib.get_executor("chroot")
