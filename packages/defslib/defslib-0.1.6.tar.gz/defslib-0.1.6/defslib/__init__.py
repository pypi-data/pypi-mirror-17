...
class Actuator():
  ...
  def flatten_assemblage(self, assemblage):
    contents = assemblage['contents'].copy()
    asses = list(filter(lambda x: x.get('kind', None) == 'assemblage', contents))
    for x in asses:
      sub = self.flatten_assemblage(x)['contents'].copy()
      supports = list(filter(lambda z: z['name'] in x.get('build-depends', []), contents)).copy()
      dependents = list(filter(lambda z: x['name'] in z.get('build-depends', []), contents)).copy()
      noncomps = list(filter(lambda z: z['name'] not in x.get('build-depends', []) and
                                       x['name'] not in z.get('build-depends'), contents))
      for i in sub:
        i['build-depends'] = i.get('build-depends', []) + list(map(lambda z: z['name'], supports))
      for y in dependents:
        y['build-depends'].remove(x['name'])
        y['build-depends'] = y.get('build-depends', []) + list(map(lambda t: t['name'], sub))
      contents = supports + dependents + sub + noncomps
