...
Cranch <https://en.wikipedia.org/wiki/William_Cranch>`_ and `Alex
Dallas <https://en.wikipedia.org/wiki/Alexander_J._Dallas_%28statesman%29>`_.
...
1. This work was originally deployed in the
   `CourtListener <https://www.courtlistener.com>`_ citation finder
...
2. An extension for Firefox known as the `Free Law
   Ferret <http://citationstylist.org/2013/08/20/free-law-ferret-document-to-cited-cases-in-a-click/>`_
   uses this code to find citations in your browser as you read things
   -- all over the Web.

3. A Node module called
   `Walverine <https://github.com/adelevie/walverine>`_ uses an
...
3. The ``variations`` key consists of data from local rules, found
   ...
   Legal Abbreviations <http://www.legalabbrevs.cardiff.ac.uk/>`_. We
...
1. Mississippi supports neutral citations, but does so in their own
   ...
   rule <http://www.aallnet.org/main-menu/Advocacy/access/citation/neutralrules/rules-ms.html>`_.
...
2. New Mexico dates confirmed via the `table
   here <http://www.nmcompcomm.us/nmcases/pdf/NM%20Reports%20to%20Official%20-%20Vols.%201-75.pdf>`_.
...
-  1.1: All dates are dialed in to the nearest year for every edition of
   ...
   provides). See `issue
   #7 <https://github.com/freelawproject/reporters-db/issues/7>`_
