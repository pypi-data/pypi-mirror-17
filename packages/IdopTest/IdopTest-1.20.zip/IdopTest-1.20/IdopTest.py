import logging , sys , time, datetime , os , threading , IdopTestLogic ,pip , subprocess
...
def upgradeMe():
    try:
		IdopTestLogic.ci_print('upgrade ')
		IdopTest = pip.main(['install','--upgrade','IdopTest'])
    except Exception as inst:
        print "Error upgrade " + str(inst)        
        logging.warning('Error in upgrade :: ' + str(inst))

def installMe():
    subprocess.call(['pip', 'install', 'IdopTest'])
	
def reinstallMe():
    subprocess.call(['pip' , 'install', 'IdopTest','--upgrade'])
...
if (len(sys.argv)>1 and sys.argv[1]=='ver'):
    ver()
if (len(sys.argv)>1 and sys.argv[1]=='reloadMe'):
    reloadMe()
if (len(sys.argv)>1 and sys.argv[1]=='upgradeMe'):
    upgradeMe()
if (len(sys.argv)>1 and sys.argv[1]=='installMe'):
    installMe()
if (len(sys.argv)>1 and sys.argv[1]=='reinstallMe'):
    reinstallMe()
