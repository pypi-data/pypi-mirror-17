    ...
    def loads(self, loadstring, softload=False, context=None): # TODO make use of context in error reporting
        ...
        for prop in self.__dynamic_properties__:
            ...
            if prop not in attriblist:
                if prop in self.__mandatory_properties__ and softload is False:
                    raise AttributeError('attribute %s required by %s class definition '
                                         'file but not found in data file %s' %
                                         (prop, striptoclassname(self.__class__), self.__context__['file']))
        ...
        for attrname, attrval in ret.items():
            if attrname not in self.__dynamic_properties__:
                ...
                # TODO if data is badly entered (child classes missing attribs, a TypeError occurs here) - needs better error messaging
                try:
                    ...
                    raise TypeError('got an error when trying to load data for %s from %s: %s'
                                    % (striptoclassname(self.__class__), self.__context__['file'], err))
