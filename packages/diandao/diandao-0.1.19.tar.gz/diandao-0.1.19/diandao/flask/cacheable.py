...
import time
from inspect import isclass, ismethod, isfunction, getargspec
...
def flushcache(method, *args, **kwargs):
    ...
    method = method._cache_method if hasattr(method,"_cache_method") else None
    if path and method:
        cache = ModelCache.fetch(path)
        if not len(args) and not len(kwargs):
            ...
            return True
        # 清空指定参数缓存
        slug = argslug(method, True, *args, **kwargs)
        redis = cache.handle()
        keys = redis.keys( cache.slug(slug) )
        if len(keys):
            redis.delete(*keys)
        return True
    else:
...
def argslug(method, wildcard=False, *args, **kwargs):
    ...
    for i, name in enumerate(argnames):
        ...
        if kwargs.has_key(name):
            v = kwargs.pop(name)
            has_value = True
        if not has_value and len(argscopy) > i:
            v = args[i]
            has_value = True
