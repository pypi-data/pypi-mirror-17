...
class XGNode(object):
    ...
    def __init__(self, name='', type='string', value='', node_id='',
                 nodes=None, flags=None, subop=None, attrs=None):
        self.type = type
        # Child nodes of this node (in tree form)
...
class XGNodeDict(object):
    ...
    def __init__(self, data=None, values_only=False):
        ...
        if data is not None:
            if isinstance(data, dict):
                ...
                raise ValueError('Invalid "data" type: {0}'.format(
                                 data.__class__))
...
# Simple coercion function for JSON requests
def coerce_arg(value, style=None):
    """Returns the value in the specified style.

    Arguments:
        value -- the value to be coerced
        style -- how to interpret the value:
                    str (default)
                    bool
                    csvstr
                    int
    ...
    if style in (None, 'str'):
        ...
        # Datetime as a YYYY-MM-DD HH:MM string
        if hasattr(value, 'strftime') and callable(value.strftime):
            return value.strftime('%Y-%m-%d %H:%M')
        else:
            return str(value)
    elif style == 'listofstr':
        # List of strings
