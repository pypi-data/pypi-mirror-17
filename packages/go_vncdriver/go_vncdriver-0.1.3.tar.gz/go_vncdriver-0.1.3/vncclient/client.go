...
FindAuth:
	...
	if auth == nil {
		return fmt.Errorf("did not support server-requested auth scheme: %#v", securityType)
	}
...
func (c *ClientConn) mainLoop() {
	...
	for {
		...
		if err := binary.Read(c.c, binary.BigEndian, &messageType); err != nil {
			if c.errorCh != nil {
				// Technically should do non-blocking write
				c.errorCh <- errors.Annotate(err, "could not read message type")
			}
			break
		}
		...
		if !ok {
			...
			if c.errorCh != nil {
				c.errorCh <- errors.Errorf("no such message type: %d", messageType)
			}
			break
		}
