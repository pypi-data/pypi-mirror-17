...
static int PyArg_ParseTuple_list_O_bang_O(PyObject *args, PyObject **a, PyObject **b) {
...
func GoVNCDriver_VNCSession_c_init(self *C.go_vncdriver_VNCSession_object, args *C.PyObject) C.int {
	...
	if C.PyArg_ParseTuple_list_O_bang_O(args, listObj, errorCallable) == 0 {
		return C.int(-1)
	}
...
func convertEventPy(eventPy *C.PyObject) (event gymvnc.VNCEvent, ok bool) {
	...
	typePystr := C.PyString_AsString(t)
	if typePystr == nil {
		return
	}
...
func PyObject_Repr(obj *C.PyObject) (string, bool) {
	...
	strObj := C.PyString_AsString(res)
	if strObj == nil {
		return "", false
	}
