...
class OPLogFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if trace_string:
            ...
            if (os.environ.get('AIRBRAKE_KEY')):
                try:
                    airbrake_logger = airbrake.getLogger()
                    airbrake_logger("%s: %s" % (kind, hash))
                except:
                    logging.info("Exception calling airbrake key: %s, id: %s, env: %s" % (os.environ.get('AIRBRAKE_KEY', 'Not Set'), os.environ.get('AIRBRAKE_PROJECT_ID', 'Not Set'), os.environ.get('AIRBRAKE_ENVIRONMENT', 'Not Set')))
