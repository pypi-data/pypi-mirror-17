...
class OPLogFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if trace_string:
            ...
            #if (os.environ.get('AIRBRAKE_API_KEY')):
            #    try:
            #        airbrake_logger = airbrake.getLogger()
            #        airbrake_logger.exception("%s: %s" % (kind, hash_value))
            #    except:
            #        #logging.info("Exception calling airbrake key: %s, id: %s, env: %s" % (os.environ.get('AIRBRAKE_API_KEY', 'Not Set'), os.environ.get('AIRBRAKE_PROJECT_ID', 'Not Set'), os.environ.get('AIRBRAKE_ENVIRONMENT', 'Not Set')))
            #        print "Exception calling airbrake key: %s, id: %s, env: %s" % (os.environ.get('AIRBRAKE_API_KEY', 'Not Set'), os.environ.get('AIRBRAKE_PROJECT_ID', 'Not Set'), os.environ.get('AIRBRAKE_ENVIRONMENT', 'Not Set'))
...
class MyHandler(pywsgi.WSGIHandler):
    ...
    def format_request(self):
        ...
        return '%s - - "%s" %s %s %5.2f' % (
            ...
            length, response_time)

    def log_request(self):
        tracetoken = self.headers.getheader('X-Trace-Token')
