...
Usage:
    headjack read sentToKindle [-s <pathToSettingsFile>]
...
def main(arguments=None):
    ...
    *The main function used when ``cl_utils.py`` is run as a single script from the cl, or when installed as a cl command*
    ...
    arguments, settings, log, dbConn, remoteDB = su.setup()
    ...
    if "dbConn" in locals() and dbConn:
        dbConn.commit()
        dbConn.close()
    if "remoteDB" in locals() and remoteDB:
        remoteDB.commit()
        remoteDB.close()
    ## FINISH LOGGING ##
