    ...
    **Usage:**
    ...
    def _get_data_to_send(
        ...
        **Return:**
        ...
        if docType == "pdf":
            sqlQuery = u"""
                select * from `reading-list` where  kind = "imported-pdf" and sentToKindle =0 and workflowStage = "reading"  order by dateCreated
            """ % locals()
        elif docType == "web":
            sqlQuery = u"""
                select * from `reading-list` where  kind = "webpage" and sentToKindle =0 and workflowStage = "reading" limit 5 order by dateCreated
            """ % locals()
