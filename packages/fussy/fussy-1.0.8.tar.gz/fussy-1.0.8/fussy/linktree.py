        ...
        for filename in filenames + subdirs:
            ...
            if os.path.islink( file_source ):
                ...
                if os.path.lexists( file_final ):
                    ...
                    create_link( file_source, file_final, link )
                else:
                    log.info( 'Linking: %s', file_final )
                    create_link( file_source, file_final, link )

def create_link( file_source, file_final, link ):
    try:
        temp = file_final + '~'
...
def main():
    ...
    if args:
        ...
        if args and not options.target:
            ...
            args = args[1:]
    link_tree( options.directory, options.target, symbolic = (not options.hardlink) )
    return 0
