...
def checkpoint(count):
    ...
    if cfg_checkpoint_count > 0 and count % cfg_checkpoint_count == 0:
        print("Checkpoint after %d commits\n" % count, file=sys.stderr)
        print('Checkpoint\n')
    return count
...
def export_file_contents(ctx, manifest, files, hgtags, encoding=''):
    ...
    for file in files:
        ...
        if not hgtags and file == ".hgtags":
            print('Skip %s' % (file), file=sys.stderr)
            continue
        d = ctx.filectx(file).data()
        if encoding:
            filename = file.decode(encoding).encode('utf-8')
        else:
            filename = file
        print('M %s inline %s' % (gitmode(manifest.flags(file)),
                                                     strip_leading_slash(filename)))
        print('data %d' % len(d)) # had some trouble with size()
        print(d)
        count += 1
        if count % cfg_export_boundary == 0:
            print('Exported %d/%d files' % (count, max), file=sys.stderr)
...
def sanitize_name(name, what="branch", mapping={}):
    ...
    if n[-1] in ('/', '.'):
        n=n[:-1] + '_'
    n = '/'.join(dot(x) for x in n.split('/'))
    p = re.compile('_+')
    n = p.sub('_', n)

    if n != name:
        print('Warning: sanitized %s [%s] to [%s]' % (what, name, n), file=sys.stderr)
    return n
...
def export_commit(ui, repo, revision, old_marks, max, count, authors,
    ...
    (revnode, _, user, (time, timezone), files, desc, branch, _
        ) = get_changeset(ui, repo, revision, authors, encoding)
    ...
    if len(parents) == 0 and revision != 0:
        print('reset refs/heads/%s' % branch)

    print('commit refs/heads/%s' % branch)
    print('mark :%d' % (revision + 1))
    if sob:
        print('author %s %d %s' % (get_author(desc,user,authors),time,timezone))
    print('committer %s %d %s' % (user,time,timezone))
    print('data %d' % (len(desc)+1)) # wtf?
    print(desc)
    wr()
...
def sh_main():
        ...
        if args.repo:
                ...
                try:
                        ...
                        raise Exception('Hg repo unknown. You must specify it with --repo')
        #
