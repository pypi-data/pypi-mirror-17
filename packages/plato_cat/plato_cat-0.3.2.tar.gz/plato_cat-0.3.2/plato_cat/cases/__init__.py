...
def wait_busy_resource(API, action, busy_payload, sleep, timeout=60*5):
    ...
    while True:
        ...
        r_ids = [r[id_key] for r in resources]
        print 'resources (%s) is still in busy status...' % r_ids

        sleep(4)

        if time.time() - start > timeout:
            instance_status = {r[id_key]: r['status'] for r in resources}
            raise Exception('wait_busy_resource timeout. '
                            'maybe job was not completely executed? '
                            'resources => status map:' + str(instance_status))
...
def wait_for_job(API, job_ids, sleep, timeout=60*4):
    """
    ...
    while True:
        ...
        busy_jobs = []
        for job in job_set:
            # if one is busy, go to next round
        ...
        job_ids = [j['jobId'] for j in busy_jobs]

        if time.time() - start > timeout:
            job_status = {j['jobId']: j['status'] for j in busy_jobs}
