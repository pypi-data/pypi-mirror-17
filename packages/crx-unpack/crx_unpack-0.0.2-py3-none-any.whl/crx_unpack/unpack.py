...
def unpack(crx_file, ext_dir=None, overwrite_if_exists=False, img_tallies=None, test_contents=True, passwd=None,
           skip_img_formats=None, unpack_in_subprocess=False, convert_in_subprocess=True):
    """
    ...
    crx_dir, base = path.split(path.abspath(crx_file))
    ...
    with open(crx_file, 'rb') as fin:
        header_vals = fin.read(4 * 4)
        if len(header_vals) < 16:
            raise BadCrxHeader('Invalid header length')
        magic, version, pup_key_len, sig_len = headers.unpack(header_vals)
        if magic != b'Cr24':
        ...
        # Detach zip file
        zip_path = base[::-1].replace('xrc.', 'piz.', 1)[::-1]
        zip_path = path.join(crx_dir, zip_path)
        with open(zip_path, 'wb') as fout:
            # Read the rest of the file and save it as a .zip
            fout.write(fin.read(crx_size))
    ...
    if False:  # convert_in_subprocess:
...
if __name__ == '__main__':
    ...
    if args['xo']:
        ...
        _ext_dir = unpack(args['CRX_FILE'], args['EXT_DIR'], args['-f'], test_contents=(not args['-t']),
