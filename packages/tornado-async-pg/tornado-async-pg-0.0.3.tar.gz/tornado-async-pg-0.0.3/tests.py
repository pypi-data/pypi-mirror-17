...
db_user = os.environ.get('MOMOKO_TEST_USER', 'zhouqi')
...
asyncpg.Pool.log_connect_errors = False
...
class BaseDataTest(BaseTest):
    ...
    @gen_test
    def set_up_10(self):
        self.conn = yield asyncpg.connect(self.dsn, ioloop=self.io_loop)
...
class MomokoConnectionTest(BaseTest):
    ...
    def test_connect(self):
        ...
        conn = yield asyncpg.connect(good_dsn, ioloop=self.io_loop)
    ...
    def test_bad_connect(self):
        ...
        try:
            conn = yield asyncpg.connect(bad_dsn, ioloop=self.io_loop)
    ...
    def test_bad_connect_local(self):
        ...
        try:
            conn = yield asyncpg.connect(local_bad_dsn, ioloop=self.io_loop)
        except Exception as error:
            self.assertIsInstance(error, psycopg2.OperationalError)
...
class MomokoConnectionSetsessionTest(BaseTest):
    ...
    def test_setsession(self):
        ...
        for i in range(len(time_zones)):
            ...
            conn = yield asyncpg.connect(self.dsn, ioloop=self.io_loop, setsession=setsession)
...
class MomokoConnectionFactoriesTest(BaseTest):
    ...
    def test_cursor_factory(self):
        ...
        conn = yield asyncpg.connect(self.dsn, ioloop=self.io_loop, cursor_factory=RealDictCursor)
    ...
    def test_connection_factory(self):
        ...
        conn = yield asyncpg.connect(self.dsn, ioloop=self.io_loop, connection_factory=RealDictConnection)
        cursor = yield conn.execute("SELECT 1 AS a")
        self.assertEqual(cursor.fetchone(), {"a": 1})
...
class MomokoPoolTest(PoolBaseTest):
    ...
    def test_connect(self):
        ...
        self.assertIsInstance(db, asyncpg.Pool)
...
class MomokoPoolStretchTest(MomokoPoolParallelTest):
    ...
    def test_stretch_after_disonnect(self):
        ...
        jobs = self.max_size - 1
        super(MomokoPoolStretchTest, self).run_parallel_queries(jobs)
        self.assertEqual(self.db.conns.total, jobs - 1)
...
class MomokoPoolVolatileDbTest(PoolBaseTest):
    ...
    def test_execute_can_start_before_connection_is_done(self):
        db = asyncpg.Pool(dsn=self.good_dsn, size=1, ioloop=self.io_loop)
        db.connect()
    ...
    def test_execute_before_connection_is_done_will_error(self):
        db = asyncpg.Pool(dsn=bad_dsn, size=1, ioloop=self.io_loop)
        db.connect()
...
class MomokoPoolVolatileDbTestProxy(ProxyMixIn, MomokoPoolVolatileDbTest):
    ...
    def test_execute_can_fail_after_disconnect_with_no_reconnect(self):
        ...
        try:
            yield f1
        except (psycopg2.DatabaseError, db.DatabaseNotAvailable):
            pass
...
class MomokoPoolPartiallyConnectedTest(PoolBaseTest):
    ...
    def test_partially_connected(self):
        ...
        exp = asyncpg.exceptions.PartiallyConnectedError
...
from asyncpg.pool import with_transaction


class TransactionTest(AsyncTestCase):
    def setUp(self):
    ...
    @with_transaction()
    @gen.coroutine
    def q(self, conn=None):
        cur = yield conn.execute('select 1')
