...
def startCameraGUI():
    ...
    f1 = ttk.Labelframe(toppane, text='ISO: (max=800)', width=100, height=100)
    f2 = ttk.Labelframe(toppane, text='Resolution: (max=2592 x 1944)', width=100, height=100)
    f3 = ttk.Labelframe(toppane, text='Delay: (seconds, not including time to setup camera)', width=100, height=100)
    toppane.add(f1)
    toppane.add(f2)
    toppane.add(f3)
    masterpane.add(toppane)
    ...
    ttk.Label(f1, text='x').grid(column=2, row=1)
    ...
    midpane = ttk.Panedwindow(masterpane, orient=HORIZONTAL)
    f4 = ttk.Labelframe(midpane, text='Exposure Modes: (Default = auto, Only select one)', width=100, height=100)
    midpane.add(f4)
    masterpane.add(midpane)

    exposuremode_night = ttk.Checkbutton(f4, text='night', variable=exposuremodenight)
    exposuremode_night.grid(column=1, row=1, sticky=W)

    exposuremode_backlight = ttk.Checkbutton(f4, text='backlight', variable=exposuremodebacklight)
    exposuremode_backlight.grid(column=2, row=1, sticky=W)

    exposuremode_verylong = ttk.Checkbutton(f4, text='verylong', variable=exposuremodeverylong)
    exposuremode_verylong.grid(column=3, row=1, sticky=W)

    exposuremode_spotlight = ttk.Checkbutton(f4, text='spotlight', variable=exposuremodespotlight)
    exposuremode_spotlight.grid(column=4, row=1, sticky=W)

    exposuremode_sports = ttk.Checkbutton(f4, text='sports', variable=exposuremodesports)
    exposuremode_sports.grid(column=5, row=1, sticky=W)

    exposuremode_off = ttk.Checkbutton(f4, text='off', variable=exposuremodeoff)
