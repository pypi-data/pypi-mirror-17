...
def startGUI():
    ...
    def picturetaken():
        try:
            ...
            intensity_array = ','.join(['R:'+'{:.3f}'.format(red_intensity)+'\n',
    ...
    ttk.Button(f5, text="Take Picture", command=picturetaken).grid(column=2, row=1, sticky=(E, W))
    ttk.Button(f5, text="Choose Directory", command=directorychosen).grid(column=2, row=4, sticky=E)
    ttk.Label(f5, text="Intensity: ").grid(column=1, row=1)
    intensitylabel = ttk.Label(f5, text='Not Taken')
    intensitylabel.grid(column=1, row=2, sticky=W)
    ttk.Button(f5, text="Show Plots", command=showimageplot).grid(column=2, row=2, sticky=(E, W))
