...
def startCameraGUI():
    ...
    def showimageplot():
        try:
            ...
            RPiCamera.showImage(r_array)
            RPiCamera.showImage(g_array)
            RPiCamera.showImage(b_array)
        except ValueError:
    ...
    ttk.Label(f3, text='Framerate: ').grid(column=6, row=1, sticky=E)
    delay_variable = ttk.Entry(f3, width=4, textvariable=frameratevar)
    delay_variable.grid(column=7, row=1, sticky=W)

    ttk.Label(f3, text='Shutter Speed: ').grid(column=8, row=1, sticky=E)
