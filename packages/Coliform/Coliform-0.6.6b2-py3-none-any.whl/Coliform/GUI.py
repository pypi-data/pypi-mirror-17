...
def startCameraGUI():
    ...
    def darkpicturetaken():
        try:
            ...
            if exposuremodeoff.get():
                exposure = 'off'
            if exposuremodenight.get():
                exposure = 'night'
            if exposuremodebacklight.get():
                exposure = 'backlight'
            if exposuremodesports.get():
                exposure = 'sports'
            if exposuremodespotlight.get():
                exposure = 'spotlight'
            if exposuremodeverylong.get():
                exposure = 'verylong'
            rgb_array = RPiCamera.takePicture(iso=iso, delay=delay, resolution=resolution, exposure=exposure)
            red_intensity, green_intensity, blue_intensity, intensity = RPiCamera.returnIntensity(rgb_array)
            intensity_array = '\n'.join(['R:'+'{:.3f}'.format(red_intensity),
                                         'G:'+'{:.3f}'.format(green_intensity),
                                         'B:'+'{:.3f}'.format(blue_intensity),
    ...
    exposuremodenight = IntVar()
    exposuremodeoff = IntVar()
    exposuremodesports = IntVar()
    exposuremodeverylong = IntVar()
    exposuremodebacklight = IntVar()
    exposuremodespotlight = IntVar()
    delayvar = IntVar()

    masterpane = ttk.Panedwindow(mainframe, orient=VERTICAL)

    toppane = ttk.Panedwindow(masterpane, orient=VERTICAL)
    f1 = ttk.Labelframe(toppane, text='Image Options:', width=100, height=100)
    f2 = ttk.Labelframe(toppane, text='Exposure Modes: (Default = auto, Only select one)', width=100, height=100)
    toppane.add(f1)
    toppane.add(f2)
    masterpane.add(toppane)

    ttk.Label(f1, text='ISO: (max=800)').grid(column=1, row=1, sticky=W)
    iso_variable = ttk.Entry(f1, width=4, textvariable=isovar)
    iso_variable.grid(column=1, row=2, sticky=W)

    ttk.Label(f1, text='Resolution: (max=2592 x 1944)').grid(column=2, row=1, sticky=E)
    xresolution_variable = ttk.Entry(f1, width=4, textvariable=resolutionvarx)
    xresolution_variable.grid(column=2, row=2, sticky=W)
    ttk.Label(f1, text='x').grid(column=3, row=2)
    yresolution_variable = ttk.Entry(f1, width=4, textvariable=resolutionvary)
    yresolution_variable.grid(column=4, row=2, sticky=W)

    exposuremode_night = ttk.Checkbutton(f2, text='night', variable=exposuremodenight)
    exposuremode_night.grid(column=1, row=1, sticky=W)

    exposuremode_backlight = ttk.Checkbutton(f2, text='backlight', variable=exposuremodebacklight)
    exposuremode_backlight.grid(column=2, row=1, sticky=W)

    exposuremode_verylong = ttk.Checkbutton(f2, text='verylong', variable=exposuremodeverylong)
    exposuremode_verylong.grid(column=3, row=1, sticky=W)

    exposuremode_spotlight = ttk.Checkbutton(f2, text='spotlight', variable=exposuremodespotlight)
    exposuremode_spotlight.grid(column=4, row=1, sticky=W)

    exposuremode_sports = ttk.Checkbutton(f1, text='sports', variable=exposuremodesports)
    exposuremode_sports.grid(column=5, row=1, sticky=W)

    exposuremode_off = ttk.Checkbutton(f2, text='off', variable=exposuremodeoff)
    exposuremode_off.grid(column=6, row=1, sticky=W)

    ttk.Label(f1, text='Capture Delay:').grid(column=2, row=3, sticky=W)
    delay_variable = ttk.Entry(f1, width=4, textvariable=delayvar)
    delay_variable.grid(column=2, row=4, sticky=W)

    ttk.Button(f1, text='Take Picture', command=darkpicturetaken).grid(column=4, row=4, sticky=W)
    ...
    ttk.Button(f5, text="Take Picture Default", command=picturetaken).grid(column=1, row=1, sticky=(W, E))
