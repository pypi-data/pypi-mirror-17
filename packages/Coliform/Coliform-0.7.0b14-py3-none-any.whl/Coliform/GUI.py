...
def startCameraGUI():
    ...
    def showimage():
        try:
            ...
            top = Toplevel(root)
            top.title('Image Capture')
            img = Image.open('/home/pi/PycharmProjects/Coliform/tests/true.png')
            image = img.resize((648, 486), Image.ANTIALIAS)
            trueimage = ImageTk.PhotoImage(image)
            Label(top, image=trueimage).pack()
    ...
    def showredimage():
        try:
            ...
            r_array = RPiCamera.setImageColor(rgb_array_red, 'r')
            misc.imsave('red.png', r_array)
            topred = Toplevel(root)
            topred.title('Red Capture')
            img = Image.open('/home/pi/PycharmProjects/Coliform/tests/red.png')
            image = img.resize((648, 486), Image.ANTIALIAS)
            redimage = ImageTk.PhotoImage(image)
            Label(topred, image=redimage).pack()
            Button(topred, text='Dismiss', command=topred.destroy).pack()
            redimage.show()
    ...
    def showgreenimage():
        try:
            ...
            g_array = RPiCamera.setImageColor(rgb_array_green, 'r')
            misc.imsave('green.png', g_array)
            topgreen = Toplevel(root)
            topgreen.title('Green Capture')
            img = Image.open('/home/pi/PycharmProjects/Coliform/tests/green.png')
            image = img.resize((648, 486), Image.ANTIALIAS)
            greenimage = ImageTk.PhotoImage(image)
            Label(topgreen, image=greenimage).pack()
            Button(topgreen, text='Dismiss', command=topgreen.destroy).pack()
            greenimage.show()
        except ValueError:
            messagebox.showinfo(message='File not found, make sure take picture before showing Image.')
    ...
    def showblueimage():
        try:
            ...
            b_array = RPiCamera.setImageColor(rgb_array_blue, 'r')
            misc.imsave('blue.png', b_array)
            topblue = Toplevel(root)
            topblue.title('Blue Capture')
            img = Image.open('/home/pi/PycharmProjects/Coliform/tests/blue.png')
            image = img.resize((648, 486), Image.ANTIALIAS)
            blueimage = ImageTk.PhotoImage(image)
            Label(topblue, image=blueimage).pack()
            Button(topblue, text='Dismiss', command=topblue.destroy).pack()
            blueimage.show()
        except ValueError:
            messagebox.showinfo(message='File not found, make sure take picture before showing Image.')
