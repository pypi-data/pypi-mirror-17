...
def showPlot(rgb_array):
    ...
    plt.plot(lu4, color='black', label='Intensity')
    plt.title("Colors by Location")
    plt.xlabel("Location")
    plt.ylabel("Intensity")
    plt.legend()

    lu5 = img_hsv[..., 2].flatten()
    plt.subplot2grid((2, 3), (0, 2))
    plt.hist(lu5, bins=256, range=(0, 256), histtype='stepfilled', color='black', label='Intesity')
    plt.title("Intensity")
    plt.xlabel("Value")
    plt.ylabel("Frequency")
    plt.legend()

    lu6 = rgb_array[..., 0].flatten()
    plt.subplot2grid((2, 3), (1, 0))
    plt.hist(lu6, bins=256, range=(0, 256), histtype='stepfilled', color='r', label='Red')
    plt.title("Red")
    plt.xlabel("Value")
    plt.ylabel("Frequency")
    plt.legend()

    lu7 = rgb_array[..., 1].flatten()
    plt.subplot2grid((2, 3), (1, 1))
    plt.hist(lu7, bins=256, range=(0, 256), histtype='stepfilled', color='g', label='Green')
    plt.title("Green")
    plt.xlabel("Value")
    plt.ylabel("Frequency")
    plt.legend()

    lu8 = rgb_array[..., 2].flatten()
    plt.subplot2grid((2, 3), (1, 2))
    plt.hist(lu8, bins=256, range=(0, 256), histtype='stepfilled', color='b', label='Blue')
    plt.title("Blue")
    plt.xlabel("Value")
    plt.ylabel("Frequency")
    plt.legend()
