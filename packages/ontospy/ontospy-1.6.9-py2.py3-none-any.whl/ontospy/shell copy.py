...
class Shell(cmd.Cmd):
	...
	def _printStats(self, g, entity=None, verbose=True):
		...
		if not entity:  # the graph
			...
			self._print("Domain of....: %d" % len(x.domain_of))
			self._print("Range of.....: %d" % len(x.range_of))
			self._print("Instances....: %d" % x.count())
			self._print("----------------")
			if verbose:
				if x.parents():
					self._print("Parents......:\n%s\n----------------" % ", ".join([p.qname for p in x.parents()]))
				if x.children():
					self._print("Children.....:\n%s\n----------------" % ", ".join([p.qname for p in x.children()]))
				if x.ancestors():
					self._print("Ancestors....:\n%s\n----------------" % ", ".join([p.qname for p in x.ancestors()]))
				if x.descendants():
					self._print("Descendants..:\n%s\n----------------" % ", ".join([p.qname for p in x.descendants()]))
				if x.domain_of:
					self._print("Domain of....:\n%s\n----------------" % ", ".join([p.qname for p in x.domain_of]))
				if x.range_of:
					self._print("Range of.....:\n%s\n----------------" % ", ".join([p.qname for p in x.range_of]))
		...
		if (not line) or (line[0] not in opts):
			...
			if self.currentEntity:
				...
				for obj in self.current['graph'].ontologies:
					self._print("==> Ontology: <%s>" % str(obj.uri), "TIP")
					label = obj.bestLabel() or NOTFOUND
					description = obj.bestDescription() or NOTFOUND
					self._print("Title: " + label, "TEXT")
					self._print("Description: " + description, "TEXT")
