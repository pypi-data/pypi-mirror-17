...
def addvar(name=None, *, var_count=[0], lowBound=0, **kwargs):
    """変数作成用ユーティリティ"""
    from pulp import LpVariable
    if not name:
        var_count[0] += 1
        name = 'v%d' % var_count[0]
...
def logistics_network(tbdi, tbde, tbfa):
    """
    ロジスティクスネットワーク問題を解く
    tbdi: 需要地 工場 輸送費
    tbde: 需要地 製品 需要
    tbfa: 工場 レーン 製品 下限 上限 生産費
    出力: 解の有無, 輸送表, 生産表
    """
    import pandas as pd
    from pulp import LpProblem, lpDot, lpSum, value
    m = LpProblem()
    tbpr = tbfa[['工場', '製品']].sort_values(['工場', '製品']).drop_duplicates()
    tbdi2 = pd.merge(tbdi, tbpr, on='工場')
    tbdi2['VarX'] = addvars(tbdi2.shape[0])
    tbfa['VarY'] = addvars(tbfa.shape[0])
    tbsm = pd.concat([tbdi2.groupby(('工場', '製品')).VarX.sum(),
                      tbfa.groupby(('工場', '製品')).VarY.sum()], 1)
    tbde2 = pd.merge(tbde, tbdi2.groupby(('需要地', '製品')).VarX.sum().reset_index())
    m += lpDot(tbdi2.輸送費, tbdi2.VarX) + lpDot(tbfa.生産費, tbfa.VarY)
    for _, r in tbsm.iterrows():
        m += r.VarX <= r.VarY
    for _, r in tbde2.iterrows():
        m += r.VarX >= r.需要
    for _, r in tbfa[tbfa.下限 > 0].iterrows():
        r.VarY.lowBound = r.下限
    for _, r in tbfa[tbfa.上限 != np.inf].iterrows():
        r.VarY.upBound = r.上限
    m.solve()
    if m.status == 1:
