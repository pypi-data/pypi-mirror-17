    ...
    def add(self, subj, predicate, obj):
        ...
        if not isinstance(obj, self.uri):
            if isinstance(obj, unicode_with_language):
                obj = self._literal(unicode(obj), lang=obj.lang)
            else:
    ...
    def _literal(self, value, lang=None):
        ...
        if lang is None:
            try:
                ...
                return self._node(literal=unicode(value))
            else:
                datatype = self.uri(datatype)
                return self._node(literal=unicode(value), datatype=datatype)
    ...
    def triples(self):
        ...
        for subj, pred, obj in self._graph.triples((None, None, None)):
            if isinstance(obj, self.uri):
                ...
                obj = obj.toPython()
            yield unicode(subj), unicode(pred), obj
...
class RDFGraphGenerator(object):
    ...
    def same_as_uris(entity):
        ...
        if urlparse(entity.absolute_url()).netloc != urlparse(entity.cwuri).netloc:
            yield entity.cwuri
    ...
    def add_entity(self, entity, reg):
        ...
        if etype not in reg.etype2rdf:
            return ()
        uri = graph.uri(entity.absolute_url())
    ...
    def _add_relations(self, uri, predicate_uri, reverse, related_entities):
        ...
        for related_entity in related_entities:
            r_uri = graph.uri(related_entity.absolute_url())
            if reverse:
