...
class GuessRDFFormatTC(TestCase):
    ...
    def test_base(self):
        self.assertEqual(guess_rdf_format('file.xml'), 'xml')
        self.assertEqual(guess_rdf_format('file.rdf'), 'xml')
        self.assertEqual(guess_rdf_format('file.n3'), 'n3')

    def test_url(self):
        self.assertEqual(guess_rdf_format('http://url.com/rdf/file.xml'), 'xml')
        self.assertEqual(guess_rdf_format('http://url.com/rdf/file.rdf'), 'xml')
        self.assertEqual(guess_rdf_format('http://url.com/rdf/file.n3'), 'n3')

    def test_error(self):
        self.assertRaises(ValueError, guess_rdf_format, 'file.unknown')
        self.assertRaises(ValueError, guess_rdf_format, 'http://url.com/rdf/file')
        self.assertRaises(ValueError, guess_rdf_format, 'http://url.com/file/rdf')
...
class RDFRegistryTC(TestCase):
    ...
    def test_register_prefix_unknown(self):
        xy = self.xy
        with self.assertRaises(RDFRegistryError) as cm:
...
class AddEntitiesToRDFGraphTC(testlib.CubicWebTC):
    ...
    def _test(self, graph):
        ...
        with self.admin_access.client_cnx() as cnx:
            ...
            c1_url = c1.absolute_url()
            c2_url = c2.absolute_url()
            l1_url = l1.absolute_url()
            l2_url = l2.absolute_url()
            cnx.commit()
        with self.admin_access.client_cnx() as cnx:
            scheme = cnx.entity_from_eid(scheme.eid)
            scheme_url = scheme.absolute_url()
            stack = entities_stack([scheme])
            graph_generator = RDFGraphGenerator(graph)
            related = None
            try:
                while True:
                ...
                pass
            expected = [
                (scheme_url, TYPE_PREDICATE_URI, skos('ConceptScheme')),
                ...
                (c1_url, TYPE_PREDICATE_URI, skos('Concept')),
                (c1_url, skos('inScheme'), scheme_url),
                (c2_url, TYPE_PREDICATE_URI, skos('Concept')),
