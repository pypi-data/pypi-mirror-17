...
class ReferenceReturnObject(ReturnObject):
    ...
    def _merge(self, other):
        ...
        result = {}
        result['output_linecount'] = {'reference': 0}
        result_lc = 0
        for key in other_dict.keys():
            ...
            if isinstance(other_dict[key], list):
                result['output_linecount']['reference'] += len(self_dict[key])
                ...
                if self_dict[key] != other_dict[key]:
                    result[key]['result'] = other_dict[key]
        if result_lc != result['output_linecount']['reference']:
            result['output_linecount']['result'] = result_lc
        return result
