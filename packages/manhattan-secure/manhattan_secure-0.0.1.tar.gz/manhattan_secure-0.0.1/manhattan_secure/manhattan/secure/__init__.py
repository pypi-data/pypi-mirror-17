...
def random_code(length=8, charset='abcdefghkmnopqrstuvw23456789'):
    ...
    no guarentee the output code will be good/safe password choice.
    """
...
def random_token():
    ...
    m.update(str(uuid.uuid4()).encode('utf8'))
    return m.hexdigest()
...
def safe_redirect_url(url, ignore_paths=None):
    ...
    URLs are converted to a local path (e.g http://www.example.com/foo becomes
    /foo).
