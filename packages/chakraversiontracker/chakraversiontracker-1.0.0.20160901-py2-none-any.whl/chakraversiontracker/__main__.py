        ...
        for repository in repositories:
            ...
            if repository == "ccr":
                ...
                while index < total_ccr_packages:
                    response = requests.get(u"https://chakralinux.org/ccr/packages.php?O={}&PP=100".format(index))
                    soup = BeautifulSoup(response.text, "lxml")
                ...
                list_packages_process = subprocess.Popen(
                    [u"pacman", "-Sl", repository],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                )
                output, errors = list_packages_process.communicate()
                for line in output.splitlines():
                    parts = line.split(" ")
                    match = _version_re.match(parts[2])
                    _repository_packages[repository][parts[1]] = \
                        match.group("pkgver")
...
def main():
    ...
    default_template_path = os.path.join(
        os.path.dirname(__file__), 'templates', 'packages.cli')
    ...
    parser.add_argument(
        ...
        help=u"Path of a Jinja2 template file to generate the output. It can "
             u"be one of the built-in templates or a custom template.")
    parser.add_argument(
    ...
    parser.add_argument('--noccr', action='store_true',
    ...
    packages = sorted(outdated_package_data, key=lambda k: k['name'])
