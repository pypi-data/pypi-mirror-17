...
class TakeAway:
    def __init__(self, country, postcode, parser, args):
        ...
        self.command(parser, args)
        self.interface.run(parser)
    ...
    def command(self, parser, args=None, user_input=None):
        if args:
        ...
        if args.new_postcode:
            if len(args.new_postcode) == 2:
                ...
                try:
                    TakeAway(country=args.new_postcode[0], postcode=args.new_postcode[1], parser=parser,
        ...
        if args.db_list:
            ...
            try:
                self.interface.db_delete(self.database.delete_postcode(args.db_del[0], args.db_del[1]), args.db_del[0],
                                         args.db_del[1])
