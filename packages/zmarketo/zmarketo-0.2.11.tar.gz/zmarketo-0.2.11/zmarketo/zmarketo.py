			...
			if resp.status_code != 200:
				...
				for error in errors:
					...
					if retry_count < 5 and error.get('code') in ['601', '602', '604', '606']:
						...
						elif error.get('code') == '606':
							# back off between 1 - 5 seconds
							time.sleep(1 + 4 * random.random())
						retry_count += 1
						handle_response(retry_count=retry_count)
...
class MarketoRest(object):
	...
	def get_lead_activities(self, activity_type_ids, next_page_token, batch_size=300, list_id=None):
