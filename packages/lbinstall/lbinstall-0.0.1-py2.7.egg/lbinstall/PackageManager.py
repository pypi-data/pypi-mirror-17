...
def flagToString(flags):
    ...
    if flags == None or (type(flags) == str and len(flags) == 0):
        return None
...
class PackageManager(object):
    '''
    Class allowing to install LHCb software
    '''

    def __init__(self, filename, relocatemap = None):
        '''
    ...
    def _getProvides(self):
        ...
        # amking sure it's teh case before calling zip
    ...
    def _getRequires(self):
        l = [self._headers["requirename"],
             ...
             map(flagToString, self._headers["requireflags"]) ]
        return zip(*l)
    ...
    def getFullName(self):
        return "%s-%s-%s" % (self.getName(), self.getVersion(), self.getRelease())
    ...
    def getTopDir(self):
        if self._topdir == None:
            with rpmfile.open(self._filename) as rpm:
                self._topdir = os.path.commonprefix([ m.name for m in rpm.getmembers()])
        return self._topdir

    def extract(self, prefixmap = None, overwrite=False):
        ''' Extract the files using the reolcation specified in the prefix maps'''
        if prefixmap == None:
        ...
        with rpmfile.open(self._filename) as rpm:
            for m in rpm.getmembers():                                        
                ...
                if attrs["isDir"]:
                    ...
                    self.log.debug("We have a link %s to: %s" % (target, lnktgt))
                    ...
                    if not os.path.exists(os.path.dirname(target)):
                        if not os.path.exist(target):
                            os.makedirs(os.path.dirname(target))
                    self.log.debug("Relocating %s to %s" % (m.name, target))
                    fin = rpm.extractfile(m.name)
    ...
    def relocate(self, path):
        ''' Useful util if the PM is initialized with the proper relocation map '''
    ...
    def checkFileSizesOnDisk(self):
        ...
        for (fname, isdir, md5, s) in mdata:
            ...
            if s != statres[stat.ST_SIZE]:
                raise Exception("%s: Mismatch in filesize vs metadata: %s vs %s" % (name, statres[stat.ST_SIZE], s  ))
            self.log.debug("File %s metadata:%s real:%s" % (name, s,  statres[stat.ST_SIZE]))
    ...
    def removeFiles(self):
        ...
        # First removing the files
        for (fname, isdir, md5, s) in mdata[::-1]:
            if not isdir:
                continue
            name = self.relocate(fname)            
            try:
    ...
    def getFileMetadata(self):
        ...
        # pprint(self._headers)        
        tmp = zip(*self.getFileNames())
        tmp.append(self._headers['filemd5s'])
        #tmp.append(self._headers['filesizes'])
    ...
    def getRelocatedFileMetadata(self):
        ...
        i.e. a tuple with (filename, bool(isdir), filemd5, filesize '''
        tmp = []
        for (name, type, md5, size) in self.getFileMetadata():
            tmp.append( (self.relocate(name), type, md5, size))
        return tmp
