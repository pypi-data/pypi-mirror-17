...
class LbInstallClient(object):
    ...
    MODE_UPDATE  = "update"
    MODE_CHECK   = "check"
    MODES = [    MODE_INSTALL, MODE_QUERY, MODE_LIST ]
    ...
    def main(self):
        ...
        try:
            ...
            if opts.siteroot != None:
                ...
                self.config = InstallAreaConfig(siteroot=opts.siteroot,
                                                configType=self.configType)
            else:
                envroot = os.environ.get('MYSITEROOT', None)
                if envroot == None:
                    raise LbInstallException("Please specify MYSITEROOT in the environment or use the --root option" )
                else:
                    self.config = InstallAreaConfig(configType=self.configType)

            # Setting the repourl if overriden
    ...
    def run(self, opts, args):
        ...
        if mode == LbInstallClient.MODE_LIST:
            # Mode that list packages according to a regexp
    ...
    ########################################################################################
    def queryPackages(self, nameregexp=None, versionregexp=None, releaseregexp=None):
        ''' Implement the listing of packages '''
        for l in sorted(list(self.installer.remoteListPackages(nameregexp, versionregexp, releaseregexp))):
            print l.rpmName()

    def install(self, rpmname, version=None, release=None):
        packagelist = self.installer.remoteFindPackage(rpmname, version, release)
        if self.dryrun:
            self.log.warning("Dry run mode, install list:" + ",".join([p.rpmName() for p in packagelist]))
        else:
            self.installer.install(packagelist, justdb=self.justdb, overwrite=self.overwrite)

    def listInstalledPackages(self, nameregexp=None, versionregexp=None, releaseregexp=None):
        ''' Implement the listing of packages '''
        for n,v,r  in sorted(list(self.installer.localListPackages(nameregexp, versionregexp, releaseregexp))):
            print "%s\t%s\t%s" % (n,v,r)
