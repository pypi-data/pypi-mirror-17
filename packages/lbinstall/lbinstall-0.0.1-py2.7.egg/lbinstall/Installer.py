...
from lbinstall.InstallAreaManager import InstallAreaConfig
...
class Installer(object):
    ...
    * Passing a full InstallAreaConfig as the named parameter config,
    ...
    def __init__(self, siteroot=None, config = None, ):
        '''
        ...
        if config is not None:
            ...
            self._config = InstallAreaConfig(siteroot=siteroot)
        siteroot = self._config.siteroot
        self._siterootBase = os.path.abspath(siteroot)
        self._siteroot = self._siterootBase
        self._lcgDir = os.path.join(self._siterootBase, "lcg")
        ...
        self.__installArea = InstallArea(self._config)
        self._lbYumClient = self.__installArea.createYumClient()
    ...
    def remoteListPackages(self, nameRegexp, versionRegexp=None, releaseRegexp=None):
        packages = self._lbYumClient.listPackages(nameRegexp, versionRegexp, releaseRegexp)
        return packages

    # Queries to the local DB
    # ############################################################################3
    def localListPackages(self, nameRegexp, versionRegexp=None, releaseRegexp=None):
        packages = self._localDB.listPackages(nameRegexp, versionRegexp, releaseRegexp)
        return packages


    # Installation routines
    # ############################################################################3
    def install(self, packagelist, justdb=False, overwrite=False, nodeps=False):
        '''
        ...
        # Deduplicating in case some package were there twice but keep the order...
        # There might be two as we take list of packages, and getPackagesToInstall is called multiple
        # times...
        ...
        filestoinstall = self._downloadfiles(rpmtoinstalldeduprev)

        # And installing...
        # We should deal with the order to avoid errors in case of problems
        # Half waythrough XXX
        filesinstalled = []
        # Taking reverse order to start with those that haven't been installed yet
        for rpm in filestoinstall[::-1]:
            self._installpackage(rpm, justdb=justdb, overwrite=overwrite)
    ...
    def _getPackagesToInstall(self, p, extrapackages = None, extrainfo = None):
        '''
        Proper single package installation method
        '''
        # Setting up data if needed
        if extrapackages == None:
            extrapackages = set()
        ...
        extrapackages.add(p)
        toinstall = [ p ]
        # Iterating though the reuired packages, first checking what's already on the
        # local filesystem...
        for req in p.requires:
            # First checking that one of the packages already schedules for install
            # do not already provide the package...
            tmp = self._findInExtrapackages(req, extrapackages)
            if tmp != None:
                self.log.debug("%s already fulfilled by %s scheduled to be installed" \
                                 % (str(req), tmp.name))

            elif self._localDB.provides(req):
                # Now checking whether the package isn't already installed...
                self.log.warning("%s already available on local system" % str(req))
                extrainfo["alreadyInstalled"].append(req)
            else:
                # Ok lets find in from YUM now...
                match = self._lbYumClient.findLatestMatchingRequire(req)
                if match != None and match not in extrapackages:
                    toinstall += self._getPackagesToInstall(match, extrapackages, extrainfo)
        return toinstall

    def _installpackage(self, filename, removeAfterInstall=True, justdb=False, overwrite=False):
        ''' To install a RPM and record the info in the local DB'''
        # The PackageManager responsible for dealing with the local package file
        ...
        try:
            ...
            if not justdb:
                ...
                self.log.warning("--just-db mode, will not install files from %s" % filename)
            # Update the local DB
        ...
        if not justdb:
            ...
            self.log.warning("--just-db mode, will not attempt to run post-install for %s" % filename)

        # Now cleanup if install was successfull
        if removeAfterInstall:
            self.log.debug("Install of %s succesful, removing downloaded RPM" % filename)
            os.unlink(filename)
    ...
    def _runPostInstall(self, packageManager, dbPackage):
        ...
        import tempfile
        import subprocess
        self.log.info("Running post-install scripts for %s" % packageManager.getFullName())
        with tempfile.NamedTemporaryFile(prefix="lbpostinst", delete=False) as piscript:
            piscript.write(piscriptcontent)
            piscript.flush()
            rc = subprocess.check_call(["/bin/sh", piscript.name], stdout=sys.stdout, stderr=sys.stderr, env=newenv)
            if rc == 0:
