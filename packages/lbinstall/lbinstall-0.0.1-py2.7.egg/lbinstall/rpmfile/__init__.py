...
class RPMInfo(object):
    ...
    def __init__(self, name, file_start, file_size, initial_offset, isdir, attrs):
        self.name = name
    ...
    def _read_new(cls, fileobj, magic=None):
        ...
        attrs["ino"] = d[0]
        attrs["mode"] = mode & 0000777
        nlink = int(d[4], 16)
        isdir = nlink >= 2 and file_size == 0
        #print("CHRIS name %s size %s nlink %s isdir %s"%(name, file_size, nlink, isdir))
        #print(d)
...
class RPMFile(object):
    ...
    def gzip_file(self):
        ...
        if self._gzip_file is None:
            ...
            #print("CHRIS _fileobj %s data_offset %s"%(self._fileobj, self.data_offset))
