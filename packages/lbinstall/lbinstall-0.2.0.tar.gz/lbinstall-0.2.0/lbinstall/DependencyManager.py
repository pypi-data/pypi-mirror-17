...
from lbinstall.Model import Package, Provides, Requires, VersionedObject
...
class Repository(object):
    ...
    def setupBackendFromRemote(self):
        ...
        for ba in self.availableBackends:
            log.info("REMOTE Checking availability of interface: %s" % ba.__name__)
            try:
                try:
                    (checksum, timestamp, filename) = remotemd[ba.yumDataType()]  # @UnusedVariable IGORE:W0612
                except KeyError:
                ...
                if not self.backend.hasDB() or ltimestamp == None or timestamp > ltimestamp:
                    ...
                    with open(self.localRepomdXml, 'w') as ftmp:
                        ftmp.write(data)
    ...
    def setupBackendFromLocal(self):
        ...
        backend = None
        for ba in self.availableBackends:
            log.info("LOCAL Checking availability of interface: %s" % ba.__name__)
            try:
                try:
...
class RepositoryXMLBackend(object):
    ...
    def getAllProvides(self, nameMatch=None):
        ...
        for prov_list_k in self.mProvides.keys():
            if nameMatch == None or (nameMatch != None and re.match(nameMatch, prov_list_k) != None):
...
class RepositorySQLiteBackend(object):
    ...
    def getAllProvides(self, nameMatch=None):
        ...
        for (name, flags, epoch, version, release, pkgKey) in res:
            ...
            if nameMatch == None or (nameMatch != None and re.match(nameMatch, name) != None):
...
class LbYumClient(object):
    ...
    def listProvides(self, nameRegexp=None, versionRegexp=None, releaseRegexp=None):
