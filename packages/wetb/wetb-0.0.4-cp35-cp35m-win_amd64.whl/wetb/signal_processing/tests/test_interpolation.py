...
class TestInterpolation(unittest.TestCase):
    ...
    def test_interpolate1(self):
        ...
        xp = [0.5, 1.5, 3]
        yp = [5, 15, 30]
        np.testing.assert_array_equal(signal_processing.interpolate(x, xp, yp), [10., 15., 20, 30.])
        np.testing.assert_array_equal(signal_processing.interpolate(x, xp, yp, 1), [10., 15., np.nan, 30.])
    ...
    def test_interpolate_max_dydxp(self):
        ...
        xp = [0, 2, 4, 6]
        yp = [358, 359, 0, 1]
        y = signal_processing.interpolate(x, xp, yp, max_dydxp=30)
        np.testing.assert_array_equal(y, [ 358., 358.5, 359., np.nan, 0., 0.5, 1. ])

        y = signal_processing.interpolate(x, xp, yp, max_dydxp=180)
    ...
    def test_interpolate_max_dydxp_cyclic(self):
        ...
        y = signal_processing.interpolate(x, xp, [178, 179, -180, -179], max_dydxp=30, cyclic_range=360)
        np.testing.assert_array_equal(y, [ 178. , 178.5, 179. , -0.5, -180. , -179.5, -179. ])

        y = signal_processing.interpolate(x, xp, yp, max_dydxp=30, cyclic_range=360)
        np.testing.assert_array_equal(y, [ 358., 358.5, 359., 359.5, 0., 0.5, 1. ])

        y = signal_processing.interpolate(xp, x, [ 358., 358.5, 359., 359.5, 0., 0.5, 1. ], max_dydxp=30, cyclic_range=360)
    ...
    def test_interpolate_max_repeated(self):
        ...
        xp = [0, 1, 2, 3, 4, 5, 6]
        yp = [4, 5, 5, 5, 6, 6, 7]
        y = signal_processing.interpolate(x, xp, yp, max_repeated=2)
        ...
        xp = [0, 2, 3, 4, 6]
        y = signal_processing.interpolate(x, xp, yp, max_repeated=1)
