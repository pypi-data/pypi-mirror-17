...
class Clizer(object):
    ...
    def override_options_from_environ(self, options):
    ...
    def override_options_from_file(self, options):
        """
        ...
        optionss_file to effective_path/effective_file.
        """
        options_file = options.get('options_file') or self.variables.get('OPTIONS_FILE')
        if not options_file:
            self.variables['optionss_file'] = None
            return
        options_path = self.variables.get('OPTIONS_PATH')
        options_dict, options_path_file = None, None
        try:
            if options_path or os.path.isabs(options_file):
                options_path_file, options_dict = read_configuration(options_file, options_path)
            else:
                for path in (os.getcwd(), os.path.expanduser('~/.clingon'), '/etc/clingon/'):
                    try:
                        options_path_file, options_dict = read_configuration(options_file, path)
                        break
                    except RuntimeError as e:
                        error = e
        except (RuntimeError, TypeError) as e:
            self._write_error(str(e))
        self.variables['optionss_file'] = options_path_file
        if options_dict:
            for k in list(options):
    ...
    def start(self, param_string=None):
        ...
        optargs = {}
        reqargs, varargs = [], []
        # override options from environ
        self.override_options_from_environ(optargs)
        # override options from configuration file
        self.override_options_from_file(optargs)
        print(optargs)
