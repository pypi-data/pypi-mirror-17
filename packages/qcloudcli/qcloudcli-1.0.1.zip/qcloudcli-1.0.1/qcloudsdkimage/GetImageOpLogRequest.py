#!/usr/bin/python
# -*- coding: utf-8 -*-
from qcloudsdkcore.request import Request
class GetImageOpLogRequest(Request):

	def __init__(self):
		Request.__init__(self, 'image', 'qcloudcliV1', 'GetImageOpLog', 'image.api.qcloud.com')

	def get_imgIdList(self):
		return self.get_params().get('imgIdList')

	def set_imgIdList(self, imgIdList):
		self.add_param('imgIdList', imgIdList)

	def get_offset(self):
		return self.get_params().get('offset')

	def set_offset(self, offset):
		self.add_param('offset', offset)

	def get_len(self):
		return self.get_params().get('len')

	def set_len(self, len):
		self.add_param('len', len)

	def get_startTime(self):
		return self.get_params().get('startTime')

	def set_startTime(self, startTime):
		self.add_param('startTime', startTime)

	def get_endTime(self):
		return self.get_params().get('endTime')

	def set_endTime(self, endTime):
		self.add_param('endTime', endTime)

