...
class Completer(object):
    ...
    def _complete_provider(self):
        ...
        if self.current_word.startswith('-'):
            ...
            l = ['--' + n for n in self.main_options
                 if n.startswith(cw)]
            retval = l
        elif self.current_word == './testcli' or self.current_word == self.qcloudcli:
    ...
    def _find_possible_options(self):
        ...
        if cmdInstance is None:
            ...
            old_arg_list = self.handleData.getAttrList(mclassname)
        new_arg_list = set()
    ...
    def _help_to_show_instance_attribute(self, classname):
        ...
        old_arg_list = self.handleData.getAttrList(classname)
        new_arg_list = set()
        if not old_arg_list is None:
            for item in old_arg_list:
                if not item.startswith('_'):
                ...
                if not cmd_obj is None:
                    for w in self.non_options:
                        if w in cmd_obj:
                            self.operation = w
                            break
                cmd_extension_obj = self.handleData.getExtensionOperationsFromCmd(self.command_name)
