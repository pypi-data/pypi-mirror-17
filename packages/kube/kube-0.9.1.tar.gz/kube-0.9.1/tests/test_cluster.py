...
class TestCluster:
    ...
    def test_init_url(self):
        c = _cluster.Cluster(url='http://localhost:8001/api')
        assert c.proxy._base_url == 'http://localhost:8001/api/v1/'
    ...
    def test_create(self, request, cluster):
        ...
        def fin():
            cluster.proxy.delete('namespaces', item.meta.name)
    ...
    def test_create_with_ns(self, request, cluster):
        ...
        def fin():
            cluster.proxy.delete('namespaces', 'default',
                                 'secrets', item.meta.name)
        request.addfinalizer(fin)
        assert isinstance(item, _base.ItemABC)
...
class TestAPIServerProxy:
    ...
    def test_init_url(self):
        ...
        p1 = _cluster.APIServerProxy(base_url='http://localhost:8001/api/v1')
        assert p1._base_url.endswith('/')
        assert p1._base_url == p0._base_url
    ...
    def test_post_201(self, request, proxy):
        ...
        ret = proxy.post('namespaces', json=data)
        name = ret['metadata']['name']
        def fin():
            proxy.delete('namespaces', name)
        request.addfinalizer(fin)
        assert name.startswith('testns-')

    def test_post_404(self, proxy):
        with pytest.raises(kube.APIError):
            proxy.post('this-resource-does-not-exist')
    ...
    def test_watch(self, proxy, monkeypatch):
        ...
        version = object()
        fields = object()
        ret = proxy.watch('namespaces', 'default',
                          version=version, fields=fields)
        assert ret == (('namespaces', 'default'), version, fields)
