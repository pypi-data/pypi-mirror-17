...
class CWTask(task.Task):
    ...
    def cw_user_cnx(self, user_eid):
        """context manager that provide a a user connection

        Yield a connection for `user_eid` or an internal connection if given
        `user_eid` is None or -1
        """
        if not self.need_cnx:
            raise ValueError("use of cw_user_cnx() requires need_cnx")
        cnx = self.cw_cnx
        if user_eid in (None, -1):
            yield cnx
        else:
            user = cnx.entity_from_eid(user_eid)
            with Session(user, cnx.repo).new_cnx() as user_cnx:
                yield user_cnx
