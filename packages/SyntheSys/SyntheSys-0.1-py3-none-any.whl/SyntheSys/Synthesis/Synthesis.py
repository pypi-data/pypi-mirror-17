...
from Utilities.Timer import Timer, ProcessTimer, TimeStamp
...
#	InputDataNumber=len(Kwargs['Tab'])
	...
	for P in (AppPath, AppSrcPath, AppSynthesisPath, AppSimuPath):
...
#		Module.HighLevelModule._TargetHW=TargetHW#Targets.pop(0)
	...
	#		pickle.dump(FD, open(os.path.join(AppPath, SystemMod.AlgoName+'.soc'), "wb+"))
	#	else:
			#------------------------
...
#	sys.exit(1)
	...
	AppStimValues, ComAddress, CommunicationService=GenerateConfData(APP, SystemMod, NoCMapping, Sched, OutputPath=AppPath)
...
#		logging.info("InputDataNumber={0}".format(InputDataNumber))
	...
	#------------------------
		...
		if "NoC" in Operation.LibServices:
			AdOCNet=Operation.LibServices["NoC"].GetModule()
		else: 
...
#====================================================================	
def GenerateConfData(APP, SystemMod, NoCMapping, Sched, OutputPath):
	"""
...
#	MemoryReadStimValues, MemoryReadValues=ComService.GenInputStim(TaskValues=[ComHeader,], DataID=DATA_ID2, X=X, Y=Y, PayloadReceived=1) 
	...
	#----------------
	GenerateBinary(Values, os.path.join(OutputPath, "DataValues.bin"), ("DataIn", 8))
	GenerateBinary(ConstantValues, os.path.join(OutputPath, "ConstValues.bin"), ("DataIn", 8))
	GenerateBinary(ReduceValues, os.path.join(OutputPath, "ReduceValues.bin"), ("DataIn", 8))
	GenerateBinary(DataPathValues, os.path.join(OutputPath, "DataPath.bin"), ("DataIn", 8))
	GenerateBinary(CtrlPathValues, os.path.join(OutputPath, "CtrlPath.bin"), ("DataIn", 8))
	
#	GenerateBinary(MemoryFillValues, os.path.join(OutputPath, "MemoryFill.bin"), ("DataIn", 8))
#	GenerateBinary(MemoryReadValues, os.path.join(OutputPath, "MemoryRead.bin"), ("DataIn", 8))
