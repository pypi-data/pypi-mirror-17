...
#from SyntheSys.YANGO import Drawing
...
#				print("NoCParam.Default:", NoCParam.Default)
			...
			FlitWidth=NoCMapping.SetupFlitWidth(ModuleList=ModList)
			SubstituteClk, SubstituteRst=None, None
			for IPChild in NoCChild.ChildLists["IP"]:
				...
				IPMod, IPServ, ServNoCMapping, ServMapping = IPChild.Generate(NameList, NoCInterfaces, Position, TargetHeader, FlitWidth, CommunicationService, OutputPath=OutputPath)
...
#						print("IPMod.ProvidedServMap", IPMod.ProvidedServMap)
...
#					print("{0}'s Mapping:".format(IPServList[i].Name), '\n'.join(["{0}:{1}".format(i,j) for i,j in Mapping.items()]))
#					print("Substitute clock by", SubstituteClk)
			if SubstituteRst:
				for i, Mapping in enumerate(MappingList):
					if IPServList[i].Name==CommunicationService.Name: 
...
#					print("Substitute reset by", SubstituteRst)
					
#			print("SubstituteClk", SubstituteClk)
#			print("SubstituteRst", SubstituteRst)
#			input()
...
#			AppM.MoveInMatrix(ParmTuple, x, y)
	...
	def AddChild(self, Type, Name=None, Model=None, FromLib=False, ExtraParameters={}):
		...
		if Type == "Algorithm": 
			...
			Child = NoC(ChildName, Parent=self)
			Serv=self.Library.Service("NoC", ServiceAlias=ChildName)
			if Serv is None: 
				logging.error("No NoC service found in library.")
				return None
			Mod=Serv.GetModule()
			if Mod is None: logging.error("No module '{0}' found in library.".format(Name))
			if FromLib: 
				if not Child.SetFromLib(NoCName=Name, NoCModule=Mod): return None
		elif Type == "IP":   
...
#			logging.info("Draw Application Characterization Graph (APCG) for application '{0}': succeeded.".format(self))	
		...
		return True
#	#----------------------------------------------------------------------
	def Binding(self, Algo=None, FpgaDesign=None):
		"""
		...
		NewNoC=self.AddChild("NoC", Name="AdOCNet", FromLib=True)
		if NewNoC is None: 
			logging.error("Unable to add NoC child to application.")
			raise Misc.SyntheSysError("NoC name AdOCNet not found in library.")
...
#		logging.debug("Set '{0}' to value '{1}' in library.".format(self.Type, Mod))
	...
	def Generate(self, NameList, NoCInterfaces, Position, Header, FlitWidth, CommunicationService, OutputPath="./"):
		"""
		Generate synthesized netlist for this IP on a specified FPGA.
		"""
		...
		# Connect IP with NoC --------------------------------------
		IPNetworkInterfaces = IPServ.GetInterfaces(self.Parameters["Common"]["Interface"])
...
#		input()
		return IPModule, IPServ, NoCMap, ServMapping
	#----------------------------------------------------------------------
...
#			self.AddChild("Library", LibName) # list of library objects
		...
		self.Parameters["Common"]["Service"]     = Mod.ProvidedServ["NoC"] # TODO: select good service
		self.Parameters["Common"]["Service"].Alias=self.Parameters["Common"]["Name"]
