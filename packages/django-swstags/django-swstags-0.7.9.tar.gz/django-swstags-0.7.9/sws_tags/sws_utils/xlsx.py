...
def ExportFile(qs,param_export_file,request=None):
	...
	# CSV case
		...
		response = StreamingHttpResponse((csv_writer.writerow(row) for row in get_csv_query(param_export_file['request_data'], qs, param_export_file['STATIC_URL']  +'/excel_csv/'+ param_export_file['filename'],col_name,tz=param_export_file['tz'])), content_type='text/csv') 
		response['Content-Disposition'] = 'attachment; filename=' + param_export_file['filename']
		...
		render_to_pdf_result=render_to_pdf('tags/pdf_format.html', context_instance=rc, header_template=None, footer_template='tags/tag_footer.html', filename=param_export_file['filename'])
...
# 		response = HttpResponse(json_data, content_type='text/csv') 
