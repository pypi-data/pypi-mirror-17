...
def changeTimeByTzNew(dates, session_tz, system_tz, format):
    if type(dates[0]) is str:
        date_from = datetime.strptime(dates[0], format)
        date_to = datetime.strptime(dates[1], format)
...
def updateDateTz(request_data):
    ...
    if request_data['django_timezone'] != '':
        ...
        user_tz = pytz.timezone(request_data['var_settins']['TIME_ZONE'])
...
def formatDateCubeNew(date, format='%Y%m%d%H%M%S'):
