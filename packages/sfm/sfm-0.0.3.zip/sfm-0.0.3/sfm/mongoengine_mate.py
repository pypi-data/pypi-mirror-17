...
def smart_insert(Document, data, minimal_size=5):
        ...
        # 失败了
            ...
            if n >= minimal_size ** 2:
                ...
                for chunk in grouper_list(data, n_chunk):
                    smart_insert(Document, chunk, minimal_size)
            # 否则则一条条地逐条插入
            else:
                for document in data:
                    try:
                        Document.objects.insert(document)
                    except mongoengine.NotUniqueError:
                        pass
    else:
        try:
