...
elif 'cx_freeze' in sys.argv:
    # compilación para linux y mac:

    from cx_Freeze import setup, Executable

    desc = "PyAfipWS cx freeze build"

    for mod in sys.modules.keys():
        try:
            if "." not in mod and not mod.startswith(("_", "swig", "Fix")):
                print "agregando", mod
                m = __import__(mod)
                includes.append(mod)
        except:
            pass
    for x in "PIL.ImageTk", "FixTk":
        excludes.remove(x)
    opts = {"build_exe": {"packages": includes, "excludes": excludes}}
    kwargs = {"executables": [Executable("wsaa.py"), Executable("rece1.py")]}
    sys.argv[sys.argv.index("cx_freeze")] = "build"
