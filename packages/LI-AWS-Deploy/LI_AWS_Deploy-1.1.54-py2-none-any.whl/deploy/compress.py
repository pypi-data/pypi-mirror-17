    ...
    try:
        with open(destPath, 'w') as f:
            for dirc, srcFile in sourcePaths:
                print(srcFile)
                with open("{}{}".format(os.path.join(baseDir, dirc), srcFile)) as inputFile:
...
def minifyJS(current_dir, baseDir=baseDirStatic, source=jsSources):
    ...
    jsDestPath = "{}all.js".format(os.path.join(workdir, "js"))
    jsMinPath = "{}all.min.js".format(os.path.join(workdir, "js"))
    return saveFile(source, jsDestPath, jsMinPath, workdir)


def minifyCSS(current_dir, baseDir=baseDirStatic, source=cssSources):
    workdir = os.path.join(current_dir, *baseDir)
    cssDestPath = "{}all.css".format(os.path.join(workdir, "css"))
    cssMinPath = "{}all.min.css".format(os.path.join(workdir, "css"))
