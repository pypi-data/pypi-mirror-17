...
class BankCardUserAuther(object):
    ...
    def _second_bindcheck(self):
        # 第二步 更改银行卡支行信息
        if all([self.auth_data[i] for i in ["province", "city", "bank_branch_name"]]):
            return True
        return False
    ...
    def _get_bankcard(self):
        ...
        bank_obj = {}
        bank_obj.update(BankVerifySerializers(bankverify_obj).data)
        bank_obj.update(BankInfoSerializers(bankverify_obj.bank_info).data)
        bank_obj.update(userpayinfo_dict)
        bank_obj.update(VerifiedUserSerializers(verifieduser_obj).data)
        return bank_obj
