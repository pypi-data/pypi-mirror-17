...
class TestWorkflow(unittest.TestCase):
    ...
    def test_apply_workflow(self):
        ...
        node = workflow.add_stream(stream)
        workflow.connect_workflow_input(node, apply=True)
        workflow.connect_workflow_output(node, apply=True)

        input = Input('item')
        input._queue.extend(([1, 2, 3, 4, 5], [5, 4, 3, 2, 1], [6, 7, 8, 9, 0], [0, 9, 8, 7, 6]))
        input.close()
        output = Output('item')
        output2 = Input('item')
        output.pipe(output2)
        ...
        self.assertEqual([2, 4, 6, 8, 10], output2._queue.popleft())
