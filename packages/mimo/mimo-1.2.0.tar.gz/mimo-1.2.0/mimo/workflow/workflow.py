...
from mimo.asynctools import azip
from mimo.io import Input, Output, IOSet
from mimo.stream import Stream, StreamFromInput, StreamToOutput, StreamFromIterator, StreamToList
...
class Workflow(Stream):
    def __init__(self, ins=tuple(), outs=tuple(), *, name=None, threshold=100, loop=None):
        ...
        self.workflow_apply_inputs = defaultdict(set)
        self.workflow_apply_outputs = {}
    ...
    async def run(self, ins=None, outs=None):
        if ins is not None:
            for workflow_input, streams in self.workflow_inputs.items():
                for stream in streams:
                    stream.input = ins[workflow_input]
        if outs is not None:
            for workflow_output, stream in self.workflow_outputs.items():
                stream.output = outs[workflow_output]

        if len(self.workflow_apply_inputs) == 0:
            tasks = [stream.run(self.input_sets[stream_id], self.output_sets[stream_id])
                     for stream_id, stream in self.streams.items()]
            await asyncio.gather(*tasks)
        else:
            apply_inputs = self.workflow_apply_inputs.keys()
            async for items in azip(*[ins[input] for input in apply_inputs]):
                for input in self.inputs.values():
                    input.open()
                for input, item in zip(apply_inputs, items):
                    for stream in self.workflow_apply_inputs[input]:
                        stream.iterator = item
                for stream in self.workflow_apply_outputs.values():
                    stream.collection = []
                tasks = [stream.run(self.input_sets[stream_id], self.output_sets[stream_id])
                         for stream_id, stream in self.streams.items()]
                await asyncio.gather(*tasks)
                for apply_output, stream in self.workflow_apply_outputs.items():
                    await outs[apply_output].push(stream.collection)
        if outs:
            for out in outs:
                out.close()
    ...
    def connect_workflow_input(self, node, *, workflow_input=None, node_input=None, apply=False):
        """
        ...
        if workflow_input is None:
            if len(self.ins) == 1:
                ...
                raise ValueError('Workflow has multiple connections')

        if apply:
            stream = StreamFromIterator(None)
            self.workflow_apply_inputs[workflow_input].add(stream)
        else:
            stream = StreamFromInput(None)
            self.workflow_inputs[workflow_input].add(stream)
        input_node = self.add_stream(stream)
        input_node.pipe(node, input=node_input)

    def connect_workflow_output(self, node, *, workflow_output=None, node_output=None, apply=False):
        """
        ...
        if apply:
            stream = StreamToList()
            self.workflow_apply_outputs[workflow_output] = stream
        else:
            stream = StreamToOutput(None)
            self.workflow_outputs[workflow_output] = stream
    ...
    def _add_vertices(self, stream, stream_id, input_ids, output_ids):
        ...
        self.input_sets[stream_id] = IOSet(Input(name, self.threshold) for name in stream.ins)
        self.output_sets[stream_id] = IOSet(Output(name) for name in stream.outs)
