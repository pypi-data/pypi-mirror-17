...
class OAIComponent(Component):
    ...
    def match(self, setspec=None, from_date=None, until_date=None,
              token=None, metadata_prefix=None):
        ...
        `setspec` does not match any known setspecs, a ValueError is raised.
        ...
        if setspec is not None:
            ...
            if (metadata_prefix is not None
                ...
                raise ValueError('metadata prefix {0} not supported for set {1}'.format(
                    metadata_prefix, setspec))
            if relation:
                try:
            ...
            if metadata_prefix is not None:
                def specifiers():
                    try:
                        ...
                        raise ValueError('metadata prefix {0} not supported'.format(
                            metadata_prefix))
        ...
        if where:
            qs += ' WHERE ' + ', '.join(where)
        if subquery:
            qs += ' WITH X BEING ({0})'.format(subquery)
...
class OAIPMHRecordAdapter(EntityAdapter):
    ...
    def metadata(self, prefix):
        ...
        try:
            ...
            raise ValueError('unsupported metadata prefix "{0}"'.format(prefix))
