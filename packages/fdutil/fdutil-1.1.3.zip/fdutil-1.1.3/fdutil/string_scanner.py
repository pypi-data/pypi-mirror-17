...
class StringScanner(object):
    ...
    def find(self,
        ...
        # Peek...
        try:
            peek = self.__string[self.__index:self.__index + value_length]
        except IndexError:
            try:
                peek = self.__string[self.__index:]
            except IndexError:
                peek = None
        #-----------------------------------------------------------------
        while not peek == value and not self.__index >= self.__len:
            self.__index += 1
            # -----------------------------------------------------------------
            # Peek...
            try:
    ...
    def read_until_value_read(self,
        ...
        start = self.__index
        i = self.__string.find(value, start)
        self.__index = self.__len if i < 0 else (self.__len if i + len(value) > self.__len else i + len(value) )
        return self.__string[start:i]
...
if __name__ == u'__main__':
    ...
    s=StringScanner(u'   <  some text blah "some more text" >    \n<!-- This is a comment <a>with an embedded element!</> -->***    xx')
    s.find(u"<")
    assert s.peek() == u"<"
    assert s.read() == u"<"
    s.skip_any(u" ")
    ...
    print s.peek(7)
    assert s.peek(7) == u"text bl",u"'{value}'".foramt(value = s.peek(7))
    s.fwd()
    a = s.read()
    assert a==u'e',a
