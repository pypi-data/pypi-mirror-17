...
def deploy_stage(client, args):
    ...
    task_definition_arn = _new_task_definition_from_test(
        client, args.test_cluster or 'test', args.service[0], args.tag,
        args.image)
    cluster = args.cluster or args.env
    service_arn = _service_arn(client, cluster, args.service[0])
    _prefix, service_name = service_arn.split('service/')
    LOGGER.info('Updating %s in %s to run %s', service_name,
                cluster, task_definition_arn.split('/')[-1])
    result = client.update_service(cluster=cluster, service=service_arn,
                                   taskDefinition=task_definition_arn)
    if result['service']['taskDefinition'] == task_definition_arn:
        LOGGER.info('Updated %s in %s to %s', service_name,
                    cluster, task_definition_arn.split('/')[-1])
        if args.wait:
            _restart_tasks(client, cluster, service_name, args.tag, args.wait)
    return True
...
def parse_cli_args():
    ...
    stage_parser.add_argument('-w', '--wait', action='store_true',
                              help='Wait for running tasks to be replaced')
    stage_parser.add_argument('--test-cluster', action='store', default=None,
                              help='The test environment ECS cluster name')
...
def _new_task_definition_from_test(client, cluster_name, service_name, tag,
                                   image_name):
    service_arn = _service_arn(client, cluster_name, service_name)
    task_definition = _task_definition(client, cluster_name, service_arn)
