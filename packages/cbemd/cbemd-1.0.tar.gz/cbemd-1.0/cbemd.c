...
/******************************************************************************
 float cbemd(signature_t *Signature1, signature_t *Signature2,
 ...
 Flow       (Optional) Pointer to a vector of flow_t (defined in cbemd.h)
          ...
          flow_t *Flow, int *FlowSize)
{
    ...
    if (_n1 > 1 && _n2 > 1)  /* IF _n1 = 1 OR _n2 = 1 THEN WE ARE DONE */
    {
        for (itr = 1; itr < MAX_ITERATIONS; itr++)
        {
            ...
            if (isOptimal(U, V))
                break;
            if (GlobalError == -99998)
            {
                return -99998.0;
            }
            /* IMPROVE SOLUTION */
            newSol();
            if (GlobalError == -99996)
            {
                return -99996.0;
            }
        ...
        if (itr == MAX_ITERATIONS)
            fprintf(stderr, "cbemd: Maximum number of iterations has been reached (%d)\n",
                    MAX_ITERATIONS);
    }
                  ...
                  float (*Dist)(feature_t *, feature_t *))
{
    ...
    {
        fprintf(stderr, "cbemd: Signature size is limited to %d\n", MAX_SIG_SIZE);
        GlobalError = -99997;
    }
...
static void findBasicVariables(node1_t *U, node1_t *V)
{
    ...
    while (UfoundNum < _n1 || VfoundNum < _n2)
    {
        ...
        if (! found)
        {
            fprintf(stderr, "cbemd: Unexpected error in findBasicVariables!\n");
            fprintf(stderr, "This typically happens when the EPSILON defined in\n");
            fprintf(stderr, "cbemd.h is not right for the scale of the problem.\n");
            GlobalError = -99999;
            break;
        }
    }
...
static int isOptimal(node1_t *U, node1_t *V)
{
    ...
    {
        fprintf(stderr, "cbemd: Unexpected error in isOptimal.\n");
        GlobalError = -99998;
    }
...
static int findLoop(node2_t **Loop)
{
    ...
    if (CurX == Loop)
    {
        fprintf(stderr, "cbemd: Unexpected error in findLoop!\n");
        GlobalError = -99996;
    }
#if DEBUG_LEVEL > 3
    printf("FOUND LOOP:\n");
