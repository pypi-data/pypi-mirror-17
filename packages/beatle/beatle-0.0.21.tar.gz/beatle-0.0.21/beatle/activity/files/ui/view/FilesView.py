...
from activity.files.ui import ui, pane, dlg
from activity.files.actions import debugPythonSession
from activity.files.actions import build
...
class FilesView(BaseView, ui.FilesView):
    ...
    def DoRenderAddElement(self, obj):
        ...
        treeOrder = [
            ...
            model.file.Dir, model.file.File
            ]
    ...
    @wxx.CreationDialog(dlg.NewFile, model.file.File)
    def OnAddFiles(self, event):
        """Event handler"""
        args = [model.file.Dir, model.Project]
    ...
    @wxx.CreationDialog(dlg.NewDir, model.file.Dir)
    def OnAddDir(self, event):
        """Event handler"""
        container = self.selected.inner(model.file.Dir)
        if not container:
    ...
    def OnDelete(self, event):
        ...
        obj = self.selected
        if type(obj) is model.file.Dir:
            stack.instance.SetName('delete dir {0}'.format(obj.name))
        elif type(obj) is model.file.File:
    ...
    def OnTreeOpenFile(self, event):
        """Handle open file"""
        obj = self.selected
        if obj is None or type(obj) is not model.file.File:
            return
        ...
        if re.match(r"(.)*\.txt$", obj._file):
            ...
            return
        if re.match(r"(.)*\.pdf$", obj._file):
            import app.ui.pane as apane
            frame.OpenDoc(apane.PdfViewPane, obj)
            return
        # fallback to text file if not binary
        try:
    ...
    def OnUpdateDelete(self, event):
        ...
        if hasattr(self, '_dbg_session'):
            ...
            if self.selected._readOnly:
                ...
                return
            t = type(self.selected)
            if t is model.file.File:
                event.Enable(True)
                return
            if t is not model.file.Dir:
                event.Enable(False)
                return
            if any(x for x in self.selected(model.file.File,
                model.file.Dir) if x._readOnly):
                event.Enable(False)
                return
            event.Enable(True)
        else:
    ...
    def OnUpdateRunFile(self, event):
        ...
        if not hasattr(self.frame, '_runProcess'):
            obj = self.selected
            event.Enable(bool(obj and type(obj) is model.file.File
    ...
    def OnToggleBreakpoint(self, event):
        ...
        if hasattr(self, '_dbg_session'):
            pfile = page._object
            if page.breakpoint:
                cmd = 'break {file}:{bp}'.format(file=pfile._file, bp=line + 1)
    ...
    def OpenPythonFileLine(self, path, line):
        ...
        for wrk in context.app.workspaces:
            items = wrk(model.file.File, filter=lambda x: x.abs_file == p)
            if not len(items):
                continue
    ...
    def UpdateBreakpoins(self):
        ...
        pfiles = project(model.file.File)
        for pfile in pfiles:
            for bp in project.breakpoints(pfile):
                cmd = 'break {file}:{bp}'.format(file=pfile._file, bp=bp + 1)
                self._dbg_session.send_command(cmd)
