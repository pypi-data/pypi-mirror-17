...
class mainWindow(base.MainWindow):
    ...
    def _import_views(self):
        ...
        self._views = dict([(cls, None) for cls in activity.ALL])
    ...
    def RemoveToolsMenu(self, name, menu):
        ...
        if index != wx.NOT_FOUND:
            app.unset_menu_handlers(self, menu,
                self.redirectedToFocus, self.redirectedToFocus4Enabled)
    ...
    @wxx.CreationDialog(app.ui.dlg.WorkspaceDialog, model.Workspace)
    ...
    def OnSettingsBuildTools(self, event):
        ...
        app.ui.dlg.BuildTools(self).ShowModal()
    ...
    def OnEditPreferences(self, event):
        ...
        app.ui.dlg.PreferencesDialog(self).ShowModal()
    ...
    def ShowReference(self, url, title='Help'):
        ...
        if wx.__version__ >= '3.0.0.0':
            #self.docBook.Freeze()
            self.docBook.AddPage(p, title, True, rc.GetBitmapIndex("info"))
            #self.docBook.Thaw()
        else:
    ...
    def RunPythonScript(self, script):
        ...
        try:
            ...
            threading.Thread(target=worker_pipe, args=(self,)).start()
        except:
            handler = self.GetEventHandler()
    ...
    def OnFindInFiles(self, event):
        ...
        theDialog = app.ui.dlg.FindInFilesDialog(self)
        if theDialog.ShowModal() != wx.ID_OK:
            return
        self.m_searchList.ClearAll()
        w, h = self.m_searchList.GetSize()
    ...
    def OnSelectSearchItem(self, event):
        ...
        try:
            import activity.files.ui.pane as fpane
            import model.file
            ...
            for wrk in context.app.workspaces:
                items = wrk(model.file.File, filter=lambda x: x._file == p)
                if not len(items):
                    continue
