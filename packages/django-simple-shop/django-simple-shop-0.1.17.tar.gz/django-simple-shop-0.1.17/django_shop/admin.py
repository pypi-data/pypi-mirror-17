    ...
    def get_form(self, request, obj=None, **kwargs):
        if not request.user.is_superuser or \
                not request.user.has_perm(
                    'django_shop.category_can_edit_important'):
            self.fields = ProductCategory.NON_IMPORTANT_FIELDS
            self.prepopulated_fields = {}
        return super(ProductCategoryAdmin, self).get_form(request,
                                                          obj=obj, **kwargs)
...
class ProductAdmin(SummernoteModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        if not request.user.is_superuser or \
                not request.user.has_perm('django_shop.product_can_edit_important'):
            self.fields = Product.NON_IMPORTANT_FIELDS
            self.prepopulated_fields = {}
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ('id', 'products_string_html', 'state', 'created',
                    'changed', 'full_price')
    fields = ('state', 'created', 'changed', 'delivery_type', 'delivery_price',
              'receiver', 'full_price', 'payment_method')
    readonly_fields = ('created', 'changed', 'receiver', 'full_price',
                       'payment_method')
    inlines = [
        ProductOrderInline
    ]
