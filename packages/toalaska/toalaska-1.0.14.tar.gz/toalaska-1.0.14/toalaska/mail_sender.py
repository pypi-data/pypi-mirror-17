...
class MailSender():
    ...
    # def ma
    def send(self,to,subject,content,attach_filepath=None,attach_filename=None,content_type=None):
        ...
        msg["to"] = to
        #正文
        if not content_type or content_type not in ["plain","html"]:
            content_type="plain"
        body =  MIMEText(content,content_type,self.encoding)
        msg.attach(body)
        #附件
        if attach_filepath:
