...
#import saltSqlaDB
import saltMongDB
#from sqlalchemy.sql.operators import Operators as SQLAOperators
import datetime
...
Case = Enum('ViewCase','viewAll viewOne edit')
...
class FieldItem:
    def __init__(self,viewObj,field ):
    ...
    @property
    def value(self):
        return self.field.__get__(self.viewObj)

    @value.setter
    def value(self,svalue ):
        self.field.__set__(self.viewObj,svalue)

    @property
    def strvalue(self):
        val =self.field.__get__(self.viewObj)
...
class BaseField(object):
    ...
    Cols are logical fields in the db. They may be direct db cols (db=True) or calculated from other
    columns. Columns with that are only used to calcuate other columns have form=False

    a dictionary of Cols derived from basecol is needed by a SaltTable to enable the salttable to do the following:-
    ...
    def __init__(self,label=None,maxim=16,wide=0,hint="",*,hint2='',name=None,src='',rowLayout='[xx]',fmt={}
            ,edit=True,readOnlyBracket="  ",db=None,row=None,form=True,misc=None,colwidth=0
            ,postPrefix='field',value=None,posn=0,comment="",rowLabel=False,key=None,cases=None):
        """
        ...
        if wide==0:wide=maxim
        self.fmt={'size':wide,'max':maxim,'postPrefix':postPrefix
            ,'readOnlyBracket':readOnlyBracket}
        self.fmt.update(fmt)
    ...
    def __get__(self, obj, objtype=None):
        ...
        if self.src is not None:
            if isinstance(self.src,str):
                ...
                if self.src not in obj.dbRows_[obj.idx_]:
                    obj.joins_[self.src]()
                return obj.dbRows_[obj.idx_][self.src].get(
                            self.name,self.default(obj))
                #        ,self.name)
            return obj.dbRows_[obj.idx_].get(self.name,self.default(obj))
    ...
    def __set__(self, obj, val):
        ...
        if self.src:
            if val!=self.__get__(obj):
                #print(' it is changed?',val,self.__get__(obj),val!=self.__get__(obj))
                if isinstance(self.src,str): #was always false for mongo- not now!
                    setattr(getattr(obj.dbRows_[obj.idx_],self.src)
                        ,self.name,self.storageFormat(val))
                else:
                    v=self.storageFormat(val)
                    obj.dbRows_[obj.idx_][self.name]=v
                    obj.changes_[obj.idx_][self.name]=v
        else:
            self._value=val
    ...
    def setup(self,name,sampleRow,view):
        ...
        if self.src is None:
            ...
            if view.joins_:
                self.src=view.joinkeys_[0] #use default
            else:
                self.src= True
...
class EnumField(BaseField):
    ...
    def Type(self,obj):
        ...
        if not self._Type:
            ...
            self._Type=Enum(self.label,zip(noBlanks(nameVals.names),nameVals.values))
        #print('at enum tr')
...
class EnumForeignField(EnumField):
    ...
    def __init__(self,*args
                 ,zeroText=None,values=None,dispFields=()
                 ,session=None,saltFilter=None,sqfilter=None
                 ,**kwargs):
    ...
    def nameVals(self ,obj):
        ...
        self.xtable=self.session.baseDB.db.get_collection(
                        saltMongDB.pclass(firstDisp)) #table name
        filt={}
        try:
            if self.saltFilter:
        ...
        rows = [r for r in self.xtable.find(filt)] #session.query(self.xtable)
        ...
        names = baseNms +[', '.join([row[saltMongDB.key(dispField)]
...
def strToDate(val):
    ...
    for char in ':/-':
        ...
        if len(val)==4: #expiry date?
            ...
            raise ValueError('Cannot conver str to date: no separator found and bad lenght')
    try:
...
class TxtListField(BaseField):
    ...
    def __xxset__(self, obj, val): #on hold in case we need it
        """as a base method this assumes 'val' is in the format data comes __get__()
          overides must handle 'txt' or format from __get__(), then call this method
          may also need to overide storage format to reverse mapping from db in get"""
        print('IntField',type(val),val)
...
class BaseArrCol(BaseCol):
    ...
    def fromStr(self,pstr):
        if isinstance(pstr,list):return pstr
        return pstr.split(',')
    ...
    def getValue(self,tbl=None):
        if self.dbTable and self.value==None:
            self.value=self.dbTable.txtArr(self.name
                ,defVal(self.misc,'minElts',0),defVal(self.misc,'maxElts',100))
        return self.value
    def putValueInDb(self,tbl=None,strVal=None,idx=None):
        if strVal!=None:self.value=self.fromStr(strVal)
        row=self.row
        if not row:row=self.dbTable.data

        #if self.dbTable and self.value!=None: -> self .dbtable set for both anyway?
        if self.dbTable and self.value!=None:
...
class NumArrCol(BaseArrCol):
    ...
    """
    def fromStr(self,pstr):
        if isinstance(pstr,list):return [saltInt(n) for n in pstr]
        return [saltInt(n) for n in pstr.split('-')]
    def getValue(self,tbll=None): #note -overides to use numArr!
        if self.dbTable and self.value==None:
            self.value=self.dbTable.numArr(self.name
                ,defVal(self.misc,'minElts',0),defVal(self.misc,'maxElts',100))
        return self.value
    ...
    def putValueInDb(self,tbll=None,strVal=None,idx=None):
        ...
        if self.dbTable and self.value!=None:
            ...
            ret=row.colToDb(self.name,'-'.join([str(n) for n in self.value]))
            return ret
        return row.colFromDb(self.name,"")
...
class BoolFlagCol(PosnField):
    def __init__(self,name,label,hint="",rowLayout='--x-',edit=True,readOnlyBracket=' ',fmt={}
        ,posn=0,char='y',**dargs):
        ...
        BaseCol.__init__(self,name,label,hint=hint,rowLayout=rowLayout,edit=edit
        ,readOnlyBracket=readOnlyBracket,fmt=fmt,posn=posn,**dargs)
        self.fldlabel=label
        self.posn=posn
        self.char=char
        return
    ...
    def extractPost(self,postData,tablee=None):
        ...
        debecho('xtrabool',val,val1,"x".join(val1),val2,self.posn+1,self.name,table.data,self.fmt,postData)
        try:
...
class TblIdxCol(BaseCol):
    def __init__(self,name,label,xtable=None,zeroText="",idField='id',dispField='label'
        ,cursor=None,**others):
    #   """(name,max,hint,rowLayout) returns std array- wrong class """
    ...
    def drawData(self,key,val,tablee=None,fmt=None):#draw(self,page): #fmdrawitem
        ...
        def drawOpt(txt,val,sel):
            ...
            return '<option value="%s" %s > %s </option><br>' % (val,selstr,txt)
        # #dumpit($fld,'flds');
        if not fmt:fmt=self.fmt #fmt probably no use with drawData!!
        tbl=self.dbTable
        row=self.row
        key=self.name
        tstr=""#self.drawLabel(tbl)# ;
        if False: #not self.edit:#simply look up value
            ...
            for id,lbl in data:
                vals.append((lbl,id))
            return dict(type='listbox',value=thisVal,name=self.name,choices=vals
                ,label=self.label,hint=self.hint)
    ...
    def fromStr(self,pstr):
        ...
        if isinstance(self.value,int):
            return saltInt(pstr)#values can be int or txt- so decide based on what we have now
        if isinstance(self.value,long):
            return saltLong(pstr)#values can be int or txt- so decide based on what we have now
        return pstr
