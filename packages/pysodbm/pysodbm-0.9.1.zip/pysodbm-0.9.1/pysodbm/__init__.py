...
class EsgentoModelRelation(object):
    """
    Basisklasse einer Tabellen-Relation. Sie stellt innerhalb eines EsgentoModels
    eine Relation zu einem anderen EsgentoModel dar. Die Definition erfolgt innerhalb
    des EsgentoModels muss im filter ein oder mehrere vorhanden Felder referenzieren.
    Dadurch wird es zur Laufzeit möglich dynamisch auf die Objekte der in der Relation
    befindlichen Tabelle zuzugreifen

    Beispieldefinition:
        CART_ID = EsgentoModelField("", int, 0)
        CART = EsgentoModelOneToOneRelation(singletonClass=Cart, filter={"ID": CART_ID})
    ...
    def transformFilter(self, obj):
        ...
        filter (self.filter) die EsgentoModelFields mit den am übergebenen Objekt
    ...
    def getRelationData(self, obj):
        ...
        :param obj:
        :return: EsgentoSingletonClass oder list mit EsgentoSingletonClass-Instanzen
        """
        return None

    def relatedModel(self):
        return EsgentoSingletonClass.getChildClasses()[self.singletonClassName].esgentoModel


class EsgentoModel(object):
    """
    Basisklasse eines Esgento Datenbank Models
    """
    ...
    def modelFields(cls):
        """
        Liefert ein Dictionary mit allen im Model definierten EsgentoModelFields.
            ...
            for key in dir(cls):
                if key[0] != "_":
                    if not key.startswith("cached"):
                        ...
                        if isinstance(attr, EsgentoModelField):
        ...
        if cls.cachedModelRelations is None:
            ...
            for key in dir(cls):
                if key[0] != "_":
                    if not key.startswith("cached"):
                        attr = getattr(cls, key)
                        if isinstance(attr, EsgentoModelRelation):
        ...
        if cls.cachedPrimaryKey is None:
            for key in dir(cls):
                if key[0] != "_":
                    if not key.startswith("cached"):
                        attr = getattr(cls, key)
                        if isinstance(attr, EsgentoModelKey):
    ...
    def parseFilter(cls, filter={}):
        ...
        for key, value in filter.items():
            if not isinstance(value, EsgentoFilter):
    ...
    def getQuerySQL(cls, primaryKeyValues):
        ...
        if where != "":
            msql = "select %s from %s where %s" % (
                ",".join(modelfields.keys()),
                cls.dbTableName,
                where
            )
            return msql
    ...
    @classmethod
    def modelMatchingFields(cls, esgentoModel, withPrimaryKeys):
        ownFields = cls.modelFields().keys()
        if not withPrimaryKeys:
            ownFields = list(set(ownFields) - set(cls.primaryKey().fields().keys()))
        foreignFields = esgentoModel.modelFields().keys()
        matchedFields = list(set(ownFields) & set(foreignFields))
        return matchedFields

    @classmethod
    def modelHasMatchingFields(cls, esgentoModel, withPrimaryKeys=True):
        matchedFields = cls.modelMatchingFields(esgentoModel, withPrimaryKeys)
    ...
    @classmethod
    def hasRelationTo(cls, esgentoModel):
        mrs = cls.modelRelations()
        if len(mrs) != 0:
            for mrName, mr in mrs.items():
                mrModel = mr.relatedModel()
                if mrModel == esgentoModel:
                    return True
        return False

    @classmethod
    def getRelationTo(cls, esgentoModel):
        mrs = cls.modelRelations()
        if len(mrs) != 0:
            for mrName, mr in mrs.items():
                mrModel = mr.relatedModel()
    ...
    def getJoinClause(cls, targetModel, relationName=None):
        ...
        for fieldName, fieldValue in relation.filter.items():
            ...
            if isinstance(fieldValue, EsgentoModelField):
...
class EsgentoModelField(object):
    """
    Basisklasse eines EsgentoModel Datenbank-Feldes.
    """
    ...
    def SetAttributeHook(modelField):
        def realSetAttributeHook(func):
            ...
            for m in modelFields:
                if isinstance(m, EsgentoModelField):
    ...
    def __init__(self, caption, fieldType, defaultValue, extra="", null=None, converter=None):
        """
        EsgentoModel Datenbankfeld. Es können verschiedene Parameter des Feldes beeinflusst werden,
        ...
        :param converter: EsgentoModelConverter Objekt
        :param primaryKey: bool, ob dieses Feld der Primary Key ist.
        """
        super(EsgentoModelField, self).__init__()
    ...
    def setConverter(self, converter):
        ...
        :param converter: EsgentoModelConverter
        """
    ...
    def convertFromValue(self, value):
        """
        Fallback Converter von Wert nach Unicode String

        :param value: Wert
        :return: unicode: Stringdarstellung des Wertes
        """
    ...
    def convertToValue(self, convertedString):
        """
        Fallback Converter von Unicode String nach Wert
...
class EsgentoModelDummyField(EsgentoModelField):
    pass


class EsgentoModelKey(object):
    """
...
class EsgentoModelValueMapping(object):
    def __init__(self, esgentoModel, keyField, valueField):
        self.esgentoModel = esgentoModel
        self.keyField = keyField
        self.valueField = valueField

    def __call__(self, esgento=None):
        selectQuery = "select %s, %s from %s" % (self.keyField, self.valueField, self.esgentoModel.dbTableName)
        return dict((x[0], x[1]) for x in esgento.dbQuery(selectQuery))


class EsgentoModelValueCleanStringMapping(EsgentoModelValueMapping):
    def __call__(self, esgento=None):
        data = super(EsgentoModelValueCleanStringMapping, self).__call__(esgento)
        if type(data.values()[0]) in (str, unicode):
...
class EsgentoFilter(object):
    @staticmethod
    def parseFilter(filter={}):
        for key, value in filter.items():
            if isinstance(value, EsgentoFilter):
...
class EsgentoFilterNot(EsgentoFilter):
    def getFilterString(self):
        return "!= %s" % self.getConvertedValue()


class EsgentoFilterIn(EsgentoFilter):
    def getConvertedValue(self):
...
class EsgentoFilterNotIn(EsgentoFilterIn):
    def getFilterString(self):
        return "not in (%s)" % self.getConvertedValue()


class EsgentoFilterIsNull(EsgentoFilterIn):
    def getFilterString(self):
        return "is Null"


class EsgentoFilterIsNotNull(EsgentoFilterIn):
    def getFilterString(self):
        return "is not Null"


class EsgentoFilterLike(EsgentoFilter):
    def getConvertedValue(self):
...
class EsgentoRecordNotFoundError(Exception):
    def __init__(self, cls=None, esgento=None, primaryKeyValues=None):
        super(EsgentoRecordNotFoundError, self).__init__(
    ...
    def getUnboundObject(self):
        obj = self.cls(esgento=self.esgento, primaryKeyValues=self.primaryKeyValues, loadFromDatabase=False)
        return obj


class EsgentoUnboundSingletonInitError(Exception):
    def __init__(self, message):
        super(EsgentoUnboundSingletonInitError, self).__init__(message)


class EsgentoSingletonMetaClass(type):
    """
    Metaklasse fuer Singleton Esgento Objekte. Die Objekte die zurückgeliefert werden, existieren nur einmal
    im Speicher.
    """
    _instances = {}
    _restClientClasses = {}
    ...
    def getCacheClass(cls):
        ...
        if cacheClass == True:
            ...
            for i in range(len(mro)):
                if mro[i] == EsgentoSingletonClass:
    ...
    def deleteClassInstance(cls, obj):
        ...
        del cls._instances[cacheClass][id(obj.esgento)][obj.primaryKeyRepresentation()]

    @classmethod
    def bind(cls, obj):
        cacheClass = cls.getCacheClass(obj.__class__)
        cls._instances[cacheClass][id(obj.esgento)][obj.primaryKeyRepresentation()] = obj
        obj.unbound = False
    ...
    def __call__(cls, *args, **kwargs):
        ...
        Klasse, Esgento-System und primaryKeyValues dienen dabei zur eindeutigen Identifikation. Esgento Objekt
        und primaryKeyValues sind Keyword Argumente.

        :param args: Argumente
        :param kwargs: Keyword Argumente. esgento, id muessen vorhanden sein um ein Objekt zu referenzieren
        ...
        if kw["primaryKeyValues"] is None:
            ...
            if cls.autoGeneratePrimaryKeyValues:
                kw["primaryKeyValues"] = cls.generatePrimaryKeyValues(esgento=esgento)
            else:
                kw["unbound"] = True

        if kw["unbound"]:
            newObj = super(EsgentoSingletonMetaClass, cls).__call__(*args, **kw)
            if newObj.initOk:
                newObj.setAutoCommitEnabled(False)
                newObj.esgento = esgento
                return newObj
            else:
                raise EsgentoUnboundSingletonInitError("Unbound Object konnte nicht erstellt werden.")

        else:
            primaryKeyValues = kw["primaryKeyValues"]
            primaryKeyString = cls.esgentoModel.primaryKey().getStringRepresentation(primaryKeyValues)
            ...
            if "loadFromDatabase" in kw:
                if not kw["loadFromDatabase"]:
                    ...
                    kw["dataDict"] = cls.loadFromDatabase(esgento=esgento, primaryKeyValues=primaryKeyValues)

            # check gibts schon ein dict für diese id in dem esgento system
...
class EsgentoBareDataClass(object):
    """
    ...
    def updateFromObject(self, obj):
        newObj = EsgentoBareDataClass.loads(obj.dumps())
        for key, value in newObj.dataDict().items():
            if not hasattr(self, key):
                setattr(self, key, value)
            else:
                attr = getattr(self, key)
                if isinstance(attr, EsgentoBareDataClass):
    ...
    def dumps(self):
        """
        Serialisiert eine EsgentoBareDataClass Instanz zu einem String.
        EsgentoBareDataClass Instanzen in den Daten werden ebenfalls rekursiv mit serialisiert.
    ...
    def prettyStr(self):
        ...
        for key, value in data.items():
            if isinstance(value, EsgentoBareDataClass):
        ...
        return u"EsgentoBareDataClass(dataDict={data})".format(data=",".join(ret))
    ...
    def loads(cls, data):
        # """
        # Erstellt eine EsgentoBareDataClass Instanz auf Basis eines vorher serialisierten Strings.
        #
        # :param data: String
        # :return: EsgentoBareDataClass Instanz
        # """
...
class EsgentoDataClass(EsgentoBareDataClass):
    """
    Erweitert die EsgentoBareDataClass um die Verwendung eines
    EsgentoModels. Wird kein Dictionary zum laden angegeben, wird
    ...
    cachedModelRelations = None
    esgentoModel = None
    esgentoModelDefaultValues = None
    ...
    def cacheModelRelations(cls):
        if not cls.cachedModelRelations:
            cls.cachedModelRelations = cls.esgentoModel.modelRelations()
    ...
    def modelFields(cls):
        ...
        """
        if cls.cachedModelFields is None:
            cls.cachedModelFields = cls.esgentoModel.modelFields()
        return cls.cachedModelFields
    ...
    def setDefaultValues(cls, object):
        ...
        """
        if cls.esgentoModelDefaultValues is None:
            cls.esgentoModelDefaultValues = dict(
                (fieldName, field.defaultValue()) for fieldName, field in cls.modelFields().items()
            )
        object.loadFromDict(cls.esgentoModelDefaultValues)

    def __init__(self, dataDict=None, defaultValues=True, esgento=None):
        """
        Konstruktor der EsgentoDataClass. Durch Angabe des dataDict Dictionaries,
        ...
        self.__changes = {}
        self.dataChanged = EsgentoSignal()
        EsgentoBareDataClass.__init__(self, dataDict=dataDict)
        self.esgento = esgento
        if defaultValues:
    ...
    def isEqual(self, compareObject):
        ...
        :param compareObject: EsgentoDataClass
        :return: bool
        """
        if self.esgentoModel != compareObject.esgentoModel:
            return False
    ...
    def __setattr__(self, key, value):
        ...
        if type(value) == float and type(self.__dict__[key]) == float:
            changed = not Toolbox.isEqual(value, self.__dict__[key])
        else:
    ...
    def modelDataDict(self):
        ...
        """
        ret = {}
        fieldNames = self.esgentoModel.modelFields().keys()
    ...
    def primaryKeyValues(self):
        ...
        :return: value
        """
        pk = self.esgentoModel.primaryKey()
    ...
    def primaryKeyRepresentation(self):
        return self.esgentoModel.primaryKey().getStringRepresentation(self.primaryKeyValues())

    def primaryKeyValuesFromRepresentation(self, string):
        return self.esgentoModel.primaryKey().primaryKeyValuesFromRepresentation(string)
    ...
    def __getattribute__(self, name):
        ...
        if not mclass.cachedModelRelations:
            mclass.cachedModelRelations = mclass.esgentoModel.modelRelations()
...
class EsgentoSingletonClass(EsgentoDataClass):
    """
    Basic Klasse für Esgento Single Instance Objekte
    """
    __metaclass__ = EsgentoSingletonMetaClass
    ...
    @classmethod
    def getNextID(cls, esgento=None):
        return esgento.getNextID(cls.numberRange)

    @classmethod
    def getNextNumber(cls, esgento=None):
        return esgento.getNextNumber(cls.numberRange)

    @classmethod
    def getNextAutoIncrement(cls, esgento=None):
        selectAutoinc = """
                select auto_increment from information_schema.tables
                where table_name = '%s'
                and table_schema = DATABASE()""" % cls.esgentoModel.dbTableName
        return esgento.dbQueryDict(selectAutoinc)[0]["auto_increment"]
    ...
    def getSingletonClassByName(cls, name):
        if name not in cls.getChildClasses():
            __import__("ES.Core.EsgentoModels.{name}".format(name=name), fromlist=[name])
            # return None
    ...
    def getAvailablePrimaryKeysSQL(cls):
        if cls.cachedGetAvailablePrimaryKeysSQL is None:
            sql = "select %s from %s" % (
                ...
                cls.esgentoModel.dbTableName
            )
            cls.cachedGetAvailablePrimaryKeysSQL = sql
        return cls.cachedGetAvailablePrimaryKeysSQL

    @classmethod
    def getAvailablePrimaryKeys(cls, esgento, filter={}):
        """
        Liefert eine Liste mit den vorhandenen Primärschlüsseln innerhalb des EsgentoModels

        :param esgento: Esgento Objekt
        :return: Liste mit Primärschlüsseln
        """
        if len(filter) != 0:
            where = cls.esgentoModel.parseFilter(filter)
            where = " where " + where if len(where) != 0 else ""
        else:
            where = ""
        msql = cls.getAvailablePrimaryKeysSQL() + where
        return esgento.dbQueryDict(msql)

    @classmethod
    def syncDelete(cls, srcEsgento, dstEsgento):
        """
        ...
        :return:
        """
        srcPKList = list(unicode(x) for x in cls.getAvailablePrimaryKeys(esgento=srcEsgento))
        dstPKList = list(unicode(x) for x in cls.getAvailablePrimaryKeys(esgento=dstEsgento))

        diffPKList = list(set(dstPKList) - set(srcPKList))

        objs = cls.getMultipleRecords(esgento=dstEsgento, listPrimaryKeys=list(eval(x) for x in diffPKList))
        for obj in objs:
    ...
    @classmethod
    def getMultipleRecordsByFilter(cls, esgento, filter={}):
        """
        Liefert eine Liste mit den vorhandenen Primärschlüsseln innerhalb des EsgentoModels
        ...
        if cls.cachedSQLFieldNames is None:
            fieldNames = []
            for name, field in cls.esgentoModel.modelFields().items():
                if not isinstance(field, EsgentoModelDummyField):
                    fieldNames.append(name)
            cls.cachedSQLFieldNames = ",".join(fieldNames)

        tableName = cls.esgentoModel.dbTableName
        filter = cls.esgentoModel.parseFilter(filter)
        filter = EsgentoFilter.parseFilter(filter)
        ...
        ret = cls.getMultipleRecordsBySQL(esgento=esgento, sql=msql)
        # print msql
        return ret

    @classmethod
    def getMultipleRecordsWhere(cls, esgento, whereClause=""):
        where = "where %s" % whereClause if whereClause != "" else ""
        ret = cls.getMultipleRecordsBySQL(
            esgento=esgento,
            sql="select %s from %s %s" % (
                ",".join(cls.esgentoModel.modelFields().keys()),
                cls.esgentoModel.dbTableName,
                where
            )
        )
    ...
    @classmethod
    def getMultipleRecords(cls, esgento=None, listPrimaryKeys=[]):
        """
        ...
        pkFields = cls.esgentoModel.primaryKey().fields()
        tableName = cls.esgentoModel.dbTableName + "."
        pkFieldNames = cls.esgentoModel.primaryKey().fields().keys()
        ...
        return cls.getMultipleRecordsWhere(esgento, inClause)

    @classmethod
    def getPrimaryKeyFieldNames(cls):
        if cls.cachedPrimaryKeyFieldNames is None:
            cls.cachedPrimaryKeyFieldNames = cls.esgentoModel.primaryKey().fields().keys()
        return cls.cachedPrimaryKeyFieldNames

    @classmethod
    def getMultipleRecordsBySQL(cls, esgento, sql):
        """
        ...
        dataDicts = esgento.dbQueryDict(sql)
        ret = list(
            cls(
                esgento=esgento,
    ...
    @classmethod
    def getRecordFromJson(cls, esgento, json):
        primaryKeyFieldNames = cls.getPrimaryKeyFieldNames()
        return cls(
            esgento=esgento,
            primaryKeyValues=dict(
    ...
    @classmethod
    def generatePrimaryKeyValues(cls, esgento=None):

        primaryKey = cls.esgentoModel.primaryKey()
        if len(primaryKey.fields()) == 1:
            nextID = cls.getNextID(esgento=esgento)
            primaryKeyValues = {primaryKey.fields().keys()[0]: nextID}
        else:
            raise ValueError, "PrimaryKey cannot be autogenerated"
        return primaryKeyValues

    def __init__(self, esgento, primaryKeyValues={}, dataDict={}, loadFromDatabase=True, unbound=False,
                 defaultValues=False, overrideAutoCommit=None):
        """
        Erzeugt eine Objektinstanz einer EsgentoSingletonClass.
        Es muss ein Esgento System sowie der Wert des PrimaryKeys uebergeben
        ...
        :param args:
        :param kwargs:  esgento=Esgento-Objekt , primaryKeyValue=Wert des Primärschluessels
        """
        ...
        EsgentoDataClass.__init__(self, defaultValues=True)
        self.esgento = esgento
    ...
    def __repr__(self):
        ...
        """
        if self.unbound:
            return "<%s %s unbound %i>" % (self.__class__.__name__, self.esgento.systemName, id(self))
        else:
            return "<%s %s %s %i>" % (
                self.__class__.__name__, self.esgento.systemName, self.primaryKeyValues(), id(self))

    def __setattr__(self, key, value):
        # if not key[0] in ("_", "esgento"):
        if not key[0] == "_":
            if hasattr(self, "initOk"):
                if self.initOk:
                    if self.esgento.hooksEnabled:
                        if key in self.modelFieldNames():
                            self.modelFields()[key].processAttributeHooks(obj=self, value=value,
                                                                          oldValue=self.__dict__[key],
                                                                          fieldName=key)
            EsgentoDataClass.__setattr__(self, key, value)
            if self.__autoCommitEnabled:
                if key in self.modelFields().keys():
                    if not isinstance(self.modelFields()[key], EsgentoModelDummyField):
                        self.commit()
        else:
            EsgentoDataClass.__setattr__(self, key, value)

    @classmethod
    def loadFromDatabase(cls, esgento, primaryKeyValues):
        """
        ...
        msql = cls.esgentoModel.getQuerySQL(primaryKeyValues)
        r = esgento.dbQueryDict(msql)
        if len(r) != 0:
            return r[0]
        else:
            raise EsgentoRecordNotFoundError(cls=cls, esgento=esgento, primaryKeyValues=primaryKeyValues)
    ...
    def reloadFromDatabase(self):
        ...
        dataDict = self.loadFromDatabase(esgento=self.esgento, primaryKeyValues=self.primaryKeyValues())
        self.loadFromDict(dataDict=dataDict)

        self.setAutoCommitEnabled(oldAutoCommit)

    def deleteInDatabase(self):
        where = self.esgentoModel.primaryKey().getWhereStatement(self.primaryKeyValues())
        if where != "":
            msql = "delete from %s where %s" % (self.esgentoModel.dbTableName, where)
            self.esgento.dbQueryDict(msql)
            EsgentoSingletonMetaClass.deleteClassInstance(self)
    ...
    def commitToBackend(self, changes):
        ...
        # print sql
        self.esgento.dbQueryDict(sql)

    def updateFromDataObject(self, dataObj):
        """
        Uebernimmt Attribute von ubergebenen EsgentoDataClass Objekt und updated die die Datenbank
        sofern Autocommit ein ist. Das übergebene Objekt muss nicht das selbe EsgentoModel haben, es
        werden nur die in beiden Models enthaltenen Felder übernommen.

        :param dataObj: EsgentoDataClass
        """
        ret = False

        primaryFieldNames = self.esgentoModel.primaryKey().fields().keys()
        # ermitteln der Felder die in beiden Models vorhanden sind
        commonFieldNames = list(set(self.esgentoModel.modelFields()) & set(dataObj.esgentoModel.modelFields()))
    ...
    def create(self, autoCommitEnabled=True):
        ...
        WICHTIG!!!!
        Sollte es überladen werden, muss EsgentoSingletonMetaClass.bind
        ...
        pk = self.esgentoModel.primaryKey()
        fields = self.esgentoModel.modelFields().copy()
        ...
        dbConnection = self.esgento.dbGetConnection()
        msql = "insert into %s(%s) values(%s)" % (self.esgentoModel.dbTableName, fieldNames, values)
        # print msql
        dbConnection.dbQuery(msql)

        if autoIncrementFieldName:
            id = dbConnection.dbQuery("SELECT LAST_INSERT_ID() FROM %s" % self.esgentoModel.dbTableName)[0][0]
            setattr(self, autoIncrementFieldName, id)
        dbConnection.release()

        EsgentoSingletonMetaClass.bind(self)
        self.setAutoCommitEnabled(autoCommitEnabled)

    def getSingletonObject(self):
        singletonClassName = self.esgentoModel.singletonClass
        if singletonClassName is None:
            return self
        else:
            singletonClass = EsgentoSingletonClass.getChildClasses()[singletonClassName]
            return singletonClass(esgento=self.esgento, primaryKeyValues=self.primaryKeyValues())

    def syncToSystem(self, dstEsgento, overwriteExisting=True, createNotExisting=True, overwriteOnlyIfNewer=False):
        returnDict = {}
        ...
        except EsgentoRecordNotFoundError as e:
    ...
    @classmethod
    def getUpdatedAfter(cls, esgento=None, startDate=None, withCheck=True, additionalWhere=None):
        if withCheck:
            if "UPDATED" not in cls.esgentoModel.modelFields():
    ...
    def syncUpdatedAfter(cls, sourceSystem, destinationSystem, startDate=None, oneway=True, overwriteExisting=True,
        ...
        :param sourceSystem: esgento Herkunftssystem aus dem die Daten synchronisiert werden sollen
        :param destinationSystem: esgento Zielsystem in das synchronisiert werden soll
        ...
        tableName = cls.esgentoModel.dbTableName.upper()
        ...
        if startDate is not None:
            srcObjs = cls.getUpdatedAfter(esgento=sourceSystem, startDate=startDate, withCheck=withCheck)
            srcPks = list(obj.primaryKeyRepresentation() for obj in srcObjs)

            if not oneway:
                dstObjs = cls.getUpdatedAfter(esgento=destinationSystem, startDate=startDate, withCheck=withCheck)
                ...
                for pk in conflictPks:
                    srcObjs.remove(cls(esgento=sourceSystem, primaryKeyValues=eval(pk)))
                    dstObjs.remove(cls(esgento=destinationSystem, primaryKeyValues=eval(pk)))

            added = []
            updated = []
            for srcObj in srcObjs:
                syncOperation = srcObj.syncToSystem(
                    dstEsgento=destinationSystem,
            ...
            if not oneway:
                ...
                if syncDelete:
                    deletePks = cls.syncDelete(srcEsgento=sourceSystem, dstEsgento=destinationSystem)
    ...
    def modifyLogClassObject(self, logClassObj):
        logClassObj.setEsgento(esgento=self.esgento)
        logClassObj.setObjInstance(objInstance=self)
        return logClassObj


class EsgentoQueryModelField(EsgentoModelField):
    def __init__(self, singletonClassString, fieldName, converter=None):
        self.__singletonClass = EsgentoSingletonClass.getChildClasses()[singletonClassString]
        self.__relatedField = self.__singletonClass.esgentoModel.modelFields()[fieldName]
        self.__relatedFieldName = fieldName
        self.__relatedTableName = self.__singletonClass.esgentoModel.dbTableName

        if converter is None:
            converter = self.__relatedField.converter()

        super(EsgentoQueryModelField, self).__init__(
...
class EsgentoQueryModel(EsgentoModel):
    def __init__(self):
        EsgentoModel.__init__(self)
    ...
    def getQuerySQL(cls, primaryKeyValues):
        ...
        #     modelFields = cls.modelFields()
        #     for key, value in filter.items():
        #         if not isinstance(value, Filter.EsgentoFilter):
        #             filter[key] = "=" + modelFields[key].convertForSQL(value)
        #     return filter


class EsgentoQueryClass(EsgentoSingletonClass):
    @classmethod
    def getMultipleRecordsFilterSQL(cls, filter):
        ...
        map = {}
        for fieldName, field in cls.esgentoModel.modelFields().items():
        ...
        where = " and ".join(list("%s %s" % (key, value) for key, value in filter.items()))
        where = "where " + where if len(where) != 0 else ""
        msql = cls.esgentoModel.dbQuerySQL + where
        return msql
    ...
    def getMultipleRecordsByFilter(cls, esgento, filter={}):
        ...
        if cls.cachedFieldNames is None:
            cls.cachedFieldNames = ",".join(cls.esgentoModel.modelFields().keys())

        msql = cls.getMultipleRecordsFilterSQL(filter)
        ret = cls.getMultipleRecordsBySQL(esgento=esgento, sql=msql)
        return ret

    def __init__(self, esgento, primaryKeyValues={}, dataDict={}, loadFromDatabase=True, defaultValues=False):
        super(EsgentoQueryClass, self).__init__(esgento, primaryKeyValues, dataDict, loadFromDatabase, defaultValues)

    def loadFromDatabase(self, esgento, primaryKeyValues={}):
        """
        Lädt Felddaten des Datensatzes von der Datenbank
        ...
        msql = self.esgentoModel.getQuerySQL(primaryKeyValues)
        r = esgento.dbQueryDict(msql)
    ...
    def getSingletonObject(self):
        singletonClass = EsgentoSingletonClass.getChildClasses()[self.esgentoModel.dbSingletonClass]
        obj = singletonClass(esgento=self.esgento, primaryKeyValues=self.primaryKeyValues())
        return obj


class EsgentoModelManyToManyRelation(EsgentoModelRelation):
    def __init__(self, singletonClass=None, combiningClass=None, relationName=None, filter={}):
        ...
        super(EsgentoModelManyToManyRelation, self).__init__(singletonClass, filter)

    def getRelationData(self, obj):
        if self.cachedTargetSelectSQL is None:
            self.combiningClass = EsgentoSingletonClass.getSingletonClassByName(self.combiningClassName)
            self.singletonClass = EsgentoSingletonClass.getSingletonClassByName(self.singletonClassName)

            ccem = self.combiningClass.esgentoModel
            scem = self.singletonClass.esgentoModel
            scfn = list(
        ...
        filter = self.combiningClass.esgentoModel.parseFilter(generatedFilter)
        filter = EsgentoFilter.parseFilter(filter)
        where = " and ".join(list("%s %s" % (key, value) for key, value in filter.items()))
        where = " where " + where if len(where) != 0 else ""
...
class EsgentoModelOneToOneRelation(EsgentoModelRelation):
    """
    ...
    def getRelationData(self, obj):
        ...
        r = self.singletonClass.getMultipleRecordsByFilter(esgento=obj.esgento, filter=generatedFilter)
        if len(r) != 0:
            return r[0]
        ...
        pkFieldNames = self.singletonClass.esgentoModel.primaryKey().fields().keys()
        filterKeys = pkValues.keys()

        if len(list(set(pkFieldNames) - set(filterKeys))) != 0:
            return None

        r = self.singletonClass(esgento=obj.esgento, primaryKeyValues=pkValues, loadFromDatabase=False)
        return r


class EsgentoModelOneToManyRelation(EsgentoModelRelation):
    """
    Basisklasse einer One-To-Many Tabellen-Relation.

    Beispieldefinition:
        ID = EsgentoModelField("Id", int, 0)
        POSITIONS = EsgentoModelOneToManyRelation(singletonClass=CartHasProduct, filter={"CART_ID": ID})
...
from .Signal import EsgentoSignal
