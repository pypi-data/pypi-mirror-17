...
def nowSeconds():
    ...
    mt = int((n - datetime.datetime(1970, 1, 1)).total_seconds())
    return mt
...
def isoDateToDateTime(isoDate):
    ...
    :return: datetime
    """
    return datetime.datetime.strptime(isoDate, "%Y-%m-%dT%H:%M:%S")
...
def convertMySQLDateToISO(datum):
    t = datetime.datetime.strptime(datum, "%Y-%m-%d %H:%M:%S")
...
def datetimeFromISO(datum):
    return datetime.datetime.strptime(datum, "%Y-%m-%dT%H:%M:%SZ")
...
def unixTimeStampToDateTime(timeStamp):
    return datetime.datetime.datetime.utcfromtimestamp(timeStamp)
