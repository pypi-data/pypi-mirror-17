    ...
    def __call__(cls, *args, **kwargs):
        ...
        if kw["primaryKeyValues"] is None:
            ...
            if cls.autoGeneratePrimaryKeyValues:
                kw["primaryKeyValues"] = cls.generatePrimaryKeyValues(esgento=db)
            else:
                kw["unbound"] = True
...
class EsgentoSingletonClass(EsgentoDataClass):
    ...
    def syncDelete(cls, srcDb, dstDb):
        ...
        :return:
        """
        srcPKList = list(unicode(x) for x in cls.getAvailablePrimaryKeys(db=srcDb))
        dstPKList = list(unicode(x) for x in cls.getAvailablePrimaryKeys(db=dstDb))
    ...
    def loadFromDatabase(cls, db, primaryKeyValues):
        ...
        if len(r) != 0:
            ...
            raise EsgentoRecordNotFoundError(cls=cls, db=db, primaryKeyValues=primaryKeyValues)
    ...
    def reloadFromDatabase(self):
        ...
        dataDict = self.loadFromDatabase(db=self.esgento, primaryKeyValues=self.primaryKeyValues())
        self.loadFromDict(dataDict=dataDict)
...
class EsgentoQueryClass(EsgentoSingletonClass):
    ...
    def __init__(self, db, primaryKeyValues={}, dataDict={}, loadFromDatabase=True, defaultValues=False):
        super(EsgentoQueryClass, self).__init__(db, primaryKeyValues, dataDict, loadFromDatabase, defaultValues)
...
class EsgentoModelOneToOneRelation(EsgentoModelRelation):
    ...
    def getRelationData(self, obj):
        ...
        r = self.singletonClass.getMultipleRecordsByFilter(db=obj.esgento, filter=generatedFilter)
        if len(r) != 0:
            return r[0]
