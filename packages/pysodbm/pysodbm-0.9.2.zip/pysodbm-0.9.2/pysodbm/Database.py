...
class Database(object):
    ...
    def __init__(self):
        ...
        self.connectionWatcher = DatabaseConnectionWatcher(esgento=self)
    ...
    def dbMakeConnection(self, host="", user="", password="", db=""):
        ...
        self.systemName = "{host}_{db}".format(host=host, db=db)
    ...
    def dbQuery(self, query, returnType=RETURN_LIST, callback=None, callbackStep=1, database=None, fetchResult=True,
                withMaxRows=False, byKey=None, cls=EsgentoBareDataClass):
...
class DatabaseConnectionWatcher(threading.Thread):
    ...
    def run(self):
        while 1:
            ...
            for con in self.esgento.dbConnections:
                if con.acquire():
                    if con.expired():
                        con.close()
                    else:
            ...
            count = len(self.esgento.dbConnections)
