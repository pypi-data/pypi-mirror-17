...
def to_pdf(paper_dir, template_dir=scriptorium.TEMPLATE_DIR, use_shell_escape=False):
    ...
    template_loc = scriptorium.find_template(template, tdir=template_dir)
    if not template_loc:
      raise IOError('Cannot find template {0}'.format(template))
    template_loc = os.path.abspath(os.path.join(template_loc, '..'))
    #Need to set up environment here
    new_env = dict(os.environ)
    new_env['TEXINPUTS'] = './:{0}:{1}'.format(template_loc + '/.//', new_env['TEXINPUTS'])
