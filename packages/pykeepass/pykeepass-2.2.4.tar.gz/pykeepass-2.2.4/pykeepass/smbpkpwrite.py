...
def write_entry(kdbx_file, kdbx_password, group_path,
    ...
    # TODO support smb:// style outfiles
    if outfile:
        if outfile.startswith('smb://'):
            file_written = kp.save()
            smb_send(file_written, outfile)
        ...
        if samba_db:
            file_written = kp.save()
            smb_send(file_written, smb_kdbx_file)
            logging.info('Send database file to {}'.format(smb_kdbx_file))
        else:
            file_written = kp.save(kdbx_file)
