...
class Mongodb(Base):
    ...
    options = {
        'rmq':{
            'exchange':{
                'default':'njord_mongodb',
                'headers':'njord_mongodb_headers_all',
                'headers_any':'njord_mongodb_headers_any'
            },
            'rk':{
    ...
    def __execute__(self,name,cluster,body,future):
        ...
        self.publish(
            body=jsonify(body),
            exchange=self.options['rmq']['exchange']['default'],
            routing_key=self.options['rmq']['rk'][name] % cluster
        )
        return True
