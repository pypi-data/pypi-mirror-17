...
from .util import check_http_response_error, ThreadPool
...
class DisasterRecovery:
    ...
    def _do_action(self, *uuids, action=None, headers=None, power_on=True,
        ...
        if total is True:
            ...
            vm_list = [uuid for uuid in uuids if uuid in self.replications]
        ...
        for vm in vm_list:
            ...
            request.vca_vm_name = self.replications.get(vm)['name']
    ...
    def _submit_vcd_task(self, request):
        """
        ...
        """
        s = Session()
        name = getattr(request, 'vca_vm_name', 'Unknown Name')
        resp = s.send(request)
