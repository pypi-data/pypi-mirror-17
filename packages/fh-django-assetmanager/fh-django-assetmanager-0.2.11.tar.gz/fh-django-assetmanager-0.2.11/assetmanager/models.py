...
from .settings import IMAGE_CONTENT_TYPES, STORAGE, PREVIEW_URL_TEMPLATE
    ...
    def preview(self):
        ...
        if self.pk:
            ...
            if PREVIEW_URL_TEMPLATE:
                reo = re.compile('(?:[^/][\d\w\.]+)$')
                result = reo.findall(url)

                if '{filename}' in PREVIEW_URL_TEMPLATE:
                    url = PREVIEW_URL_TEMPLATE.replace('{filename}', result[0])
                else:
                    # legacy mode where the URL_TEMPLATE is really a BASE_URL
                    url = '{}{}'.format(PREVIEW_URL_TEMPLATE, result[0])
