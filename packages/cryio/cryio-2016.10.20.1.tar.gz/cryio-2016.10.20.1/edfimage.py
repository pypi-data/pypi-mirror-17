...
import copy
import numpy as np
from . import templates, ImageError, cbfimage


class EdfError(ImageError):
    pass
...
class EdfImage(object):
    ...
    def __init__(self, filepath):
        ...
        if self._begin == -1 or self._end == -1:
            raise EdfError('Could not find header tokens in {}. Might it be not an EDF?'.format(filepath))
        self._parse()
    ...
    def __add__(self, other):
        if not isinstance(other, (EdfImage, cbfimage.CbfImage)):
            return self
