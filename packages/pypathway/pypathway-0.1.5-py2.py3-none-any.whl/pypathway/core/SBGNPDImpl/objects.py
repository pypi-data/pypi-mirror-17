# Implement SBGN-PD elements version 1
# from general to specific, top to bottom
from xml.sax import *
from ..objects import Pathway
from ...query.common import PathwayFormat
from xml.dom.minidom import Document
from ...utils import environment as env
from ...utils import SBGNParseException
from ..BioPAXImpl.objects import NativeBioPAXParser
from ..ReactomeCustome.compress import vertex2id
import os
import shutil
import json
import re
...
def id_handle(id):
    '''
    In Cytoscape.js if the id contain some spacial characters, will lead to a select failure, so, we replace the
    ...
    def parseFromFile(file, BioPAX=None):
        try:
            with open(file) as fp:
                return SBGNParser.parse(fp.read(), BioPAX=BioPAX)
        except IOError:
...
class SBGNObject(Pathway):
    ...
    def _xml_object(self, father, doc):
        ...
        for x in self.children:
            if hasattr(x, "type") and x.type == "bbox":
                new.insert(0, x)
            else:
                new.append(x)
        for x in new:
...
class SBGNRoot(SBGNObject):
    ...
    def draw(self, setting=None):
        ...
        if env():
            ...
            #                     + "/../../static/SBGN/sampleapp-components/data/config.json", "w") as fp:
            #     fp.write(json.dumps(content))
            # with open(os.path.dirname(os.path.abspath(__file__))
            #                   + "/../../static/SBGN/sampleapp-components/data/pathway.xml", "w") as fp:
            #     fp.write(self.export(PathwayFormat.SBGN))
            try:
                shutil.rmtree(os.getcwd() + "/SBGN")
            except:
                import traceback
                pass
            shutil.copytree(os.path.dirname(os.path.abspath(__file__)) + "/../../static/SBGN", os.getcwd()
                            + "/SBGN")
            with open(os.getcwd() + "/SBGN/sampleapp-components/data/config.json", "w") as fp:
                fp.write(json.dumps(content))
            with open(os.getcwd() + "/SBGN/sampleapp-components/data/pathway.xml", "w") as fp:
                fp.write(self.export(PathwayFormat.SBGN))
            path = "'SBGN/index.html'"
            con = con.replace("{{path}}", path)
            con = con.replace("{{ratio}}", "0.7")
            return HTML(con)
    ...
    def fix_reactome(self, ratio=2.2):
        ...
        for x in self.members:
            if hasattr(x, "type") and x.type == "compartment":
                refs.append([x, [x.bbox[0].x, x.bbox[0].y, float(x.bbox[0].w) + float(x.bbox[0].x), float(x.bbox[0].h) + float(x.bbox[0].y)], []])
        for x in self.members:
            ...
            if display:
                ...
                for x in pax.find_child("feature"):
                    if root.find_by_id(x.props["rdf:resource"].replace("#", "")).class_ == "bp:ModificationFeature":
                        target = root.find_by_id(x.props["rdf:resource"].replace("#", "")).find_child("modificationType")[0]
                ...
                if s:
                    if root.find_by_id(s.props["rdf:resource"].replace("#", "")):
                        if root.find_by_id(s.props["rdf:resource"].replace("#", "")).find_child("stoichiometricCoefficient"):
                            count = root.find_by_id(s.props["rdf:resource"].replace("#", "")).find_child("stoichiometricCoefficient")[0].value
...
if __name__ == '__main__':
    with open("/Users/sheep/sbgnviz-js/sample-app/samples/mapk_cascade.xml") as fp:
        con = fp.read()
    tree = SBGNParser.parse(con)
    tree.draw()
    #print tree.summary()
    with open("/Users/sheep/Desktop/test.xml", "w") as fp:
        fp.write(tree.export(PathwayFormat.SBGN))
