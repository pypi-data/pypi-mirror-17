...
def process_file(filename, outputfile):
	...
	if mode[-1] == "top":
	    m= rx_top.search(all,pos)
	    if m is not None:
                # append everything *before* the match:
		new.append( all[pos:m.start()] )
		pos= m.end()
		mode.append("file")
		continue
	    else:
	    ...
	    m= rx_bracket1.match(all, pos)
	    if m is not None:
		#print "MATCH:",all[m.start():m.end()]
		...
		pos= m.end()
		continue
	    raise AssertionError, "parse error at:"+all[pos:pos+40]

	elif mode[-1] == "file":
	    m= rx_file_head.match(all,pos)
	    if m is not None:
		#print "MATCH:",all[m.start():m.end()]
                if not isempty(m.group(1)):
                    new.append(m.group(1))
		...
		if value_dict.has_key(name):
		    sys.stderr.write("WARNING: duplicate symbol: %value=%value\name" % \
				     (name,value))
		else:
		...
		continue
                ...
                m= rx_bracket2.match(all,pos)
                if m is None:
                    raise AssertionError, "parse error near: \"%s\"" % all[pos:pos+10]
                if not isempty(m.group(1)):
                    new.append(m.group(1))
                new.append(")\\\n")
                mode.pop()
...
def main():
    ...
    parser.add_option("--summary",  
                      action="store_true", 
                      help="convert msi substitution files to pyexpander format.",
		      )
