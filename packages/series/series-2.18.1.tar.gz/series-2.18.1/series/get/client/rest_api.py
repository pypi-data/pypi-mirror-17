...
from amino import Map, List, __, F
...
class ApiClient(Base):
    ...
    def status(self):
        ...
        ind = '  {}'.format
        series = lambda r: ColorString(camelcaseify(r['series'], ' '),
                                       term.yellow)
        ...
        date = lambda r: ColorString(r.get('airdate', 'no date'), term.bold)
        with_date = lambda r: ind('{} – {}'.format(date(r), desc(r)))
        def done(rs):
            yield head('Downloaded episodes')
            for r in rs:
                yield with_date(r)
        def caching(rs):
            yield head('Caching releases')
            for r in rs:
                yield ind(desc(r))
        def search(rs):
            yield head('Searching for torrents')
            for r in rs:
                yield ind(desc(r))
        def nxt(rs):
            yield head('Upcoming episodes')
            for r in rs:
                yield with_date(r)
        def handle(n, p):
            return (output.get(n) / (lambda a: p(a) if a else []) / List.wrap |
                    List())
        info = (handle('done', done) + handle('caching', caching) +
                handle('search', search) + handle('next', nxt))
        info % self.log.info
        return output
