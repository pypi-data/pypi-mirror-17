...
class DownloaderProc(Process, Logging):
    ...
    def _download(self):
        try:
            dl = self._downloader_fact(self.link.download_url,
...
class Downloader(ReleaseHandler):
    ...
    def _handle(self, monitor):
        if self._proc.empty:
            self._proc = self._download(monitor)
    ...
    def _download(self, monitor):
        ...
        proc = DownloaderProc(link, self._download_dir)
        proc.start()
        return Just(ProcAdapter(proc, monitor))

    def _cleanup(self):
        if (self._proc.exists(self._clean_done) or
                self._proc.exists(self._clean_timeouts)):
            self._proc = Empty()
    ...
    def _clean_done(self, a):
        ...
        if proc.done.value:
            if proc.fail.value:
                self.log.error('Downloader: {}'.format(proc.error.value))
                self._fail(a.monitor, proc.link)
            else:
                path = Path(proc.file_path.value.decode())
                ok = self._check_download(a.monitor, proc.link,
                                          proc.size.value,
                                          proc.file_size.value, path)
            self._update_id(a.monitor.id, downloading=False)
            return ok

    def _clean_timeouts(self, a):
        return False
