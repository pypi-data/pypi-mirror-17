...
class CanCatchUp(SimpleCondition):
    ...
    *latest* is the newest release in the db.
    ...
    def latest_episode(self, show):
        return show.latest_episode

    def latest_release(self, show):
        return self._latest(show)

    def ev(self, show):
        return self.latest_release(show).exists(self.latest_episode(show) > _)
    ...
    def _repr(self, show, match):
        op = '>' if match else '<'
        def has(release):
            return '{} {} {}'.format(self.latest_episode(show), op, release)
...
class ShowScheduler(ShowHandler, Tvdb):
    ...
    def _handle_valid_show(self, show):
        def catch_up(latest):
            for episode in range(latest + 1, show.latest_episode + 1):
