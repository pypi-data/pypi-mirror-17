...
from amino import List, Maybe, _, __, Try, Map
...
class ReleasesFacade(DbFacade):
    ...
    def torrent_cached(self, id):
        def set(l):
            l.cached = True
        self.torrent_by_id(id).foreach(set)
    ...
    @exclusive
    def status(self):
        thresh = now_unix() - self._status_new_interval
        done = List.wrap((
        ...
        caching, search = not_cached.split(_.caching)
        return Map(
            done=done / _.release.info,
