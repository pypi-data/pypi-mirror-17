...
class TorrentCleaner(ReleaseHandler):
    ...
    def _candidates(self):
        down = self._no_cached_torrents_q.filter(
            and_(ReleaseMonitor.downgrade_after > 0,
                 ReleaseMonitor.last_torrent_update_stamp < now_unix() -
                 ReleaseMonitor.downgrade_after)
        ).all()
        caching = (
    ...
    def _qualify_job(self, job):
        ...
        if isinstance(job, Downgrade):
            return ('' not in item.resolutions and not
                    self._alternative_hd_releases(item))
    ...
    def _downgrade(self, monitor):
        ...
        self._update(
            monitor,
            resolutions=monitor.resolutions.cat(''),
