...
from series.condition import LambdaCondition, StrictCondition
...
class TorrentCleaner(ReleaseHandler):
    ...
    def __init__(self, releases, *a, **kw):
        super().__init__(releases, 600, 'torrent cleaner', cooldown=3600, **kw)
    ...
    def _downgrade_conditions(self):
        return (
            ...
            ~HasStandardDef() &
            ~LambdaCondition('alternative hd', self._alternative_hd_releases) &
            (
    ...
    def _alternative_hd_releases(self, monitor):
        return self._search.search(monitor).exists(_.choose.is_just).value
