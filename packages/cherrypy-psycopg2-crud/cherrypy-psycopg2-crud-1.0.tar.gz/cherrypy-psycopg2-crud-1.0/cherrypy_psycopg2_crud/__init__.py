...
#      (or, search) /resource/?col1:like=val1&_limit=12&_offset=1&_sort=-col1,col2
...
class CrudController:
    ...
    def default(*args, **kwargs):
        ...
        elif method == "PUT" and object_id:
            return self.update(object_id, kwargs)
        elif method == "DELETE" and object_id:
    ...
    def search(self, parameters):
        ...
        sql = """
            WITH
                data AS (WITH results AS (SELECT {columns} FROM {table} {where} {sort} {limit} {offset}) SELECT coalesce(json_agg(row_to_json(results)), '[]'::json) AS data FROM results),
                count AS (WITH count AS (SELECT count(*) AS count FROM {table} {where}) SELECT count FROM count)
            SELECT row_to_json(result)::text AS result FROM (data CROSS JOIN count) AS result"""  # noqa: E501

        sql = sql.format(
            columns=columns_sql, table=quote_identifier(self.table),
            where=where_sql, sort=sort_sql, limit=limit_sql, offset=offset_sql)

        # Because the where clause is used twice, need to duplicate the
        # sql parameters that it references
        sql_parameters = sql_parameters * 2

        cursor = cherrypy.request.psycopg2_cursor
        error_handling_sql_execute(cursor, sql, sql_parameters)
...
def error_handling_sql_execute(cursor, operation, parameters=None):
    print("Executing: {!r}".format(operation))
    try:
...
def quote_identifier(unquoted_identifier):
    print("Quoting identifier {}".format(unquoted_identifier))
