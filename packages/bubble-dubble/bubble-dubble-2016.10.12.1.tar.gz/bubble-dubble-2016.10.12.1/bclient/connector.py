...
import cryio
from bubble.bcommon import compressor, default, corrections
...
class Connector(QtCore.QObject):
    ...
    def unpackResponseArrays(self, response):
        ...
        if data1d:
            response['data1d'] = [compressor.decompressNumpyArray(a) for a in data1d]
        if image:
            response['image'] = compressor.decompressNumpyArray(image)
        return response
