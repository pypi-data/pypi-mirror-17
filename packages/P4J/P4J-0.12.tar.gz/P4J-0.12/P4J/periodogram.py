...
class periodogram:
    ...
    def fit(self, t, y, s, subtract_average=True):
        ...
        self.t = t
        if subtract_average:
            self.y = y - np.average(y, weights=1.0/s**2)
        else:
            self.y = y
        self.s = s
        self.T = t[-1] - t[0]
    ...
    def get_confidence(self, per_value):
        ...
        """
        if param is None:
            return ValueError("Fit extreme cdf before asking for confidences")
    ...
    def get_FAP(self, p):
        ...
        if param is None:
            return ValueError("Fit extreme cdf before asking for False Alarm Probabilities")
        mu = self.param[1]
        ss = self.param[2]
        chi = -self.param[0]
