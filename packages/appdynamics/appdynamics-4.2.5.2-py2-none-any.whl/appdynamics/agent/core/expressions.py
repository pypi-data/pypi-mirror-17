...
class ExpressionNodeEvaluationError(Exception): pass
...
def evaluate_regexCaptureOp(node, ctx):
    ...
    return filter(lambda x: x is not None, (match.group(i) for i in regexCaptureOp.regexGroups))
...
# def evaluate_callerOp(node, ctx):
#     """Return the result of calling the method with the supplied parameters.

#     """
#     callerOp = node.callerOp
#     base = _evaluate(callerOp.base, ctx)
#     return getattr(base, callerOp.method)(*(_evaluate(p, ctx) for p in callerOp.params))
...
OPERATORS = {
    ...
    # pb.ExpressionNode.CALLER: evaluate_callerOp,
