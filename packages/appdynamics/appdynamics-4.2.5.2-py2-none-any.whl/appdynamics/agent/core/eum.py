...
from appdynamics.lib import iteritems, Morsel, utf8
...
class EUMCookie(Morsel):
    def __init__(self, name, value, is_secure):
        super(EUMCookie, self).__init__()
        self.set(name, value, value)
        self['path'] = EUM_COOKIE_PATH
    ...
    def output(self):
        return ('Set-Cookie', self.OutputString(None))

    def __str__(self):
        return self.OutputString(None)
...
def delete_cookie(cookie, response_headers):
...
def _generate_eum_metadata(eum_config, bt):
    ...
    if global_account_name:
        metadata[global_account_name_key(use_long_form)] = utf8(global_account_name)
...
def inject_eum_metadata(eum_config, bt, response_headers):
    ...
    if not bt.eum_guid and eum_metadata_injection_enabled(eum_config, bt.request.user_agent):
        ...
        if bt.request.is_ajax:
            ...
            for i, data in enumerate(iteritems(metadata)):
                header = (EUM_HEADER_NAME.format(i), EUM_HEADER_SEPERATOR.join(map(str, data)))
...
def delete_stale_eum_cookies(cookies, response_headers, is_secure):
    ...
    for name, value in iteritems(cookies):
        if match_eum_cookie_name(name):
            cookie = EUMCookie(name, value, is_secure)
            logger.debug('Deleting stale EUM cookie: %s' % cookie)
            delete_cookie(cookie, response_headers)
