...
def make_snapshot_dict(bt):
    ...
    snapshot = {
        ...
        'processID': os.getpid(),
        'exitCalls': filter_redundant_exit_calls(itervalues(bt._exit_calls)),
    }
...
def filter_redundant_exit_calls(exit_calls_list):
    ...
    Following method ensures that the exit call information is NOT added redundantly to the BT details.

    It adds the exit call information to the dict iff.

    1. If the exit call is already not added to the call graph.
    (or)
    2. the exit call is of type EXIT_DB; since proxy expects snpshot info to have DB exit calls and treats them specially
