...
from appdynamics.lib import normalize_header, u2b, items

from appdynamics.agent import pb
from appdynamics.agent.core import conditions, expressions
...
class BackendRegistry(object):
    ...
    def backend_hash(self, exit_call_type, exit_call_subtype, properties):
    ...
    def match_backend(self, backend_properties, match_rule):
        ...
        for key in backend_properties:
            ...
            if match_rule.HasField(field_name):
                if not conditions.match(backend_properties[key], getattr(match_rule, field_name)):
                    return False
        return True
    ...
    def match(self, entry_type, request, base_name=None):
        ...
        for rule in entry_point_config['custom_definitions']:
            if conditions.match_http(request, rule['condition']):
                ...
                custom_bt_name = transactions.get_http_bt_name(custom_naming_scheme, request,
                                                               rule['name'], custom=True)
