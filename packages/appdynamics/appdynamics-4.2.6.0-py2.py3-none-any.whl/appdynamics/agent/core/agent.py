...
from appdynamics.lang import str, values
...
class Agent(object):
    ...
    def start_exit_call(self, bt, caller, backend, **kwargs):
        """Start an exit call, forcing a sample if the BT is snapshotting.

        """
        exit_call = bt.start_exit_call(caller, backend, **kwargs)
        if exit_call and bt.snapshotting:
            bt.sample_data.take_leaf(caller, exit_call.timer.start_time_ms, bt.active_exit_calls)
        return exit_call

    def end_exit_call(self, bt, exit_call, exc_info=None):
        """End an exit call.

        """
        bt.end_exit_call(exit_call, exc_info=exc_info)

    def end_transaction(self, bt=None):
