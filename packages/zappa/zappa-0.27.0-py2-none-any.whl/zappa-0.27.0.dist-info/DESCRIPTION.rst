...
Inside your app, the authenticated username will be available through
the ``REMOTE_USER`` environment variable (e.g. in Flask
``request.environ.get('REMOTE_USER')``) If you are wondering for what
you would use an Authorizer, here are some potential use cases: 1. Call
out to OAuth provider 2. Decode a JWT token inline 3. Lookup in a
self-managed DB (for example DynamoDB)
