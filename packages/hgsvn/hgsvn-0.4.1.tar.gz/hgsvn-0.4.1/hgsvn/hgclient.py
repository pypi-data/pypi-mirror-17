...
def run_hg(args=None, bulk_args=None, output_is_locale_encoding=False, prompt = None):
    ...
    def decode_output(output, enc):
        ...
        try:
            outlines = unicode(output, encoding=enc, errors = 'strict').splitlines(True) #'utf-8'
        except:
            ...
            if tuple(e.args) == pass_args:
                ...
                if args[0]=='heads':
                    ...
                    try:
                        ...
                        if e.args == pass_args:
                            raise RunCommandError("External program sure failed", e.ret, e.args, e.err, e.out)
                        else:
                            raise RunCommandError("External program failed on strange args", e.ret, e.args, e.err, e.out)
                else:
