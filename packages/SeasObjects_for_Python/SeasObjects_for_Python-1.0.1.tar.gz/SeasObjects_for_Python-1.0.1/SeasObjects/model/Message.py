...
class Message(Evaluation):
	...
	def __init__(self, uri = None):
		...
		self.method = None
		self.errors = []
		self.setType(CLASS.SEAS_MESSAGE)
		
	def serialize(self, model):
		resource = super(Message, self).serialize(model)

		# errors
		errorProp = model.createProperty( PROPERTY.SEAS_ERROR )
		...
		# status
		if self.hasStatus():
			resource.addProperty(model.createProperty( PROPERTY.SEAS_STATUS ), self.getStatus().serialize(model))

		if self.hasMessageId():
			resource.addProperty(model.createProperty( PROPERTY.SEAS_MESSAGEID ), model.createLiteral(self.getMessageId()))

		if self.hasProcessId():
			resource.addProperty(model.createProperty( PROPERTY.SEAS_PROCESSID ), model.createLiteral(self.getProcessId()))

		if self.hasMethod():
			resource.addProperty(model.createProperty( PROPERTY.SEAS_METHOD ), self.getMethod())
	...
	def parse(self, resource):
		...
		if isinstance(resource, Resource):
			if resource == None:
				raise NullPointerException("Unable to parse null to Response.")
	
			if not resource.isAnon():
				msg = Message(resource.toString())
			else:
				msg = Message()
			msg.clearTypes()
			...
			# status
			if predicate == PROPERTY.SEAS_MESSAGEID:
				try:
			...
			# status
			if predicate == PROPERTY.SEAS_PROCESSID:
				try:
			...
			# status
			if predicate == PROPERTY.SEAS_STATUS:
				try:
			...
			# status
			if predicate == PROPERTY.SEAS_METHOD:
				try:
			...
			super(Message, self).parse(statement)
