...
class Condition(Obj):
	...
	def __init__(self, uri = None):
		...
		self.ors = []
		self.ands = []
		self.setType(CLASS.SEAS_CONDITION)
	...
	def parse(self, resource):
		if isinstance(resource, Resource):
			...
			if predicate.equals(PROPERTY.SEAS_OR):
				...
				# controllability
				if types.contains(CLASS.SEAS_CONTROLLABILITY):
					try:
				...
				# availability
				if types.contains(CLASS.SEAS_AVAILABILITY):
					try:
				...
				# condition
				if types.contains(CLASS.SEAS_CONDITION):
					try:
