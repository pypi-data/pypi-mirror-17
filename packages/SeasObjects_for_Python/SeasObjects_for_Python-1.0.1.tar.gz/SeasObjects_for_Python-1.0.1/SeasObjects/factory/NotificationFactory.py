...
class NotificationFactory(object):
	...
	def create(self, generatedBy):
		...
		notification.addType(CLASS.SEAS_NOTIFICATION)

		return notification

	def fromString(self, data, serialization):
		try:
			return Notification().parse(Tools().getResourceByType(CLASS.SEAS_NOTIFICATION, Tools().fromString(data, serialization)))
		except:
