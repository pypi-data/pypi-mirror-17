...
class RequestFactory(object):
	...
	def create(self, generatedBy):
		...
		evaluation.addType(CLASS.SEAS_REQUEST)
	...
	def fromString(self, data, serialization):
		try:
			return Request().parse(Tools().getResourceByType(CLASS.SEAS_REQUEST, Tools().fromString(data, serialization)))
		except:
	...
	def createRegistrationRequest(self, generatedBy):
		...
		req.addType(CLASS.SEAS_REQUEST)
	...
	def createWeatherForecastTimeserie(self, generatedBy):
		...
		evaluation.addType(CLASS.SEAS_EVALUATION)
		evaluation.addCategory(CLASS.SEAS_FORECAST)
		systemOfInterest = SystemOfInterest()
		systemOfInterest.setType(CLASS.SEAS_WEATHER)
