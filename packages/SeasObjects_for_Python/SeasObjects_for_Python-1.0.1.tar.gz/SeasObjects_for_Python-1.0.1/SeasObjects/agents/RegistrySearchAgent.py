...
class RegistrySearchAgent(Agent):
    ...
    def generateSearchMessage(self):
        ...
        request.setMethod(CLASS.SEAS_READ)
    ...
    def polygonSearchArea(self, polygon):
        global entity
        entity.add(URIRef(CLASS.SEAS_POLYGON), polygon)
    ...
    def pointSearchArea(self, center, kilometers):
        ...
        maxR = ValueObject()
        maxR.setQuantity(CLASS.SEAS_DISTANCE)
        maxR.setUnit(CLASS.SEAS_KILOMETER)
    ...
    def searchServicesByOutputType(self, mySeasUri, type):
        ...
        agent.clear()
        agent.setSenderUri(mySeasUri)
        agent.ofType(CLASS.SEAS_SERVICE)
