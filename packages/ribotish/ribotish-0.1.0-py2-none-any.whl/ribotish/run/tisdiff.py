...
def set_parser(parser):
  #### basic input options ####
  parser.add_argument("-1", type=str, dest="tis1path", required=True, help="Findorf result of group 1 TIS data")
  parser.add_argument("-2", type=str, dest="tis2path", required=True, help="Findorf result of group 2 TIS data")
...
def run(args):
  ...
  '''
  global pth, qth, tis1bampaths, tis2bampaths, tis1offdict, tis2offdict
...
def _get_tis(ps) : 
  ...
  r1, r2 = [], []
  if g1t is not None :
    t1tis = ribo.multiRibo(t, tis1bampaths, offdict = tis1offdict)
    #ttis = ribo.Ribo(merge, tisbam1file, offdict = tis1offdict, compatible = False)
    for i in g1t : r1.append((i, t1tis.cnts[i]))
  if g2t is not None :
    t2tis = ribo.multiRibo(t, tis2bampaths, offdict = tis2offdict)
