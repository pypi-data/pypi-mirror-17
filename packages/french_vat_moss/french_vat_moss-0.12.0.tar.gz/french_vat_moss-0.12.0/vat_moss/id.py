...
def validate(vat_id):
    ...
    # EU countries
        ...
        if valid_elements[0].text.lower() != 'true':
            raise InvalidError('VAT ID is invalid %s' % vat_id)

        company_name = name_elements[0].text
        address = address_elements[0].text
        logger.info('validate vat vat=%s country=%s, company_name=%s address=%s', vat_id, ID_PATTERNS[country_prefix]['country_code'], company_name, address.replace("\n", " "))
    return ID_PATTERNS[country_prefix]['country_code'], vat_id, company_name, address
