...
def search_logs(name, filter_pattern):
  ...
  for event in events:
    ...
    print '\t'.join((human_date.strftime('%Y-%m-%d %H:%M:%S'), event.get('logStreamName'), message))
...
def dump_stream(name, stream_id):
  client = boto3.client('logs')
  log_name = '/aws/lambda/' + name
  logs = client.get_log_events(logGroupName=log_name,
                               logStreamName=stream_id,
...
def main():
  ...
  elif configuration is not False and argument in ('stream'):
    stream_id = sys.argv[2]
    dump_stream(configuration['name'], stream_id)
...
def help():
  print '''Available command line arguments 
              ...
              clamda stream "group" - spit out an entire loggroup
