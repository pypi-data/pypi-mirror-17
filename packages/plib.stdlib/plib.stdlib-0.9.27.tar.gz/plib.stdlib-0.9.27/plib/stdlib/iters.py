...
INTERLEAVE_DEFAULT = object()


def interleave(iterables, default=INTERLEAVE_DEFAULT,
               chain=chain, izip_longest=izip_longest):
    
    # Return elements in round-robin alternation from iterables
    return tuple(item for item in chain(*izip_longest(*iterables, fillvalue=default)) if item is not default)
