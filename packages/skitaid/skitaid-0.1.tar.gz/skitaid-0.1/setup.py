...
if "install" in sys.argv or "develop" in sys.argv:
	if os.name == "nt":
		...
		if not os.path.isfile (conf):
			shutil.copyfile ("skitaid\\etc\\skitaid\\skitaid.conf", conf)
		...
		for path in pathes:
			...
			try: 
				...
				if why.errno == 2: pass
			shutil.copyfile (os.path.join ("skitaid\\etc\\skitaid", path), target)
		
		for fn in os.listdir ("skitaid\\bin"):
			if fn == "__pycache__": 
				continue
			...
			try: os.remove (target)
				...
				if why.errno == 2: pass	
			shutil.copyfile (os.path.join ("skitaid\\bin", fn), target)
		...
		for fn in ("webapp.py", "static\\reindeer.jpg", "templates\\documentation.html", "templates\\index.html", "templates\\websocket.html"):
			...
			try: os.remove (target)
				...
				if why.errno == 2: pass	
			shutil.copyfile (os.path.join ("skitaid\\pub\\sample", fn), target)
		...
		if not os.path.isfile (conf) and not os.path.islink (conf):
			shutil.copyfile (os.path.join ("skitaid/etc/skitaid/skitaid.conf"), "/etc/skitaid/skitaid.conf")
		
		with open ("skitaid/etc/skitaid/servers-enabled/sample.conf") as f:
		...
		for path in ("cert/README.txt", "servers-available/README.txt"):
			try: 
				...
				if why.errno == 2: pass
			shutil.copyfile (os.path.join ("skitaid/etc/skitaid", path), os.path.join ("/etc/skitaid", path))
		...
		for fn in ("skitaid.py", "skitaid-instance.py", "skitaid-smtpda.py", "skitaid-cron.py"):
			...
			with open (os.path.join ("skitaid/bin", fn), "rb") as f:
				data = f.read ()				
		...
		shutil.copyfile ("skitaid/etc/init.d/skitaid", "/etc/init.d/skitaid")
