...
def configPrintRedact(enable):
    global redact_logs
    redact_logs = enable

def configPrint(enable_verbose_exceptions = False, enable_redact_values = False):
    configPrintException(enable_verbose_exceptions)
    configPrintRedact(enable_redact_values)
...
def printGeneric(out, msg, newLine = True):
    global redact_logs
    if False: # redact_logs:
        prefixes = [ 'acl-', 'elb-', 'i-', 'sg-', 'nacl-', 'vpc-' ] 
        for prefix in prefixes:
            msg = re.sub(r'%s\w+?' % prefix, '%s-????????' % prefix, msg)
    out.write(msg)
    if newLine == True:
        out.write('\n')
    out.flush()
