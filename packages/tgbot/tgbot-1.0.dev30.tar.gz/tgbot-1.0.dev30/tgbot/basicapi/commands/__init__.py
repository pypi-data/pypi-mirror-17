...
def send_inline_keyboard(message,chat_id,text,inline_keyboard,**kwargs):
    values = {"chat_id": chat_id, "text": text}
    for key, value in kwargs.items():
        values[key] = value
    return sendreply_markup(values,inline_keyboard=inline_keyboard)
...
def getChat(chat_id):
    return sendrequest("getChat",chat_id=chat_id)


def editmessageText(text,**kwargs):
    return sendrequest("editMessageText",text=text,**kwargs)

def answerCallBackQuery(callback_id,text,show_alert):
    return sendrequest("answerCallbackQuery",callback_query_id=callback_id,text=text,show_alert=show_alert)
