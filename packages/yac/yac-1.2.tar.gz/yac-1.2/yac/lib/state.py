...
def load_state(s3_path, service_alias):
    ...
    s3_bucket, s3_path = find_bucket_with_state_in_cache(s3_path, service_alias)
    
    if not s3_bucket:    
        
        # find bucket based on bucket name and presence of state data in bucket
        s3_bucket, s3_path = find_bucket_with_state_by_name(s3_path, service_alias)
...
def find_bucket_with_state_in_cache(s3_path, service_alias):

    s3_bucket = ""
    this_s3_path = s3_path

    s3_bucket_candidate = find_bucket_in_cache(service_alias)

    if s3_bucket_candidate:

        this_s3_path = get_s3_full_path(s3_path, service_alias)
...
def find_bucket_with_state_by_name(s3_path, service_alias):
