...
class AttachedStorageDomainService(Service):
    ...
    def activate(
        ...
        buf = io.BytesIO()
        writer = xml.XmlWriter(buf, indent=True)
        writers.ActionWriter.write_one(action, writer)
        writer.flush()
        body = buf.getvalue()
        writer.close()
        buf.close()
    ...
    def get(
        ...
        if response.code in [200]:
            ...
            try:
                buf = io.BytesIO(response.body)
                reader = xml.XmlReader(buf)
                return readers.ImageTransferReader.read_one(reader)
            finally:
                if buf is not None:
                    buf.close()
                if reader is not None:
...
class ImageTransfersService(Service):
    ...
    def add(
        ...
        try:
            buf = io.BytesIO()
            writer = xml.XmlWriter(buf, indent=True)
            writers.ImageTransferWriter.write_one(image_transfer, writer)
            writer.flush()
            request.body = buf.getvalue()
        finally:
            if writer is not None:
    ...
    def list(
        ...
        if response.code in [200]:
            ...
            try:
                buf = io.BytesIO(response.body)
                reader = xml.XmlReader(buf)
                return readers.ImageTransferReader.read_many(reader)
            finally:
                if buf is not None:
                    buf.close()
                if reader is not None:
...
class TemplateService(Service):
    ...
    def export(
        ...
        # Check the types of the parameters:
        Service._check_types([
            ('exclussive', exclussive, bool),
            ('storage_domain', storage_domain, types.StorageDomain),
        ])

        # Populate the action:
        action = types.Action(
            exclussive=exclussive,
            storage_domain=storage_domain,
        )
...
class HostService(MeasurableService):
    ...
    def upgrade_check(
        ...
        request = http.Request(
            method='POST',
            path='%s/%s' % (self._path, 'upgradecheck'),
            body=body,
        )
        response = self._connection.send(request)
        self._check_action(response)
