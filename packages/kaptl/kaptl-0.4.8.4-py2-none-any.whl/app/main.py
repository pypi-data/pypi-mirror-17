...
"""Usage: kaptl init <name> <rules> | --rules-file=RULESFILE --license=KEY [--backend=mvc] [--frontend=angular] [--build] [--recipe=RECIPENAME]
       kaptl parse <rules> --license=KEY [--recipe=RECIPENAME]
...
def checkLicense():
    args = docopt(__doc__)
    if not args["--license"]:
        print "Use of KAPTL requires a license key. Obtain a valid key at www.kaptl.com and use --license to provide your key."
        sys.exit()
...
def main():
    ...
    if args["parse"]:
        checkLicense()
        Common.parse_rules(session,args["<rules>"],Utils.get_stack_info(args),None,args["--license"],args["--recipe"])
