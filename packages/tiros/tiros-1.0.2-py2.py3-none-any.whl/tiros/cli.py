...
lambda_parser.set_defaults(func=lambda_)
...
lambda_parser.add_argument(
    '--sns-topic',
    '-t',
    help='The SNS topic to send messages to when the Lambda completes'
)

lambda_parser.add_argument(
    '--sns-subject',
    '-s',
    help='The SNS subject line'
)
...
lambda_parser.add_argument(
    ...
    '-p',
    help="""
    The S3 prefix where Tiros results will be written.  For example, if
    -p s3://acme/tiros is specified, each time Tiros runs it will
    create a file like s3://acme/tiros/results-2016-01-01-10:01:02.json
    """
)
