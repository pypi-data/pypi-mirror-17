...
class WalletedAgent(Agent):
    ...
    def _fetchAllAvailableClaimsInWallet(self, li, availableClaims):
        ...
        def postFetchCredDef(reply, err):
            self.notifyMsgListener("reply for get cred def: {}".format(reply))
    ...
    def _handleAcceptInviteResponse(self, msg):
        ...
        if isVerified:
            ...
            if li:
                ...
                if availableClaims:
                    self.notifyMsgListener("link available claims before: {}".
                                           format(len(li.availableClaims)))
                    li.availableClaims.extend(availableClaims)
                    self.notifyMsgListener(
                        "link available claims after: {}".format(
                            len(li.availableClaims)))
    ...
    def _handleReqClaimResponse(self, msg):
        ...
        if isVerified:
            ...
            if li:
                ...
                self.wallet.addCredentialToProofBuilder((data[NAME],
                                                         data[VERSION],
