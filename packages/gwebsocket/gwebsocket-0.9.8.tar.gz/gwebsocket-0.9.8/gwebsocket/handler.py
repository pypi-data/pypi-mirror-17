...
from .exceptions import SocketDeadError
...
class WebSocketHandler(WSGIHandler):
    ...
    http_log_format = '{ip} {method} {path} {status} {length} {time} {user_agent} {referer}'
    websocket_log_format = '{ip} WebSocket {event} {path} {time} {user_agent} {referer}'

    def get_log_info(self, **kwargs):
        ...
        # If present, user agent should have quotes around it.
        user_agent = self.environ.get('HTTP_USER_AGENT')
        user_agent = '"%s"' % user_agent if user_agent else '-'
    ...
    # Override the log formatter function from pywsgi to remove redundant
    # timestamp (already included by gunicorn's logger)
    def format_request(self):
        return self.http_log_format.format(**self.get_log_info())

    def format_websocket_request(self, event):
        return self.websocket_log_format.format(**self.get_log_info(event=event))

    def log_websocket_start(self):
        self.logger.info(self.format_websocket_request('start'))

    def log_websocket_end(self):
        self.logger.info(self.format_websocket_request('end'))

    @property
    def logger(self):
        # Prefer the Gunicorn logger, if it's there
    ...
    def run_websocket(self):
        ...
        try:
            ...
            self.time_finish = 0
            self.log_websocket_start()
            self.application(self.environ, lambda s, h, e=None: [])
        except SocketDeadError:
            # WebSocket disconnects happen all the time.  By the time they've
            # bubbled up to this level, the application has already had a chance
            # to catch them and do its own cleanup.  Trap them here so they
            # don't clutter the logs excessively.
            pass
        finally:
            self.time_finish = time.time()
            self.log_websocket_end()
