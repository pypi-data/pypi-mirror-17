...
class DnBsl(bsl.SerialBSLTarget, object):
    ...
    def __init__(self):
        ...
        self.options.persist_memory = False
...
#  TODO: check and remove if unnecessary
    ...
    def add_program(self, args):
        ...
        if args.persist_memory:
            self.options.persist_memory = True
            self.add_action(self.read_int_memory)
    ...
    def open_connection(self):
        ...
        if self.options.persist_memory:
            def rm():
                self.password(DEFAULT_PASSWORD)
                self.int_mem = repr(self.memory_read(0x1000, 100))
                print repr(self.int_mem)
            self.read_int_memory()
...
def main(argv=None):
    ...
    program_p.add_argument('-ne', '--no-erase', action='store_true',
                           ...
                           ' programming')
    program_p.add_argument('-pm', '--persist-memory', action='store_true',
                           help='persist the internal flash memory')
