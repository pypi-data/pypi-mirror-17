...
def dispatch_groups(request):
    ...
    for group in groups:
        hidash_group = {"group_name": str(group.name), "charts": []}
        charts = group.chart.all()
        for chart in charts:
            hidash_group['charts'].append(str(chart.name))
        hidash_groups.append(hidash_group)
    return render(request, 'hidash-groups.html',
                  {'hidash_groups': hidash_groups})
...
def _parse_charts():
    ...
    for chart in list(charts):
        ...
        for group in chart_groups:
            group_charts = group.chart.all()
            for group_chart in group_charts:
                if group_chart.id == chart.id:
