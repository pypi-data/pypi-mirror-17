...
def dispatch_group_reports(request):
    if hasattr(settings, 'HIDASH_SETTINGS') and 'dashboard_auth' in settings.HIDASH_SETTINGS:
        if settings.HIDASH_SETTINGS['dashboard_auth'](request):
...
def dispatch_chart_configurations(request):
    return HttpResponse(content=json.dumps(_get_chart_congiguration(request)),
                        content_type="application/json")


def dispatch_group_filters_as_json(request):
    return HttpResponse(content=json.dumps(_get_group_filters(request)),
                 content_type="application/json")
...
def get_email_data_and_recipients(report_id):
  if report_id:
      scheduled_report_id = report_id.split(',')
      scheduled_reports = ScheduledReport.objects.prefetch_related('report').filter(id__in=scheduled_report_id)
  else:
