...
def get_email_reports(reports):
    ...
    for report in reports_to_be_emailed.report.all():
        ...
        query, bind_param = _sql_query_param_interpreter(report.query, scheduled_report_param)
        cursor.execute(query, bind_param)
...
def get_email_data_and_recipients(request):
    cur_dt = datetime.now()
    scheduled_reports = ScheduledReport.objects.prefetch_related('report').filter(next_run_at__lt = cur_dt)
    all_report_params = ScheduledReportParam.objects.all()
    ...
    for report in scheduled_reports:
        ...
        report_params = all_report_params.filter(scheduled_report_id = report.id)
        for params in report_params:
            response[report.id][params.parameter_name] = params.parameter_value
        email_to = ''
...
def _sql_query_param_interpreter(query, report_param):
    bind_params = {}
    query_params = {}
    for param in report_param:
        if param.is_parameter_value_sql_function:
            parameter = param.parameter_value.upper()
            if parameter == 'NOW()' or re.match(r'^NOW\(\)\s(\+|-)\sINTERVAL\s\d+\s\D*[^\s]+$', parameter):
                query_params[param.parameter_name] = parameter
            else:
                query_params[param.parameter_name] = '%(' + param.parameter_name + ')s'
                curr_date = datetime.now().date()
                bind_params[param.parameter_name] = curr_date
        else:
             query_params[param.parameter_name] = '%('+ param.parameter_name +')s'
