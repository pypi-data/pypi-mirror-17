...
class ILoadBalancer(object):
    ...
    def pick(self):
        ...
        if not self._up:
            logger.exception("self._up list with endpoints is empty: NoAvailableEndpointsLbException")
        ...
        logger.exception("Tried all the endpoints, nothing is available: NoAvailableEndpointsLbException")
        raise NoAvailableEndpointsLbException('No endpoints in closed state. Nothing to choose.')
