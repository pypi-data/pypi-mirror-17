...
APPLICATION_BABEL_DIR = "%s/babel" % APPLICATION_DIR
...
def get_mambo_config(cwd, key):
    with open("%s/%s" % (cwd, "propel.yml")) as f:
        config = yaml.load(f)

    if key not in config:
        raise Exception("Missing '%s' in mambo.yml" % key)
    return config[key]
...
def get_push_remotes_list(cwd, key=None):
    ...
    :return: list
    """
    config = get_mambo_config(cwd, "push")
...
def syncdb():
    ...
    if serve.app.db and serve.app.db.Model:
        ...
        for m in serve.app.db.Model.__subclasses__():
            if hasattr(m, "_sync_model"):
                print("Sync up model: %s ..." % m.__name__)
                getattr(m, "_sync_model")()
...
def assets2s3(project):
    ...
    projects = get_mambo_config(CWD, "assets2s3")
