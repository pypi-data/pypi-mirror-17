...
# ie, set new model property -> model.MyNewModel = MyModel
# ie: use property -> model.MyNewModel.all()
...
def register_models(**kwargs):
    ...
    :return:
    """
    [setattr(model, k, v) for k, v in kwargs.items()]
...
class Mambo(object):
    ...
    def setup_installed_apps(cls):
        ...
        if "INSTALLED_APPS" in cls._app.config:
            ...
            for k in cls._app.config.get("INSTALLED_APPS"):
                ...
                _kwargs = {
                    ...
                    "menu_nav": prop.get("menu_nav") or {"title": None},
                    ...
                    if isinstance(modules, list):
                        ...
                        for n, kw in modules.items():
                            kw2 = {
                                ...
                                "menu_nav": kw.get("menu_nav") or {"title": None},
    ...
    def _expose_models(cls):
        if cls._app.db:
            register_models(**{m.__name__:m
                               ...
                               if not hasattr(model, m.__name__)})
