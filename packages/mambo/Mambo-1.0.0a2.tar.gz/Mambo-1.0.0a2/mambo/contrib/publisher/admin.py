...
def main(**kwargs):
    ...
    class PublisherAdmin(Mambo):
        ...
        def index(self):
            ...
            posts = model.PublisherPost.query()
            types = model.PublisherType.query().order_by("name asc")
            types_options = [(t.id, t.name) for t in types]

            selected_type = None
            if type_id:
                selected_type = model.PublisherType.get(type_id)

            if id:
                posts = posts.filter(model.PublisherPost.id == id)
            if slug:
                posts = posts.filter(model.PublisherPost.slug == slug)
            if type_id:
                type_id = int(type_id)
                posts = posts.filter(model.PublisherPost.type_id == type_id)
            if category_id:
                posts = posts.join(model.PublisherCategoryMap) \
                    .join(model.PublisherCategory) \
                    .filter(model.PublisherCategory.id == category_id)
            if tag_id:
                posts = posts.join(model.PublisherTagMap) \
                    .join(model.PublisherTag) \
                    .filter(model.PublisherTag.id == tag_id)
            if status == "publish":
                posts = posts.filter(model.PublisherPost.is_published == True)
            elif status == "draft":
                posts = posts.filter(model.PublisherPost.is_draft == True)
            elif status == "revision":
                posts = posts.filter(model.PublisherPost.is_revision == True)

            posts = posts.order_by(model.PublisherPost.id.desc())
        ...
        def preview(self, id):
            ...
            Read Post
            """
            post = model.PublisherPost.get(id)
            if not post:
        ...
        def edit(self, id):
            ...
            types = [(t.id, t.name) for t in
                     model.PublisherType.query().order_by(model.PublisherType.name.asc())]
            categories = [(c.id, c.name) for c in
                          model.PublisherCategory.query().order_by(
                              model.PublisherCategory.name.asc())]
            checked_cats = []
            ...
            images = model.PublisherAssets.query() \
                .filter(model.PublisherAssets.type == "IMAGE") \
                .order_by(model.PublisherAssets.name.asc())
        ...
        def save_post(self):
            ...
            if id and status in ["delete", "revision"]:
                ...
                if status == "delete":
                    ...
                    post = model.PublisherPost.create(**data)
                ...
                for tag in tags:
                    tag = tag.strip().lower()
                    _tag = model.PublisherTag.get_by_slug(name=tag)
                    if tag and not _tag:
                        _tag = model.PublisherTag.new(name=tag)
                    if _tag:
        ...
        def categories(self):
            ...
            if request.method == "POST":
                ...
                try:
                    if not id:
                        cat = model.PublisherCategory.new(name=name, slug=slug)
                        if ajax:
                            return jsonify({
                                "id": cat.id,
                                "name": cat.name,
                        ...
                        post_cat = model.PublisherCategory.get(id)
                        if post_cat:
                            if action == "delete":
                                post_cat.delete()
                ...
                cats = model.PublisherCategory.query().order_by(
                    model.PublisherCategory.name.asc())
        ...
        def tags(self):
            ...
            if request.method == "POST":
                ...
                try:
                    if not id:
                        tag = model.PublisherTag.new(name=name, slug=slug)
                        if ajax:
                            return jsonify({
                                "id": tag.id,
                                "name": tag.name,
                        ...
                        post_tag = model.PublisherTag.get(id)
                        if post_tag:
                            if action == "delete":
                                post_tag.delete()
                ...
                tags = model.PublisherTag.query().order_by(model.PublisherTag.name.asc())
        ...
        def types(self):
            ...
            if request.method == "POST":
                try:
                    ...
                    slug = request.form.get("slug", None)
                    if not id:
                        model.PublisherType.new(name=name, slug=slug)
                        flash_success("New type '%s' added" % name)
                    else:
                        post_type = model.PublisherType.get(id)
                        if post_type:
                            if action == "delete":
                                post_type.delete()
                ...
                types = model.PublisherType.query().order_by(model.PublisherType.name.asc())
        ...
        def images(self):
            ...
            if request.method == "POST":
                ...
                description = request.form.get("description")
                if id:
                    image = model.PublisherAssets.get(id)
                    if image:
                        if action == "delete":
                            image.delete()
            ...
            try:
                ...
                if request.files.get("file"):
                    ...
                    if obj:
                        ...
                        upload_object = model.PublisherAssets.create(name=obj.name,
