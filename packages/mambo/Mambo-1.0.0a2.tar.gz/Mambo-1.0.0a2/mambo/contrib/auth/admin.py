...
def main(**kwargs):
    ...
    class AuthAdmin(Mambo):
        ...
        def _user_roles_options(cls):
            _r = model.AuthRole.query()\
                .filter(model.AuthRole.level <= current_user.role.level)\
                .order_by(model.AuthRole.level.desc())
        ...
        def index(self):
            ...
            users = model.AuthUser.query(include_deleted=include_deleted)
            users = users.join(model.AuthRole).filter(model.AuthRole.level <= current_user.role.level)

            if name:
                users = model.AuthUser.search_by_name(users, name)
            if email:
                users = users.join(model.AuthUserLogin).filter(model.AuthUserLogin.email.contains(email))
            if role:
                users = users.filter(model.AuthUser.role_id == int(role))
            if sorting and "__" in sorting:
                col, dir = sorting.split("__", 2)
                if dir == "asc":
                    users = users.order_by(getattr(model.AuthUser, col).asc())
                else:
                    users = users.order_by(getattr(model.AuthUser, col).desc())
        ...
        def info(self, id):
            ...
            user = model.AuthUser.get(id, include_deleted=True)
            if not user:
        ...
        def save(self):
            ...
            try:
                ...
                if action == "info":
                    ...
                    if dob:
                        data["date_of_birth"] = arrow.get(dob, model.AuthUser.DOB_FORMAT).datetime

                    if current_user.id != user.id:
                        user_role = request.form.get("user_role")
                        _role = model.AuthRole.get(user_role)
                        if not _role:
                    ...
                    user_login = user.get_email_login()
                    if not user_login:
        ...
        def create(self):
            try:
                ...
                new_user = model.AuthUserLogin.new(login_type=model.AuthUserLogin.TYPE_EMAIL,
                                                   email=email,
                                                   user_info=data)
                if new_user:
        ...
        def roles(self):
            ...
            if request.method == "POST":
                try:
                    ...
                    if name and level:
                        level = int(level)
                        name = name.upper()
                        _levels = [r[0] for r in model.AuthRole.ROLES]
                        _names = [r[1] for r in model.AuthRole.ROLES]
                        if level in _levels or name in _names:
                            raise AppError("Can't modify PRIMARY Roles - name: %s, level: %s " % (name, level))
                        else:
                            if id:
                                role = model.AuthRole.get(id)
                                if role:
                                    if action == "delete":
                                        role.delete()
                                        flash_success("Role '%s' deleted successfully!" % role.name)
                                    elif action == "update":
                                        if role.level != level and model.AuthRole.get_by_level(level):
                                            raise AppError("Role Level '%s' exists already" % level)
                                        elif role.name != name and model.AuthRole.get_by_name(name):
                                ...
                                if model.AuthRole.get_by_level(level):
                                    raise AppError("Role Level '%s' exists already" % level)
                                elif model.AuthRole.get_by_name(name):
                                    raise AppError( "Role Name '%s'  exists already" % name)
                                else:
                                    model.AuthRole.new(name=name, level=level)
            ...
            roles = model.AuthRole.query().order_by(model.AuthRole.level.desc())
        ...
        def reset_password(self):
            ...
            try:
                ...
                if not user_login:
                    raise AppError("This account doesn't have an email login type")
                send_mail_password_reset(user_login)
            except AppError as ae:
                flash_error(ae.message)
