...
Layouts can be loaded with the `tecplot.load_layout()` method. This will accept both ``lay`` and ``lpk`` (packaged) files. The exported image interprets the image type by the extension you give it. See `tecplot.export_image()` for more details.
