...
class BaseForm(Form):
    ...
    - By default CSRF protection is implement by forms provided a secure key
      is defined against the applications config as `CRSF_SECRET_KEY`. To
      disable this behaviour set the meta class attribute `csrf` to `False`.
    """
...
class CSRF:
    ...
    def init_app(cls, app):
        ...
        @app.before_request
        def _csrf_protect():
            # If the request method is protect bail
    ...
    def generate_token(self):
        ...
        return BaseForm().csrf_token.current_token

    @classmethod
    def validate_token(self):
        """Validate a CSRF token"""
        return BaseForm(flask.request.values).validate()
