...
def features_change(runner, params):
    ...
    # Publish metrics delay only if we have a new value for it > 0
    publish_metrics_delay = params.get('publish_metrics_delay', 0)
    if publish_metrics_delay > 0:
        runner.set_publish_metrics_delay(publish_metrics_delay)
