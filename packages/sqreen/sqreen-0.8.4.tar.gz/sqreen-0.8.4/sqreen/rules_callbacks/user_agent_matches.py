...
ERROR_STRING = "<html><head><title>Internal Server Error</title></head><body><h1><p>Internal Server Error</p></h1></body></html>"
...
class UserAgentMatchesCB(RegexpRule):
    ...
    def pre(self, original, *args, **kwargs):
        ...
        server_is_gunicorn = environ.get('SERVER_SOFTWARE', '').startswith('gunicorn')
        is_gevent_loaded = 'gevent' in sys.modules
        if server_is_gunicorn and is_gevent_loaded and self.should_block():
            # Log the error
