...
class MetricsStore(object):
    ...
    def register_metric(self, name, kind, period):
        ...
        if name in self.metrics:
            saved_metric_kind = self.metrics[name]['kind']
    ...
    def _new_metric(self, kind, period):
    ...
    def update(self, name, at, key, value):
        ...
        elif metric["start"] + timedelta(seconds=metric["period"]) < at:
            metric = self.finalize_period(metric, name, at)
