...
class FbObject(metaclass=ABCMeta):
    ...
    # @staticmethod
    # def log_warning_if_greater_than_limit(value, limit: int, value_name: str):
    #     if value_name and not isinstance(value_name, str):
    #         raise TypeError('value_description must be instance of string.')
    #     length = len(value)
    #     if length > limit:
    #         warn_message = 'Limit is {limit} - {value_name} length is {length}: {value}'.format(limit=limit,
    #                                                                                             length=length,
    #                                                                                             value_name=value_name,
    #                                                                                             value=value[:100])
...
class QuickReply(FbObject):
    ...
    def __init__(self, title: str, payload: str, title_limit=20, payload_limit=1000):
    ...
    def throw_if_not_valid(self):
    #     self.throw_if_greater_than_limit(value=self._title, limit=self._title_limit, value_name='title')
...
class Button(FbObject):
    ...
    _types = [TYPE_WEB_URL, TYPE_POSTBACK]

    def __init__(self, type: str, title, url=None, payload=None, title_limit=20, payload_limit=1000):
        self._type = type
        self._title = title
        self._url = url
        self._payload = payload
...
class Element(FbObject):
    ...
    def throw_if_not_valid(self):
        ...
        # if self._subtitle:
        #     self.throw_if_greater_than_limit(value=self._subtitle, limit=self._subtitle_limit, value_name='subtitle')
