...
# Decorator for getting data from raw json data
def get_json_data(func):
    def get_data(request, *args, **kwargs):
        json_data = request.body.decode('utf-8')

        try:
            kwargs['data'] = json.loads(json_data)
        ...
        if access_token is not None:
            try:
                if settings.API_TESTING:
                    user_profile_obj = BaseProfile.objects.get(access_token=access_token)
                    if user_profile_obj.verify_access_token_expiry():
                        kwargs['user'] = user_profile_obj
                    else:
                        raise BaseProfile.DoesNotExist

                else:
                    if device_id is not None:
                        user_profile_obj = BaseProfile.objects.get(
