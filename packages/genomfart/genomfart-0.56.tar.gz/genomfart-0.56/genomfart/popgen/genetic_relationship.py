...
@jit
def _append_genetic_relationship_with_missing_Endelman(genotypes, relationship_mat, snp_counts,
                                              min_MAF, max_missing, taxa_count, scale_factor):
    """ Calculates portion of genetic relationship accounted for by a single locus.
    Adds onto current matrix of genetic relationships, but can account for missing data,
    by not adding to a pair if at least one of the genotypes for that pair is missing. SNP
    counts used for each pair are updated at the same time.

    This is part of the calculation for Eq. 13 in Endelman and Jannink 2012.
    ...
    scale_factor : np.ndarray
        Current value of the scaling factor
    ...
    if (p < min_MAF or (1.-p) < min_MAF):
        ...
        return False
    E_geno = 2*p
    scale_factor[0] += 2*p*(1-p)
    # Get genetic relationship values
    for j in xrange(taxa_count):
        if genotypes[j] < 0: continue
        for k in xrange(j+1):
            if genotypes[k] < 0: continue
            relationship_mat[j,k] += ((genotypes[j]-E_geno)*(genotypes[k]-E_geno))
            snp_counts[j,k] += 1
    return True
...
class genetic_relationship:
    ...
    @staticmethod
    def get_genetic_relationships_with_missing_Endelman(snp_generator, min_MAF=0.025,
            max_missing = 500, verbosity = None):
        """ Gets the genetic relationship matrix, as defined in the 
        Endelman and Jannink paper
        (e.g. eq. 13 from http://www.g3journal.org/content/2/11/1405.full.pdf)
        ...
        while 1:
            try:
                ...
                if relationship_array is None:
                    ...
                    marker_count = np.zeros((taxa_count, taxa_count),
                                            dtype=np.int64)
                included = _append_genetic_relationship_with_missing_Endelman(genotypes, relationship_array,
                            marker_count, min_MAF, max_missing, taxa_count, scale_factor)
                iter_count += 1
                if verbosity:
                    if iter_count % verbosity == 0:
                        print "%d markers processed..." % iter_count
                ...
                continue
        return np.multiply((1./scale_factor[0]),relationship_array), marker_count
