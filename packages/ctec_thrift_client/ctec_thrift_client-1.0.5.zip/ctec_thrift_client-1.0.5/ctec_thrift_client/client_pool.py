...
class ClientPool:
    def __init__(self, service, server_hosts=None, zk_path=None, zk_hosts=None, max_renew_times=3, maxActive=20, maxIdle=10, get_connection_timeout=30, socket_timeout=30):
        '''
        ...
        with self.lock:
            if self.pool_size < self.maxActive:
                ...
                if ip:
                    ...
                    # 创建新的thrift client
                    socket = TSocket(ip.split(':')[0], int(ip.split(':')[1]), socket_timeout=1000*self.socket_timeout)
