...
def get_token(url_info):
    ...
    headers = {
        ...
        'Host': "www.sdarot.pm",
        'Origin': "http://www.sdarot.pm",
        'Accept-Language': "he-IL,he;q=0.8,en-US;q=0.6,en;q=0.4",
    }

    r = requests.post('http://www.sdarot.pm/ajax/watch', data=form, headers=headers)
...
def get_download_url_data(url_info):
    ...
    r = requests.post('http://www.sdarot.pm/ajax/watch', data=form, headers=headers, cookies=url_info['cookies'])
...
def main():
    ...
    try:
        ...
        for i in xrange(SECONDS_TO_WAIT):
            stdout.write('\r[*] Waiting {seconds} seconds... '.format(seconds=SECONDS_TO_WAIT-i))
            stdout.flush()
            sleep(1)
        stdout.write('Done!\n');
        stdout.flush()
