...
def post_regularise(params, px, new_px):
    ...
    count = px.shape[0]
    p_ixs = np.array([ix for (_, ix) in params['model'].param_info()])

    # TODO: allow models to identify parameters that should not be resampled?
    #       e.g., parameters that take discrete values.
    ...
    # TODO: use a relative scale?
    #       divide the range by the mean?
    #       hard to do robustly if parameters span 0
    ...
    # Ignore parameters that are highly correlated.
    # Note that the general problem of having a non-positive definite
    # covariance matrix means that *at least* one of the parameters can be
    # expressed as a linear combination of the other parameters.
    # If two parameters are linearly correlated, then we can identify them and
    # ignore them (non-diagonal entries will be 1 +/- epsilon).
    # In the more general case, we need to explore every possible subset of
    # parameters, or use a smarter method.
    # We *should* have np.linalg.matrix_rank(cov_mat) = npar
    # If not ... use the QR decomposition?
    # http://inst.eecs.berkeley.edu/~ee127a/book/login/l_mats_qr.html
    # (source: http://stackoverflow.com/questions/31093261/)
    # Test non-positive definite matrix:
    #     x = np.array([[1.0000, 0.7426, 0.1601, -0.7000, 0.5500],
    ...
    #     perm_mat = scipy.linalg.lu(x)[1]
    #     keep_mask = np.diagonal(perm_mat) == 1
    ...
    try:
        ...
        # TODO: remove rows/columns where absmax >= 1 - epsilon.
        #       i.e., ignore parameters with near-total correlations.
        x = cov_mat.copy()
        np.fill_diagonal(x, 0)
        max_cov = np.max(np.abs(x), axis=0)
        toln = 1 - 1e-3
        keep_mask = max_cov <= toln
        if not np.all(keep_mask):
            # TODO: logger.debug -- ignoring strongly correlated parameters
            cov_mat = cov_mat[keep_mask, :][:, keep_mask]
            # TODO: if no parameters left, don't regularise!
            #       if 1 parameter left, does the following behave sensibly?
            #       answer: yes it does!
        ...
        names = [name for (name, ix) in params['model'].param_info()
                 if ix in p_ixs]
        ...
        logger = logging.getLogger(__name__)
        logger.warn("Post-RPF Cholesky decomposition: {}".format(e.message))
    ...
    new_px[:, p_ixs] = np.clip(new_px[:, p_ixs] + scaled_samples,
                               params['param_min'][None, good],
                               params['param_max'][None, good])
...
def log_llhd_of(params, hist, hist_ix, obs, max_back=None):
    ...
    # Determine whether the log-likelihood function accepts an extra argument
    # and, if so, also pass the particle weights.
    log_llhd_args = inspect.getargspec(log_llhd)[0]
    if len(log_llhd_args) >= 5:
        ...
        if save_when is not None and save_to is not None:
            if when in save_when:
                ...
                with h5py.File(save_to, 'a') as f:
                    grp = f.require_group(pypfilt.text.to_bytes(when))
                    # Replace any previously-saved state for this date.
                    if 'offset' in grp:
                        del grp['offset']
                    if 'hist' in grp:
                        del grp['hist']
...
def resume_from_cache(params, streams, fs_dates):
    """
    ...
    :returns: A dictionary with keys:
        ...
        - ``'start'``: Either the time from which to begin the simulation
          (``datetime.datetime``) or ``None`` if there is no cached state.
        - ``'end'``: Either the time at which to end the simulation
          (``datetime.datetime``) or ``None`` if there is no cached state.
    ...
    def df_to_dict(df):
        ...
        def convert_values(tuples):
            ...
            for (name, v) in tuples:
                name = pypfilt.text.to_unicode(name)
                if name == 'date':
                    f = '%Y-%m-%d %H:%M:%S'
                    v = pypfilt.text.to_unicode(v)
                    yield (name, datetime.datetime.strptime(v, f))
                else:
                    if pypfilt.text.is_bytes(v):
                        v = pypfilt.text.to_unicode(v)
                    yield (name, v)

        return [dict(convert_values(zip(cols, row))) for row in df]
        ...
        try:
            with h5py.File(cache_file, 'r') as f:
                ...
                if 'obs' in f:
                    ...
                    current_obs = [o for o in sum(streams, [])
                                   if o['date'] <= min(fs_dates)]
                    diff_list = [o for o in cached_obs + current_obs
                                 if (o not in cached_obs) or
                                 (o not in current_obs)]
                    if diff_list:
                    ...
                    grp = pypfilt.text.to_bytes(last_good)
                    if grp not in f:
                        logger.debug("Last known-good date not in cache")
                        logger.debug("Looking for '{}' in:".format(grp))
                        for node in f.keys():
                        ...
                        for date in try_dates:
                            grp = pypfilt.text.to_bytes(date)
                            if grp in f:
                                msg = "Most recent cache: {}".format(date)
                                logger.debug(msg)
                                last_good = date
                                break
...
def forecast(params, start, end, streams, dates, summary, filename):
    ...
    invalid_fs = [pypfilt.text.to_unicode(d) for d in dates
                  if d < start or d >= end]
    if invalid_fs:
    ...
    sim = resume_from_cache(params, streams, forecast_dates)
    ...
    for start_date in forecast_dates:
        ...
        with h5py.File(sim['save_to'], 'r') as f:
            grp = f[pypfilt.text.to_bytes(start_date)]
    ...
    # Also save the observations to the cache file.
    from . import summary as summary_mod
    obs_df = summary_mod.obs_table(forecasts['obs'])
...
def default_params(model, max_days=0, px_count=0):
    ...
    """
    p_min, p_max = model.param_bounds()
    params = {
        'resample': {
