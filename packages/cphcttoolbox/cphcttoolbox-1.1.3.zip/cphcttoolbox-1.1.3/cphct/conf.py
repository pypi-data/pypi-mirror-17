...
def gap_pair_values(val):
    """Split gap position string of comma separated entries each
...
def _shared_opts():
    ...
    opts = {
        ...
        'compute_units': {
            ...
            'handler': int_value,
            'default': 1,
            'description': 'Number of compute units (per host)',
            },
        'limit_sources': {
            'long': 'limit-sources',
            'short': None,
        ...
        'workers': {
            ...
            'handler': None,
            'default': 0,
            'description': 'Helper to keep track of multi-GPU/core/node runs',
            },
        'worker_id': {
            'long':  None,
            'short': None,
...
def _gpu_opts():
    """Shared GPU-specific options

    Returns
    -------
    output : dict
        Returns a dictionary of GPU specific options helper dictionaries.
    """
    ...
    # Similar advice seems to apply for OpenCL.
...
def lrms_opts():
    """For enabling LRMS specific options

    Returns
    -------
    output : dict
        Returns a dictionary of LRMS options helper dictionaries.
    """
    ...
    opts = {
        'lrms_type': {
            ...
            'short': None,
            'args': str,
            'handler': lambda val: allowed_values(val, allowed_lrms_types),
            'default': None,
            'description': 'Local Resource Management System type',
            'allowed': allowed_lrms_types,
            },
        'lrms_nodes': {
            'long': 'lrms-nodes',
            'short': None,
            'args': str,
            'handler': colon_str_values,
            'default': None,
            'description': 'Colon separated list of cluster nodes',
            },
        'lrms_partition': {
            'long': 'lrms-partition',
            'short': None,
            ...
            'handler': str_value,
            'default': None,
            'description': 'LRMS partition to use',
            },
        }
    return opts
...
def engine_conf():
    ...
    -------
    output : dict
        Returns a configuration dictionary where corresponding option is set.
    """
...
def lrms_conf():
    ...
    return enable_conf_option(lrms_opts())
...
def parse_command(argv, opt_handlers, conf, ignore_opts={}):
    ...
    conf : dict
        A dictionary of configuration options.
    ignore_opts : dict
        A dictionary of application options to ignore for compatibilty.
    ...
    for (key, val) in ignore_opts.items() + opt_handlers.items():
        ...
        # Do not handle ignore_opts entries: set None handler
        if key in ignore_opts.keys():
            val['handler'] = None
        if val['short']:
            ...
            if entry['handler'] is not None:
                try:
                    ...
                    raise ParseError("invalid '%s' option: %s" % (key,
                            err))
            #else:
            #    print "DEBUG: ignore compatibility option: %s" % key
        else:
...
def parse_config(
    ...
    for (key, val) in parser.items('ALL'):
        ignore_entry = ignore_opts.get(key, None)
        if ignore_entry:
            continue
...
def parse_setup(
    ...
    dummy = parse_command(argv, opt_handlers, conf, ignore_opts)
    ...
    conf = parse_config(optional_files, required_files, opt_handlers,
                        conf, ignore_opts)
    conf = parse_command(argv, opt_handlers, conf, ignore_opts)
