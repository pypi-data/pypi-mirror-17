...
def get_access_token(code):
    ...
    if token:
        ...
        return token
    url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid={}&secret={}&code={}&grant_type=authorization_code'.format(settings.WEIXIN['app_id'], settings.WEIXIN['app_seckey'], code)
    r = requests.get(url, verify=False)
    if r.status_code == 200:
        response_data = r.json()
