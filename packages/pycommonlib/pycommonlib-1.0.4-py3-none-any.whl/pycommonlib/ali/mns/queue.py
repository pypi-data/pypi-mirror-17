...
class Queue(object):
    ...
    def send_message(self, message):
        if not isinstance(message, Message):
            raise ParameterError('message must be Message object')
    ...
    def receive_message(self, waitseconds):
        resp = http_client.get(self._composeUrl('messages'), params={'waitseconds':waitseconds}, valid_codes=(200, 404))
        if resp[0] == 404:
            return None
        else:
    ...
    def delete_message(self, receiptHandle):
        http_client.delete(self._composeUrl('messages'), '', params={'ReceiptHandle':receiptHandle}, valid_codes=(204, 404))
