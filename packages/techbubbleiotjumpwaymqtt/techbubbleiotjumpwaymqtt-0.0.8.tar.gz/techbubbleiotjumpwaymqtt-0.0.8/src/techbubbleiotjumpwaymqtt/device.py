...
class JumpWayPythonMQTTDeviceConnection():
	...
	def publishToDevicePhotos(self, data):
		if self._configs['locationID'] == None:
			print("locationID is required!")
			return False
		elif self._configs['zoneID'] == None:
			...
			devicePhotosTopic = '%s/Devices/%s/%s/Photos' % (self._configs['locationID'], self._configs['zoneID'], self._configs['deviceId'])
			self.mqttClient.publish(devicePhotosTopic,json.dumps(data))
			print("Published to Device Photos "+devicePhotosTopic)
