...
netcdf_incdir = subprocess.check_output(['nc-config', '--includedir'])
# remove trailing \n and leading spaces
netcdf_incdir = re.sub(r"\n\s*$", "", netcdf_incdir)

cmd = subprocess.check_output(['nc-config', '--libs'])
...
subprocess.call(["python", "pycf/generateLibCFConfig.py", 
                 "--netcdf_incdir={}".format(netcdf_incdir),
                 "--srcdir=./", "--builddir=./", 
                 "--config=pycf/libCFConfig.py"])
