...
* Module level settings are not synchronized.
  ...
  Use `cpy2py.twinterpreter.group_state.TwinGroupState` for initialisation, or write modules aware of twinterpreters.
...
See the python documentation on the :option:`-O` option and `PYTHONOPTIMIZE` environment variable.
