...
class VersionerCli(object):
    ...
    def trigger_cli(self):
        ...
        if jenkins_username is not None and jenkins_password is not None:
            requests.post(self.config.continuous_deployment_hook, HTTPBasicAuth(username=jenkins_username,
    ...
    def execute_tasks(self, main_tasks):
    ...
    def execute_post_tasks(self, post_tasks):
        for task in post_tasks:
            if task == 'publish npm':
                ...
                self.api.make_release(project_name=self.args.project_name,
                                      version=self.version,
                                      type='environment',
                                      text=self.args.build_type)
            elif task == 'trigger cli':
                self.trigger_cli()
