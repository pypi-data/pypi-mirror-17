...
def encode_kmer(kmer, kmer_size=31):
    print("HO!!!")
    bitpadding = 8*math.ceil(31*2/8)-(31*2)
    print(bitpadding)
...
def decode_kmer(binary_kmer, kmer_size):
    ...
    assert kmer_size <= 31
    # d = {"00": "A", "01": "G", "10": "C", "11": "T"}
    b = "{0:064b}".format(binary_kmer)[::-1]
    # print(b, binary_kmer)
    ret = ""
    for j in range(kmer_size):
        # print(j * 2, (j + 1) * 2, b[j * 2: (j + 1) * 2])
...
class CortexRecord(object):
    ...
    def __init__(self, kmer_size, kmer, coverages, edges, num_colours=1, binary_kmer=False):
        if binary_kmer:
            ...
            kmer = struct.unpack_from("Q", kmer, 0)[0]
