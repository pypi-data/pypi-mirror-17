...
class GenderAnnotator(FeatureAnnotator):
    ...
    use_names_list = True
    use_probabilistic_gender_classification_option = "probabilistic_gender"

    prominence_boost = 0.5
    threshold = 2

    def __init__(self, graph, config):
        FeatureAnnotator.__init__(self, graph, config)
        self.logger = getLogger(__name__)
        self.use_probabilistic_gender_classification = \
            self.use_probabilistic_gender_classification_option in config
        if self.use_probabilistic_gender_classification:
            self.bergsma_dict = bergma_counter
            self.logger.info(
                "Gender extractor: Using Bersgma and lin dictionary")
        if self.use_names_list:
            self.logger.info("Gender extractor: Using name List")
        if self.use_bergsma_gender_lists:
            self.logger.info("Gender extractor: Using Bergsma 2005 List")
    ...
    def _get_statistic_gender(self, mention_string):
        ...
        @return:  MALE, FEMALE, NEUTRAL constants or None.
        """
        if mention_string not in self.bergsma_dict:
            return None
        male, female, neutral, plural = \
            self.bergsma_dict.get(mention_string)
