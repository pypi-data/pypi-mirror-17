...
def process(config, text, parse_tree, speakers_list, output):
    ...
    processor = TextProcessor(
        ...
        soft_filter=config.soft_filter,
...
def metaparameters(config):
    """ Process the parameters that changes the parameters.
    @param config:
    @return: Nothing
    """
    # Sieves
    if "NO" in config.sieves or "no" in config.sieves:
    ...
    elif "no" in config.mention_purges or "NO" in config.mention_purges:
    ...
    elif "no" in config.mention_filters or "NO" in config.mention_filters:
    ...
    if "all" in config.mention_catchers or "ALL" in config.mention_catchers:
        ...
        config.mention_catchers = ['NamedEntities', 'PronounCatcher', 'EnumerableCatcher', 'ConstituentCatcher']
    ...
    elif "no" in config.mention_features or "NO" in config.mention_features:
        logger.info("Features: NO")
        config.mention_features = []
    elif len(config.mention_features) == 0:
        logger.debug("Features: DEFAULT")
        config.mention_features = [
            'type', 'construction', 'number', 'gender', 'animacy', 'speaker',
...
def generate_parser():
    ...
    parser.add_argument(
        '--file', '-f', dest='input', action='store', default=None,
        help='File to process. If not specified standard input is used.')
    parser.add_argument(
        '--document_id', "-ID", dest='document_id', action='store', default=None,
        help='The id used to identify the document used in writing output process.')
    parser.add_argument(
        '--treebank', '-t', dest='parse_tree', action='store', default=None,
        help="An additional file with the document syntactic parse trees if base one don't have it")
    parser.add_argument(
        '--speakers', '-s', dest='speakers', action='store', default=None,
        help='an aditional file with the speakers of the tokens.')
    parser.add_argument(
        '--language', '-l', dest='language', action='store', default="en",
        help='The language-annotation to use')
    parser.add_argument(
        '--unsafe_tree', dest='secure_tree', action='store_false')
    parser.add_argument(
        '--reader', '-r', dest='reader', action='store', default="NAF",
        help='The input file reader to use. Default NAF.')
    parser.add_argument(
        '--writer', '-w', dest='writer', action='store', default="NAF",
    ...
    parser.add_argument(
        '--sieves_options', dest='sieves_options', action="append", default=[],
        help="The options passed to the sieves.")
    parser.add_argument(
        '--extractor_options', dest='extractor_options', action="append",
        default=['probabilistic_gender'],
        help="The options used during mention extraction.")
    parser.add_argument(
        '--soft_purge', action='store_true', help='Purges extract mentions from'
        'entities but not remove they from the result.')
    parser.add_argument(
        '--soft_filter', action='store_true', help='Filters avoid mention to be'
        'processed but are keep on the response.')
    parser.add_argument(
        '--version', dest='version', action='store', default=version)
    parser.add_argument(
        '--linguisticParserName', dest='linguistic_parser_name',
        action='store', default=None)
    parser.add_argument(
        '--linguisticParserVersion', dest='linguistic_parser_version',
        action='store', default=version)
    parser.add_argument(
        '--linguisticParserLayer', dest='linguistic_parser_layer',
        action='store', default="coreferences")
    parser.add_argument(
        '--hostname', dest='hostname', action='store', default=os.name)
    parser.add_argument(
        '--time_stamp', dest='start_time', action='store', default=time.gmtime())
    return parser
