...
from corefgraph.constants import SPAN, ID, NER, CONSTITUENT, HEADWORD, SENTENCE
...
class BaseGraphBuilder(object):
    ...
    def new_graph(self):
        """ Create a new Graph and set all base properties for graph, vertex and
         edges."""
    ...
    def get_all_sentences(self, graph):
        ...
        return GraphWrapper.get_all_node_by_type(
            graph=graph, node_type=self.root_type)
    ...
    def get_all_coref_entities(self, graph):
        ...
        """
        return GraphWrapper.get_all_node_by_type(
            graph=graph, node_type=self.entity_node_type)
    ...
    def add_gold_mention(self, mention_id, entity_id, label):
        ...
        @param gold_mention_id: The ID of the gold mention in the graph
    ...
    def get_all_gold_mentions(self, graph):
        ...
        """
        return GraphWrapper.get_all_node_by_type(
            graph=graph, node_type=self.gold_mention_node_type)
    ...
    # Named entities
    def get_all_named_entities(self, graph):
        """ Get all named entities of the graph
        ...
        """
        return GraphWrapper.get_all_node_by_type(
            graph=graph, node_type=self.named_entity_node_type)
    ...
    # Words
    def add_word(self, form, node_id, label, lemma, pos, order,
                 sentence=None, begin=-1, end=-1):
    ...
    def get_head_word(self, element):
        ...
        #    self.graph, element, self.head_word_edge_type)
        # if head is None:
        #     head = self.get_words(element)[-1]
        try:
            return element[HEADWORD]
        except KeyError:
            if element.get(self._head, False):
                return self.get_head_word(element[self._head])
            else:
                self.logger.warning("No HEAD word:%s", element)
                return self.get_words(element)[-1]
    ...
    def get_gephi(self):
        GraphWrapper.get_gephi(graph=self.graph)
