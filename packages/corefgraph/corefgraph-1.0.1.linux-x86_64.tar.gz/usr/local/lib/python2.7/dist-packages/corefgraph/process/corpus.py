...
import pycorpus
from file import generate_parser as generate_parser_for_file, \
    process as process_file
...
def generate_parser():
    ...
    parser.add_argument('--result', dest='result', action='append',
                        default=["coref"], help="The extension added to result files.")
...
def file_processor(file_name, config):
    ...
    file_handler = logging.FileHandler(
        filename=config.log_base + name + datetime.datetime.now().time().isoformat() + ".log", mode="w")
