...
class SpeakerSieve(Sieve):
    ...
    def __init__(self, multi_sieve_processor, options):
        ...
        if len(self.configs.intersection(set(options))):
            ...
            self.logger.info("SPEAKER Default options")
    ...
    def is_speaker(self, speaker):
        """ Is the mention a speaker?

        @param speaker: a mention that may be a speaker
        @return: True or False
        """
        # TODO check if only head
        speaker_words = self.graph_builder.get_words(speaker)
    ...
    def equal_speakers(self, mention, candidate):
        if self.is_speaker(mention) and self.is_speaker(candidate):
