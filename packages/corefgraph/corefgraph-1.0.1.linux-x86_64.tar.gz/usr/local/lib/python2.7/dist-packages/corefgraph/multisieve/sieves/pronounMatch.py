...
from corefgraph.constants import SPAN, FORM, ID, TAG, CONSTITUENT, UTTERANCE, PREV_SPEAKER
from corefgraph.multisieve.features.constants import NUMBER, GENDER, PREDICATIVE_NOMINATIVE, MENTION, \
    PERSON, FIRST_PERSON, SECOND_PERSON, THIRD_PERSON, UNKNOWN, DEMONYM
...
class PronounMatch(Sieve):
    ...
    def are_coreferent(self, entity, mention, candidate_entity, candidate):
        ...
        @param candidate: The candidate that may refers the entity.
    ...
    def get_candidates(self, text_order, candidate_order, mention, index_sent):
        ...
        if len(candidate_order[index_sent][index_ment]["entity"][1]) == 1 and self.is_pronoun(mention):
            ...
            return super(PronounMatch, self).get_candidates(text_order, candidate_order, mention, index_sent)
