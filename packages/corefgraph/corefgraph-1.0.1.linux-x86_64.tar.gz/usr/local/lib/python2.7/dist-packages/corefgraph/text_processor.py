...
from corefgraph.constants import SPAN, FORM, ID, HEADWORD, NER
from corefgraph.graph.kafbuilder import KafAndTreeGraphBuilder
...
class TextProcessor:
    ...
    def __init__(self, verbose, reader, secure_tree, lang, sieves,
                 sieves_options, extractor_options, mention_catchers,
                 mention_filters, mention_features, soft_filter, mention_purges, soft_purge):
        self.verbose = verbose
        self.logger = logging.getLogger(__name__)
        self.logger.info("Sieve Options %s", sieves_options)
    ...
    def reset_graph(self):
        ...
        self.coreference_processor = CoreferenceProcessor(
            graph=graph, sieves_list=self.sieves,
            sieves_options=self.sieves_options,
            ...
            soft_purge=self.soft_purge)

        self.feature_extractors = [
            features.annotators_by_name[annotator](self.graph_builder, self.extractor_options)
    ...
    def build_graph(self, document):
        ...
        for index, sentence in enumerate(sentences_parsed):
            self.logger.info("#### Sentence {0}".format(index))
            # syntax graph construction
            sentence_root = self.graph_builder.process_sentence(
                graph=self.graph, sentence=sentence,
    ...
    def process_graph(self):
        ...
        """
        self.meta[self.graph_builder.doc_type] = self.graph.graph[self.graph_builder.doc_type]
        self.meta["sentences"] = []
        self.meta["features"] = defaultdict(Counter)
        # for sentence in self.coreference_processor.get_mentions():
        for sentence in self.coreference_processor.mentions_textual_order:
            sentence_mentions = []
            ...
            for mention in sentence:
                ...
                for feature_extractor in self.feature_extractors:
                    ...
                    for feature in feature_extractor.features:
                        ...
                        if isinstance(feature_value, set):
    ...
    def store(self, stream, config):
        ...
        writer.store(
            graph=self.graph,
            encoding=config.encoding, language=config.language,
            version=config.version,
            linguistic_parsers=(
                (config.linguistic_parser_name,
                 config.linguistic_parser_version,
                 config.linguistic_parser_layer),),
