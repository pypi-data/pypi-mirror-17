...
class ConllDocument(BaseDocument):
    ...
    def store(self, graph, coreference_processor,**kwargs):
        ...
        self._annotate_ner(graph_builder,
                           graph_builder.get_all_named_entities(graph))
        for coref_index, entity \
                in enumerate(graph_builder.get_all_coref_entities(graph), 1000):
