...
from corefgraph.constants import TAG, FORM, NER, POS, ID, LEMMA, SPAN, CONSTITUENT
...
def is_pleonastic(graph_builder, constituent):
    ...
    # except:
    #     self.logger.error("ERROR: %s in %s ", mention, np_siblings)
    #     raise Exception()
    ...
    valid_verb, valid_verb_index = graph_builder.check_sibling_property(
        0, vp_constituents, LEMMA, verbs.pleonastic_verbs)
    if valid_verb:
        return True
    return False
