...
def device_inspect(device_id, as_json=False):
    ...
    print('Status:      ', device['status_text'],
          '({0})'.format(device['status_reason'])
          if device['status_reason'] else '')
...
def device_ls(as_json=False):
    ...
    fmt_string = ("{device_id:<20}  {location:<30}  {connection:10}  "
                  + "{carrier:<20}  {status:10}  {reason:20}  {status_when:20}")
    ...
    def colored_status(status_text):
        ...
        color = mapping[status_text]
        if len(status_text) < 10:
            status_text += ''.join([' '] * (10 - len(status_text)))
    ...
    for device in sorted_devices:
        ...
        print(fmt_string.format(
            ...
            status=colored_status(device['status_text']),
            reason=device['status_reason'] or '-',
