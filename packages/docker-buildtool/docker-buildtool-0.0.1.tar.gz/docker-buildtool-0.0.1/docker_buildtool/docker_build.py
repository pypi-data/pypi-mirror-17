...
def expand_include(paths, build_root):
    expanded = []
    for path in paths:
        # Temporarily try to support Python 2
        if '**' in path:
            try:
                ...
                if major == '3':
                    ...
                    raise error.Error('Could not expand glob: {}. (HINT: recursive=True is a Python 3-only feature. You are on Python 2!)'.format(e))
        else:
...
class DockerBuild(object):
    def __init__(self, dockerfile, build_root, include, docker_repo, workdir=None, ignore=None, tag=None):
...
""".format(self.workdir)
    ...
    def run(self, dryrun=False):
        ...
        try:
            ...
            always_ignore = ['**/.git', '**/*.egg-info', '**/*.o']
            ignore = [entry for entry in os.listdir(self.build_root) if entry != '.' and entry != '..'] + \
