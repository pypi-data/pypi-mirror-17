...
import os
import unittest
from jinja2_registry import Renderer, \
...
class TestBasic(unittest.TestCase):
    ...
    def test_defaults(self):
        ...
        # Confirm structure
        structure = ','.join([tag.name for tag in bs.find('html').find_all()])
        expected = 'head,title,body,ul,li,a,li,a,div,dl,dt,dd,dt,dd,dt,dd,dt,dd,dt,dd'
        self.assertEqual(structure, expected)

        # Confirm default handling
        values = ','.join([tag.text for tag in bs.find('dl') if tag.name == 'dd'])
