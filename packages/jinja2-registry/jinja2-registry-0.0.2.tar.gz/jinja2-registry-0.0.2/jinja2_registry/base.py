"""Template registry behaviour and base classes"""
...
_PATTERN = re.compile(r'[\W_]+', re.UNICODE)
def _classify(blob):
    """Remove non-alphanumeric characters and
    capitalize the first letter"""
    return string.capitalize(_PATTERN.sub('', blob))
...
class Registry(object):
    ...
    def environment_options(self):
        ...
        envopts['loader'] = jinja2.PrefixLoader(self.namespaces)
        return envopts
...
class Renderer(dict):
    ...
    def __init__(self, template, namespace=None):
...
def register_loader(namespace, loader, defaults=None):
