...
def now():
    ...
    Get epoch time in milliseconds
    """
    return (concurrent.Context.runtime()).now()
...
def sleep(seconds):
    ...
    Suspend execution of this thread for some number of seconds
    """
    (concurrent.Context.runtime()).sleep(seconds);
...
def uuid():
    ...
    Get a v4 random UUID (Universally Unique IDentifier)
    """
    return (concurrent.Context.runtime()).uuid()
    ...
    def _parseLong(self, num):
        ...
        while ((i) < (len(num))):
            ...
            if (((d) < (ParsedNumber.ZERO)) or ((d) > (ParsedNumber.NINE))):
                ...
                val = ((10L) * (val)) + (((d) - (ParsedNumber.ZERO)))
...
class ParsedLong(ParsedNumber):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_value")):
            (self)._value = _cast(value, lambda: long)
...
def _getLogger(topic):
    if (logging.Config._autoconfig()):
        (logging.makeConfig()).configure();

    return (concurrent.Context.runtime()).logger(topic)
...
def toJSON(obj, cls):
    ...
    if (((cls).name) == (u"quark.Map")):
        ...
        while ((idx) < (len(strKeys))):
            ...
            value = (map).get(key);
            if (((reflect.Class.get(_getClass(key))).name) == (u"quark.String")):
...
def fromJSON(cls, result, json):
    ...
    if (((cls).name) == (u"quark.List")):
        list = _cast(result, lambda: _List);
        while ((idx) < ((json).size())):
...
class ServletError(error.Error):
    def _init(self):
        error.Error._init(self)
...
class ServiceInstance(_QObject):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"breaker")):
            (self).breaker = _cast(value, lambda: behaviors.CircuitBreaker)
...
class Server(_QObject):
    ...
    def onHTTPRequest(self, request, response):
        ...
        if ((((envelope).getObjectItem(u"$method")) == ((envelope).undefined())) or (((envelope).getObjectItem(u"rpc")) == ((envelope).undefined()))):
            ...
            result = _cast((method).invoke(self.impl, args), lambda: concurrent.Future);
            (result).onFinished(ServerResponder((self)._sendCORS, request, response));

    def onServletError(self, url, error):
        (concurrent.Context.runtime()).fail((((u"RPC Server failed to register ") + (url)) + (u" due to: ")) + ((error).getMessage()));
...
class _CallbackEvent(_QObject):
    ...
    def fullfilPromise(promise, value):
        if ((reflect.Class.ERROR).hasInstance(value)):
            (promise)._reject(_cast(value, lambda: error.Error));
        else:
            (promise)._resolve(value);

    def fireEvent(self):
        result = ((self)._callable)((self)._value) if callable((self)._callable) else ((self)._callable).call((self)._value);
        if ((reflect.Class.get(u"quark.Promise")).hasInstance(result)):
...
class _Callback(concurrent.EventContext):
    def _init(self):
        concurrent.EventContext._init(self)
        self._callable = None
        self._next = None
    ...
    def _setField(self, name, value):
        if ((name) == (u"_context")):
            (self)._context = _cast(value, lambda: concurrent.Context)
...
class _CallIfIsInstance(_QObject):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_class")):
            (self)._class = _cast(value, lambda: reflect.Class)
...
class PromiseValue(_QObject):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_failureResult")):
            (self)._failureResult = _cast(value, lambda: error.Error)
...
class Promise(_QObject):
    ...
    def __init__(self):
        ...
        (self)._successResult = None
        (self)._failureResult = _cast(None, lambda: error.Error)
    ...
    def _resolve(self, result):
        if ((reflect.Class.ERROR).hasInstance(result)):
            (self)._reject(_cast(result, lambda: error.Error));
            return
...
class _BoundMethod(_QObject):
    ...
    def __init__(self, target, methodName, additionalArgs):
        self._init()
        (self).target = target
        (self).method = (reflect.Class.get(_getClass(target))).getMethod(methodName)
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"method")):
            (self).method = _cast(value, lambda: reflect.Method)
...
class IO(_QObject):
    ...
    def httpRequest(request):
        ...
        (concurrent.Context.runtime()).request(request, _IOHTTPHandler(factory));
    ...
    def schedule(delayInSeconds):
        ...
        """
        factory = PromiseFactory();
        (concurrent.Context.runtime()).schedule(_IOScheduleTask(factory), delayInSeconds);
        return (factory).promise
