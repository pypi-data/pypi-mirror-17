...
class MDKRuntime(_QObject):
    ...
    def _init(self):
        ...
        self.dispatcher = actors.MessageDispatcher()
    ...
    def getScheduleService(self):
        ...
        """
        return _cast(((self).dependencies).getService(u"schedule"), lambda: actors.Actor)
    ...
    def getFileService(self):
        ...
        Return File service.
        """
        return _cast(((self).dependencies).getService(u"files"), lambda: files.FileActor)
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"dispatcher")):
            (self).dispatcher = _cast(value, lambda: actors.MessageDispatcher)
...
class QuarkRuntimeWSActor(_QObject):
    ...
    def __init__(self, originator, factory):
        self._init()
    ...
    def onWSConnected(self, socket):
        (self.logger).debug((((((u"onWSConnected, current state ") + ((self).state)) + (u"originator: ")) + (_toString((self).originator))) + (u" and I am ")) + (_toString(self)));
        if (((self).state) == (u"ERROR")):
            (self.logger).debug(u"Connection event after error event!");
            return
    ...
    def onWSError(self, socket, error):
        (self.logger).debug((((u"onWSError, current state ") + ((self).state)) + (u"originator: ")) + (_toString((self).originator)));
    ...
    def onWSMessage(self, socket, message):
        (self.logger).debug((((u"onWSMessage, current state: ") + ((self).state)) + (u"originator: ")) + (_toString((self).originator)));
        ((self).dispatcher).tell(self, WSMessage(message), (self).originator);

    def onWSFinal(self, socket):
        (self.logger).debug((((u"onWSFinal, current state ") + ((self).state)) + (u"originator: ")) + (_toString((self).originator)));
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"factory")):
            (self).factory = _cast(value, lambda: promise.PromiseResolver)

        if ((name) == (u"originator")):
            (self).originator = _cast(value, lambda: actors.Actor)
...
class QuarkRuntimeWebSockets(_QObject):
    ...
    def connect(self, url, originator):
        ...
        factory = promise.PromiseResolver((self).dispatcher);
        actor = QuarkRuntimeWSActor(originator, factory);
        (self.connections).append(actor);
        ((self).dispatcher).startActor(actor);
        (quark.concurrent.Context.runtime()).open(url, actor);
...
class FakeWSActor(_QObject):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"resolver")):
            (self).resolver = _cast(value, lambda: promise.PromiseResolver)
...
class _ScheduleTask(_QObject):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"requester")):
            (self).requester = _cast(value, lambda: actors.Actor)
...
class FakeTime(_QObject):
    ...
    def onMessage(self, origin, msg):
        sched = _cast(msg, lambda: Schedule);
        (self._scheduled)[(len(_List((self._scheduled).keys())))] = (_FakeTimeRequest(origin, (sched).event, ((self)._now) + ((sched).seconds)));
    ...
    def pump(self):
        ...
        """
        idx = 0;
        keys = _List(((self)._scheduled).keys());
        while ((idx) < (len(keys))):
    ...
    def scheduled(self):
        ...
        """
        return len(_List(((self)._scheduled).keys()))
...
def defaultRuntime():
    ...
    fileActor = files.FileActorImpl(runtime);
