...
class WSClient(_QObject):
    ...
    def _init(self):
        ...
        self.sock = None
        self.sockUrl = None
        self.lastConnectAttempt = 0L
        self.lastHeartbeat = 0L
    ...
    def onScheduledEvent(self):
        rightNow = long(round((((self).timeService).time()) * (1000.0)));
        heartbeatInterval = long(round((float(self.ttl) / float(2.0)) * (1000.0)));
        reconnectInterval = long(round((self.reconnectDelay) * (1000.0)));
        if (self.isConnected()):
            if (self.isStarted()):
                self.pump();
    ...
    def doOpen(self):
        self.open(self.url());
        self.lastConnectAttempt = long(round((((self).timeService).time()) * (1000.0)))

    def doHeartbeat(self):
        self.heartbeat();
        self.lastHeartbeat = long(round((((self).timeService).time()) * (1000.0)))
