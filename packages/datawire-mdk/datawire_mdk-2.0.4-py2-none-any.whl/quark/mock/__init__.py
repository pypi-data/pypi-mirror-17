...
class MockResponse(_QObject):
    ...
    def getHeaders(self):
        return _List((self.headers).keys())
...
class MockTask(_QObject):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_scheduledFor")):
            (self)._scheduledFor = _cast(value, lambda: long)
...
class MockRuntime(_QObject):
    ...
    def schedule(self, handler, delayInSeconds):
        ...
        (task)._scheduledFor = ((self).now()) + (long(round((1000.0) * (delayInSeconds))))
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_currentTime")):
            (self)._currentTime = _cast(value, lambda: long)
