...
class LogEvent(TracingEvent):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"timestamp")):
            (self).timestamp = _cast(value, lambda: long)
        ...
        if ((name) == (u"sequence")):
            (self).sequence = _cast(value, lambda: long)
...
class TracingClient(mdk_protocol.WSClient):
    def _init(self):
        ...
        self._syncRequestPeriod = 5000L
        self._syncInFlightMax = 50
        self._buffered = _List([])
        self._inFlight = _List([])
        self._logged = 0L
        self._sent = 0L
        self._failedSends = 0L
        self._recorded = 0L
        self._lastSyncTime = 0L
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"lastConnectAttempt")):
            (self).lastConnectAttempt = _cast(value, lambda: long)

        if ((name) == (u"lastHeartbeat")):
            (self).lastHeartbeat = _cast(value, lambda: long)
        ...
        if ((name) == (u"_syncRequestPeriod")):
            (self)._syncRequestPeriod = _cast(value, lambda: long)
        ...
        if ((name) == (u"_logged")):
            (self)._logged = _cast(value, lambda: long)

        if ((name) == (u"_sent")):
            (self)._sent = _cast(value, lambda: long)

        if ((name) == (u"_failedSends")):
            (self)._failedSends = _cast(value, lambda: long)

        if ((name) == (u"_recorded")):
            (self)._recorded = _cast(value, lambda: long)

        if ((name) == (u"_lastSyncTime")):
            (self)._lastSyncTime = _cast(value, lambda: long)
