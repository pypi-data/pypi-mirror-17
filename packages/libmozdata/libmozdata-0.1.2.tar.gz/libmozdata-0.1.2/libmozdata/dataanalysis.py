...
def have_crashes_stopped(crashes_info, all_versions, product='Firefox', thresholds={}, path=None):
    def get_date(build_id, version, channel):
        ...
        if channel in ['nightly', 'aurora']:
            ...
            if major in all_versions[channel] and version in all_versions[channel][major]['versions']:
    ...
    def handler(json, data):
        ...
        for info in json['facets'].get('build_id', []):
        ...
        if True or (path and data['stop'] == 'no'):
            signature = data['signature']
            bugid = data['bugid']
            channel = data['channel']
            filename = os.path.join(path, '%s_%s_%s' % (signature, bugid, channel))
            if len(filename) >= 251:
                ...
                if not min_date or push_date >= min_date:
                    ...
                    if add_data:
                        ...
                        if dates[1]:
                            ...
                            d['build_id'] = ['>=' + utils.get_buildid_from_date(dates[0]),
                                             '<' + utils.get_buildid_from_date(today)]
                            d['versions'] = v['all']
                        data.append(d)
                    elif is_untimely:
                        d['stop'] = 'untimely'
