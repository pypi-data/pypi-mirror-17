...
class Relationship(object):
    def __init__(self, serializer_class, field_name, required_field=None):
    ...
    def get_serializer(self):
        if not self.serializer_instance:
            self.serializer_instance = self.serializer_class(utils.GlobalStore.controller)
        return self.serializer_instance

    def check_for_field(self):
        if self.required_field:
            return self.get_serializer().controller.has_field(self.required_field)
        return True
...
class OneToOneFieldRelationship(Relationship):
    def __init__(self, serializer_class, field_name, required_field=None, related_name=None):
        self.related_name = related_name
        super().__init__(serializer_class, field_name, required_field)
...
def serialize(controller, serializer_class, model):
