...
class AbstractVnfm(object):
    ...
    def create_vnf_record(vnfd, flavor_key, vlrs, extension):
        vnfr = {"lifecycle_event_history": [], "parent_ns_id": extension.get("nsr-id"), "name": vnfd.get("name"),
                "type": vnfd.get("type"), "requires": vnfd.get("requires"), "provides": vnfd.get("provides"),
    ...
    def on_message(self, body):
        ...
        if action is "INSTANTIATE":
            ...
            vnfr = self.instantiate(vnf_record=AbstractVnfm.create_vnf_record(vnfd, vnfdf.get("flavour_key"), vlrs, extension),
                             scripts=scripts, vim_instances=vim_instances)

        nfv_message = "{'action':'INSTANTIATE', 'vnfr': %s }" % json.dumps(vnfr)
