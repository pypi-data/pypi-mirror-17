...
def update_function_option_entries(job_file):
    with util.options.OptionsFile(job_file) as options:
        ...
        try:
            for option in tracer_input_options:
                options[option]
        except KeyError:
            pass
        else:
            ...
            try:
                del options['/model/initial_constant_concentrations']
            except KeyError:
                pass
            else:
                logger.info('/model/initial_constant_concentrations removed in {}.'.format(job_file))
