...
def update_function_rename_option_entries(job_file, old_entry, new_entry, not_exists_okay=False):
    with util.options.OptionsFile(job_file) as options:
        try:
            options[new_entry]
        except KeyError:
            try:
                ...
                if not not_exists_okay:
                    logger.error('Entry {} is missing in job option file {}.'.format(old_entry, job_file))
            else:
                options[new_entry] = options[old_entry]
                del options[old_entry]
                logger.info('Renamed {} to {} in job option file {}.'.format(old_entry, new_entry, job_file))
...
def update_function_option_entries(job_file):
    with util.options.OptionsFile(job_file) as options:
        ...
        # try:
        #     options['/metos3d/initial_constant_concentrations_string']
        # except KeyError:
        #     try:
        #         initial_constant_concentrations = options['/model/initial_constant_concentrations']
        #     except KeyError:
        #         pass
        #     else:
        #         initial_constant_concentrations_string = ','.join(map(str, initial_constant_concentrations))
        #         options['/metos3d/initial_constant_concentrations_string'] = initial_constant_concentrations_string
        
        
        ## remove initial concentrations if tracer input files exist
        tracer_input_options = ['/model/tracer_input_files', '/metos3d/tracer_input_dir', '/metos3d/tracer_input_filenames']
        try:
            for tracer_input_option in tracer_input_options:
                options[option]
        except KeyError:
            pass
        else:
            try:
                del options['/metos3d/initial_constant_concentrations_string']
            except KeyError:
                pass
            else:
                logger.info('/metos3d/initial_constant_concentrations_string removed in {}.'.format(job_file))
            try:
                del options['/metos3d/constant_concentrations_string']
            except KeyError:
                pass
            else:
                logger.info('/metos3d/constant_concentrations_string removed in {}.'.format(job_file))
...
if __name__ == "__main__":
    with util.logging.Logger():
        ...
        # update_job_options(update_function, model_names=['MITgcm-PO4-DOP'])
        
        def update_function (job_file):
            # update_function_rename_option_entries(job_file, '/metos3d/input_filenames', '/metos3d/tracer_input_filenames', not_exists_okay=True)
            # update_function_rename_option_entries(job_file, '/metos3d/output_filenames', '/metos3d/tracer_output_filenames', not_exists_okay=False)
            update_function_option_entries(job_file)
        update_job_options(update_function)
