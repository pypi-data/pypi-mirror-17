...
from onyx.core import UfoBase, CreateInMemory, ValueType, b36encode
...
class AgingTradableObj(TradableObj):
    ...
    # -------------------------------------------------------------------------
    @ValueType("Callable")
    def ExpectedSecurities(self, graph, *args):
        if len(args):
            trans_type, *_ = args
        else:
            trans_type = graph(self, "ExpectedTransaction")
        return graph(self, graph(self, "TradeTypes")[trans_type])
