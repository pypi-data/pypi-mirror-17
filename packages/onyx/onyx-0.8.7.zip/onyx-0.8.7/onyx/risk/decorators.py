...
def WithRiskValueTypes(cls):
    ...
    cls.ForwardVol = ValueType("PropSubGraph")(ForwardVol)
...
# -----------------------------------------------------------------------------
def regress_returns(sec_prcs, idx_prcs):
    ...
    explanatory_vars = np.vstack([idx_rets, np.ones_like(idx_rets)]).T
    return np.linalg.lstsq(explanatory_vars, sec_rets)[0]
