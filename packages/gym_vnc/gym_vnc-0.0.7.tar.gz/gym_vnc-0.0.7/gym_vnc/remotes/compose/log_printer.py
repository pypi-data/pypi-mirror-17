...
from gym_vnc.remotes.compose import colors, utils
from gym_vnc.remotes.compose.signals import ShutdownException
from gym_vnc.remotes.compose.utils import split_buffer

def build(containers, service_names):
    presenters = build_log_presenters(service_names, False)
    printer = LogPrinter(containers, presenters)
    printer.start()
