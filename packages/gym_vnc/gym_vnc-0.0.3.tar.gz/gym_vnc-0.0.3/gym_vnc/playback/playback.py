...
class ActionQueue(object):
    ...
    def key_event(self, key, down):
        self.actions.append(('KeyEvent', key, down))

    def pointer_event(self, x, y, buttonmask):
        self.actions.append(('PointerEvent', x, y, buttonmask))
...
def VNCMultiReader(logfile_dirs, *args, **kwargs):
    return itertools.chain(*[VNCReader(logfile_dir, *args, **kwargs) for logfile_dir in logfile_dirs])

class VNCReader(object):
    """
    ...
    """
    def __init__(self, logfile_dir, fps=30, render=False, include_buffer_update_in_observation=False):
        self.playback = Playback(logfile_dir, render=render)
        self._frame_length_in_s = 1.0 / fps

        self.observation_in_frame = None
        self.include_buffer_update_in_observation = include_buffer_update_in_observation

        # Get the first observation
        while self.observation_in_frame == None:
            timestamp, action, observation, reward, done, info = self.playback.next()
            self._end_of_frame_timestamp = timestamp
            self.timestamp = timestamp

            self.action_in_frame = action
            self.observation_in_frame = observation
            self.reward_in_frame = reward
            self.done_in_frame = done
            self.info_in_frame = info

    def __iter__(self):
        return self

    def next(self):
        # Reset everything from the last frame except for the observation
        self.action_in_frame = []
        self.reward_in_frame = 0.
        self.done_in_frame = False
        self.info_in_frame = None

        while self.timestamp <= self._end_of_frame_timestamp:
            timestamp, action, observation, reward, done, info = self.playback.next()
            self.timestamp = timestamp
            self.action_in_frame += action
            self.observation_in_frame = observation  # Use the last observation
            self.reward_in_frame += reward
            self.done_in_frame = self.done_in_frame or done  # True if any done
            self.info_in_frame = info or self.info_in_frame  # Take the last info

        self._end_of_frame_timestamp += self._frame_length_in_s

        if self.include_buffer_update_in_observation:
            observation_in_frame = self.observation_in_frame
        else:
            observation_in_frame = self.observation_in_frame[0]

        return (
            self.action_in_frame,
            observation_in_frame,
            self.reward_in_frame,
            self.done_in_frame,
            self.info_in_frame
        )
