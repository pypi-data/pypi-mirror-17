...
class VNCCoreEnv(vnc_env.VNCEnv):
    docker_image = gym_vnc.docker_image('quay.io/openai/vnc-core-envs')
    ...
    def wrap(self, env):
        """Wraps an environment (possibly itself, with extra pre-existing
        wrappers) to make its interface match the core env version as
        much as possible.

        For Atari this means cropping the image and translating the
        action space.
        """
        spec = gym.spec(self.core_env_id)
        if spec._entry_point.startswith('gym.envs.atari:'):
            from gym_vnc.wrappers import atari_wrappers
            env = atari_wrappers.wrap(env)
        return env
