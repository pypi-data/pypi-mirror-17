...
if _useNumpy:
    _allocateArray = lambda length: numpy.empty(length, dtype=numpy.float64)
    ...
    _finaliseArray = lambda array: array

    _allocateIntArray = lambda length: numpy.empty(length, dtype=numpy.int32)

    _finaliseIntArray = _finaliseArray

    _getArrayData = lambda array: array.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
    ...
    _allocateArray = lambda length: (ctypes.c_double * length)()
    ...
    _finaliseArray = lambda array: tuple(array)

    _allocateIntArray = lambda length: (ctypes.c_int * length)()

    _finaliseIntArray = _finaliseArray

    _getArrayData = lambda array: ctypes.cast(ctypes.byref(array), ctypes.POINTER(ctypes.c_double))
...
class HelperMethods(object):
    ...
    def CreateOrcaFlexObject(modelHandle, objectHandle, _type=None):
        if type(objectHandle) is not Handle:
    ...
    @staticmethod
    def GetBooleanModelProperty(modelHandle, propertyId):
        result = ctypes.wintypes.BOOL()
        status = ctypes.c_int()
        _GetModelProperty(modelHandle, propertyId, ctypes.byref(result), status)
        _CheckStatus(status)
        return bool(result)
...
class Model(object):
    ...
    def frequencyDomainProcessComponents(self):
        ...
        def componentGenerator():
            ...
            _GetFrequencyDomainProcessComponents(self.handle, count, None, status)
            _CheckStatus(status)

            components = (FrequencyDomainProcessComponent * count.value)()
            _GetFrequencyDomainProcessComponents(self.handle, count, components, status)
            _CheckStatus(status)
    ...
    def RunSimulation(self, enableAutoSave=False, autoSaveIntervalMinutes=DefaultAutoSaveIntervalMinutes, autoSaveFileName=None):
    ...
    def ProcessBatchScript(self, filename, enableAutoSave=False, autoSaveIntervalMinutes=DefaultAutoSaveIntervalMinutes, autoSaveFileName=None):
        filename = _PrepareString(filename)
        params = RunSimulationParameters(enableAutoSave, autoSaveIntervalMinutes, autoSaveFileName)
        status = ctypes.c_int()
        _ProcessBatchScript(
    ...
    def FrequencyDomainResultsFromProcess(self, process):
        ...
        status = ctypes.c_int()
        _GetFrequencyDomainResultsFromProcess(self.handle, len(process), process, result, status)
        _CheckStatus(status)
    ...
    def FrequencyDomainSpectralDensityFromProcess(self, process):
        ...
        _GetFrequencyDomainSpectralDensityGraphFromProcess(
            self.handle, len(process), None, pointCount, None, status)
        _CheckStatus(status)
        curve = _GraphCurve(pointCount.value)
        _GetFrequencyDomainSpectralDensityGraphFromProcess(
            self.handle, len(process), process, pointCount, curve, status)
        _CheckStatus(status)
...
class OrcaFlexObject(DataObject):
    ...
    def FrequencyDomainResults(self, varName, objectExtra=None):
        ...
        result = FrequencyDomainResults()
        status = ctypes.c_int()
        _GetFrequencyDomainResults(self.handle, objectExtra, varID, result, status)
        _CheckStatus(status)
        return result.asObject()
    ...
    def RangeGraph(self, varName, period=None, objectExtra=None, arclengthRange=None, stormDurationHours=None):
        ...
        def frequencyDomainRangeGraph(period, objectExtra, arclengthRange, stormDurationHours):
            ...
            if period.PeriodNum != pnWholeSimulation:
                raise DLLError(
                    'Invalid period for frequency domain range graph, must be pnWholeSimulation')
            staticStateRangeGraph = self.RangeGraph(
                varName, pnStaticState, objectExtra, arclengthRange)
            fdr = [self.FrequencyDomainResults(varName, objectExtraAtArcLength(X))
        ...
        period = HelperMethods.PreparePeriod(self.modelHandle, period)
        if HelperMethods.IsFrequencyDomainDynamics(self.modelHandle) and period.PeriodNum != pnStaticState:
...
def RainflowAssociatedMean(values):
    ...
    return [objectFromDict({ 'HalfCycleRange': X, 'AssociatedMean': Y }) for X, Y in zip(summary.X, summary.Y)]
