...
class BaseHandler(logging.Handler):
    ...
    def get_extra_info(self, record):
        if hasattr(record, 'extra_info'):
            ...
            elif type(record.extra_info) in (dict, SortedDict, OrderedDict):
                extra_info = u""
                if 'list' in record.extra_info:
                    extra_info = u"\n".join([unicode(row) for row in record.extra_info.pop('list')]) + "\n\n"
                extra_info += u"\n".join([u"%s: %s" % (unicode(key), unicode(val)) for key, val in record.extra_info.iteritems()])
            else:
