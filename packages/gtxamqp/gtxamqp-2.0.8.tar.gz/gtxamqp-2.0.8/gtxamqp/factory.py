...
# The spec specifies that the maximum number of channels per connection is a 16-bit number.
...
class AmqpReconnectingFactory(protocol.ReconnectingClientFactory):
    ...
    def get_available_channel_id(self):
        ...
        client_id = self.unused_client_ids.pop() if self.unused_client_ids else len(self.clients) + 1
        return client_id
