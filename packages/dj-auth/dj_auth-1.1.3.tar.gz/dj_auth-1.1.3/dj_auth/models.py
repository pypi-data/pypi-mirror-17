...
class ObjectFilter(models.Model):
    ...
    def build_filter(cls, user=None, content_type=None, session=None):
        ...
        if model_class._meta.label_lower in cls._filter_dict:
            for k, v in cls._filter_dict[model_class._meta.label_lower][FILTER_ID].iteritems():
                filter_query += "Q(**{'%s': %s}) | " % (k, v)

            if filter_query.endswith('| '):
                filter_query = filter_query[:-2]

            for k, v in cls._filter_dict[model_class._meta.label_lower][EXCLUDE_ID].iteritems():
            ...
            for related_model, field_name_list in related_models.iteritems():
                for filter_text, filter_value in cls._filter_dict[related_model._meta.label_lower][FILTER_ID].iteritems():
                    for field_name in field_name_list:
                        filter_query += "Q(**{'%s__%s': %s}) | " % (field_name, filter_text, filter_value)

                for filter_text, filter_value in cls._filter_dict[related_model._meta.label_lower][EXCLUDE_ID].iteritems():
                    for field_name in field_name_list:
    ...
    def apply_user_generic_objects_filter(self, user=None, session=None):
        ...
        for record in data:
            ...
            if exclude_query:
                ...
                queryset = queryset.exclude(content_type_id=record.content_type.id, object_id__in=ids).values_list('id', flat=True)
            if filter_query:
                ids = model_class.objects.filter(filter_query).values_list('id', flat=True)
                queryset = queryset.filter(content_type_id=record.content_type.id, object_id__in=ids).values_list('id', flat=True)
