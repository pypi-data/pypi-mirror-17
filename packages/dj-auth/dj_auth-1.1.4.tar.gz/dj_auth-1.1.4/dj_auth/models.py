    ...
    def apply_user_generic_objects_filter(self, user=None, session=None):
        ...
        for record in data:
            ...
            if exclude_query:
                ...
                queryset = queryset.exclude(content_type_id=record.content_type.id, object_id__in=ids)
            if filter_query:
                ids = model_class.objects.filter(filter_query).values_list('id', flat=True)
                queryset = queryset.filter(content_type_id=record.content_type.id, object_id__in=ids)
