...
class Options(object):
    def __init__(self, sjts=False, sjts_chunk_size=16384, sjts_stream=True):
...
def milli_to_datetime(milli):
    """
    Converts unix time to utc datetime TODO

    :param int float milli: unix time TODO
    :return:
    """
...
def date_to_milli(dt):
    """
    Date to utc unix time TODO

    :param datetime.date dt: date
    :return: the date as a unix time string TODO
    :rtype: str
    """
...
def datetime_to_milli(dt):
    """
    Datetime to utc unixtime TODO

    :param datetime.datetime dt: date
    :return : date as a unix time string TODO
    :rtype : str
    """
...
def to_milli(dt):
    ...
    if isinstance(dt, datetime.datetime):
        ...
        raise ValueError("You can pass unixtime(int) or datetime.date objects only") #TODO
...
class Connection(object):
    ...
    def search(self, query='', date_start=None, date_finish=None, fields=None, size=0,
        ...
        :rtype: dict
        """
        data = self.shooju_api.search_series(query, date_start, date_finish, size,
                                             query_size, fields, sort_on=sort_on, sort_order=sort_order)
        series = []
    ...
    def scroll(self, query='', date_start=None, date_finish=None, fields=None, size=0,
               query_size=2500, sort_on=None, sort_order=None, operators=None):
        """
        ...
        r = self.shooju_api.search_series(query, date_start, date_finish, size, fields=fields, scroll=True,
    ...
    def get_points(self, series_id, date_start=None, date_finish=None, size=10,
        ...
        :rtype: list
        """
        data = self.shooju_api.get_series(series_id, date_start, date_finish, size,
                                          snapshot_job_id=snapshot_job_id, snapshot_date=snapshot_date, reported_date=reported_date)
        return list(map(lambda p: Point(p[0], p[1]), data['points'])) if data is not None else None

    def get_point(self, series_id, dt, snapshot_job_id=None, snapshot_date=None, reported_date=None):
        """
        ...
        :rtype: shooju.Point
        """
        points = self.get_points(series_id, dt, dt, snapshot_job_id=snapshot_job_id, snapshot_date=snapshot_date, reported_date=reported_date)
        if points is None:
            return None
        return points[0] if len(points) == 1 else Point(dt, None)

    def get_field(self, series_id, field, snapshot_job_id=None, snapshot_date=None, reported_date=None):
        """
        ...
        :rtype: str
        """
        fields = self.get_fields(series_id, [field], snapshot_job_id=snapshot_job_id, snapshot_date=snapshot_date, reported_date=reported_date)
        if fields is None:
            return None
        return fields.get(field)

    def get_fields(self, series_id, fields=None, snapshot_job_id=None, snapshot_date=None, reported_date=None):
        """
    ...
    def _post_bulk(self, bulk_data, job_id=None, collision_retry_timeout=60, async=False):
        ...
        """
        return self.shooju_api.bulk_series(bulk_data, job_id, collision_retry_timeout, async)
    ...
    def _get_bulk(self, bulk_data):
        ...
        """
        return self.shooju_api.bulk_series(bulk_data)
...
class ShoojuApi(object):
    ...
    def get_series(self, series_id, date_start=None, date_finish=None,
                   size=10, fields=None, snapshot_job_id=None, snapshot_date=None, reported_date=None):
        """
        Retrieves series
        ...
        try:
            response = self.client.get(self.API_SERIES_SINGLE, params=params, sjts=options.sjts)
    ...
    def bulk_series(self, requests, job_id=None, collision_retry_timeout=60, async=False):
        """
        ...
        while req_index_to_send and attempts_left:
            ...
            payload = {'requests': requests_to_send}
            response = self.client.post(self.API_SERIES_BULK, ignore_exceptions=True, data_json=payload, params=params)
    ...
    def _can_retry(resp):
        ...
        """
        if not resp['success'] and 'series is locked for change' in resp.get('description', ''):
            return True
        else:
            return False
    ...
    def delete_series_by_query(self, query, force=False, job_id=None):
        ...
        """
        return self.client.delete(self.API_SERIES_DELETE_BY_QUERY,
    ...
    def search_series(self, query='', date_start=None, date_finish=None, size=10,
                      query_size=None, fields=None, scroll=False,
                      sort_on=None, sort_order=None, operators=None):
        """
...
class Client(object):
    ...
    def _call(self, path=None, ignore_exceptions=False, params=None, sjts=False, **kwargs):
        """
        ...
        :param sjts: if True will use sjts response format. Supported only on GET series apis.
        ...
        if binary_response or sjts:
            method = partial(method, stream=options.sjts_stream)
        ...
        if binary_response:
            ...
            response = self._read_sjts(self._stream_bytes_from_chunked_content(r.iter_content(chunk_size=options.sjts_chunk_size)))
            return response if ignore_exceptions else _check_errors(response)
    ...
    def _stream_bytes_from_chunked_content(self, chunks):
        """
        Generates byte-by-byte from a generator of byte chunks (strings)

        :param chunks: generator of byte chunks
        """
        for chunk in chunks:
            for byte in chunk:
                yield byte

    def _read_sjts(self, stream):
        """
        Reads and parses data from sjts response

        :param stream: sjts bytes iterator
        """
        header = self._read_bytes(stream, 4)
        json_size = struct.unpack('I', header)[0]
        json_body = json.loads(self._read_bytes(stream, json_size))
        number_of_points = json_body.pop('points', 0)
        json_body['points'] = self._read_points_stream(stream) if number_of_points else []
        return json_body

    def _read_bytes(self, stream, n_bytes):
        """
        Reads bytes from iterator

        :param stream: bytes iterator
        :param n_bytes: number of bytes to read from stream
        """
        return ''.join(islice(stream, 0, n_bytes))

    def _read_points_stream(self, stream):
        """
        Iterates points from sjts stream

        :param stream: sjts bytes iterator
        """
        header_size = 8  # Q
        while True:
            header = self._read_bytes(stream, header_size)
            if not header:
                break
            strip_len = struct.unpack('Q', header)[0]
            for dt, val in self._unpack_strip(stream, strip_len):
                yield [dt, val[0]]

    def _unpack_strip(self, strip_stream, strip_len):
        """
        Extracts points data from sjts strip.

        :param strip_stream: strip bytes iterator
        :param strip_len: strip length in bytes
        """
        strip_head_size = 8  # q
        date_interval_size = 4  # I
        val_size = 8  # d
        dt = struct.unpack('q', self._read_bytes(strip_stream, strip_head_size))[0]
        if strip_len > 16:
            step = self._read_bytes(strip_stream, date_interval_size)
            step = struct.unpack('I', step)[0]
            bytes_read = strip_head_size + date_interval_size
            while bytes_read < strip_len:
                yield dt, struct.unpack('d', self._read_bytes(strip_stream, val_size))
                dt += step
                bytes_read += val_size
        else:
            yield dt, struct.unpack('d', self._read_bytes(strip_stream, val_size))
