...
class DFASATBuild(build):
    def run(self):
        ...
        target_files = [os.path.join(build_path, file_) for file_ in ['dfasat.lib.so', 'dfasat.so']]

        def compile():
            call(cmd, cwd=os.path.join(BASEPATH, SOURCE_DIR))

        self.execute(compile, [], 'Compiling dfasat')

        lib_folder = os.path.join(self.build_lib, 'dfasat', 'lib')
...
setup(name='dfasat',
      version='0.0.1.42',
      description='DFASAT',
      author='Benjamin Loos, Christian Hammerschmidt',
      packages=find_packages(),
#      dependency_links=DEPENDENCY_LINKS,
      install_requires=INSTALL_REQUIRES,
#      setup_requires=SETUP_REQUIRES,
      author_email='christian.hammerschmidt@uni.lu, loos.benjamin@gmail.com',
      package_dir = {'dfasat': 'dfasat'},
      zip_safe=False,
