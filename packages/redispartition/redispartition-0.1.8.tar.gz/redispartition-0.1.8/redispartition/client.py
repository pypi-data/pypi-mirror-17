...
class RedisCluster(object):
    ...
    @pipeiflist
    def set(self, k, _bytes, conn=None, partition_arg=0):
        return conn.set(k, _bytes)

    @pipeiflist
    def hset(self, name, key, value, conn=None, partition_arg=0):
        return conn.hset(name, key, value)

    @pipeiflist
    def hexists(self, name, key, conn=None, partition_arg=0):
        return conn.hexists(name, key)

    @pipeiflist
    def hget(self, name, key, conn=None, partition_arg=0):
        return conn.hget(name, key)

    @pipeiflist
    def hkeys(self, name, conn=None, partition_arg=0):
        return conn.hkeys(name)

    @pipeiflist
    def hgetall(self, name, conn=None, partition_arg=0):
        return conn.hgetall(name)

    @pipeiflist
    def setbit(self, k, i, bit, conn=None, partition_arg=0):
        return conn.setbit(k, i, bit)

    @pipeiflist
    def get(self, k, conn=None, partition_arg=0):
        return conn.get(k)

    @pipeiflist
    def getbit(self, k, i, conn=None, partition_arg=0):
        return conn.getbit(k, i)

    @pipeiflist
    def rpush(self, k, i, conn=None, partition_arg=0):
        return conn.rpush(k, i)

    @pipeiflist
    def lrange(self, name, start, end, conn=None, partition_arg=0):
        return conn.lrange(name, start, end)

    @pipeiflist
    def llen(self, k, conn=None, partition_arg=0):
        return conn.llen(k)

    @pipeiflist
    def incr(self, *args, conn=None, partition_arg=0, **kwargs):
        return conn.incr(*args, **kwargs)

    @pipeiflist
    def sadd(self, k, v, conn=None, partition_arg=0):
        return conn.sadd(k, v)

    @pipeiflist
    def srem(self, k, v, conn=None, partition_arg=0):
        return conn.srem(k, v)

    @pipeiflist
    def sismember(self, k, v, conn=None, partition_arg=0):
        return conn.sismember(k, v)

    @pipeiflist
    def smembers(self, k, conn=None, partition_arg=0):
        return conn.smembers(k)

    @pipeiflist
    def delete(self, k, conn=None, partition_arg=0):
        return conn.delete(k)

    @pipeiflist
    def bitcount(self, k, conn=None, partition_arg=0):
        return conn.bitcount(k)

    @pipeiflist
    def bitpos(self, k, v, conn=None, partition_arg=0):
        return conn.bitpos(k, v)

    @pipeiflist
    def scard(self, k, conn=None, partition_arg=0):
        return conn.scard(k)

    @pipeiflist
    def pfadd(self, name, *values, conn=None, partition_arg=0):
        return conn.pfadd(name, *values)

    @pipeiflist
    def pfcount(self, *sources, conn=None, partition_arg=0):
        return conn.pfcount(*sources)
