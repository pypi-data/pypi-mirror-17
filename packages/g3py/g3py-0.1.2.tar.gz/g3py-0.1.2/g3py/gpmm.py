...
class KernelMM(KernelFunction):
    def __init__(self, _kern, _steps, _noise, _w):
        self.kern = _kern
        self.steps = _steps[:, np.newaxis].T
        self.noise = _noise
        self.w = _w
    ...
    def cov(self, X, X2=None):
        ...
        Xsteps = X + self.steps
        if X2 is None:
            FullCov = self.kern.cov(Xsteps.reshape(Xsteps.size, 1))
            FullCov = np.kron(np.eye(X.size), w).dot(FullCov).dot(np.kron(np.eye(X.size), w).T) + np.exp(
                self.noise) * np.eye(X.size)
        else:
            X2steps = X2 + self.steps
            FullCov = self.kern.cov(Xsteps.reshape(Xsteps.size, 1), X2steps.reshape(X2steps.size, 1))
            FullCov = np.kron(np.eye(X.size), w).dot(FullCov).dot(np.kron(np.eye(X2.size), w).T)
        return FullCov
    ...
    def cross(self, X, _x):
        ...
        Xsteps = X
        _xsteps = _x + self.steps
        Cw = self.kern.cov(Xsteps.reshape((Xsteps.size, 1)), _xsteps.reshape((_xsteps.size, 1)))
        return Cw.dot(np.kron(np.eye(_x.size), w).T)
