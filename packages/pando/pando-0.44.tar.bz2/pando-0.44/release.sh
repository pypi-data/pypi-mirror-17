...
if [ -z "$1" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        until python setup.py sdist --formats=zip upload; do :; done
        until python setup.py sdist --formats=gztar upload; do :; done
        until python setup.py sdist --formats=bztar upload; do :; done
        until python setup.py bdist_wheel --universal upload; do :; done
