...
def test_query():
    db = bob.db.bio_filelist.Database(example_dir, use_dense_probe_file_list=False)
...
def test_query_protocol():
    db = bob.db.bio_filelist.Database(os.path.dirname(example_dir), use_dense_probe_file_list=False)
    p = 'example_fl'
    ...
    assert len(db.objects(groups='dev', purposes='enroll', model_ids='3',
                          protocol=p)) == 4  # 4 samples for to enroll model '3' in the dev set
    assert len(db.objects(groups='dev', purposes='enroll', model_ids='7',
                          protocol=p)) == 0  # 0 samples for enrolling model '7' (it is a T-Norm model)
    ...
    assert len(db.objects(groups='dev', purposes='probe', classes='client',
                          protocol=p)) == 8  # 8 samples as client probes in the dev set
    assert len(db.objects(groups='dev', purposes='probe', classes='impostor',
                          protocol=p)) == 4  # 4 samples as impostor probes in the dev set
    ...
    assert len(db.tobjects(groups='dev', model_ids='3',
                           protocol=p)) == 0  # 0 samples for enrolling T-norm model '3' (no T-Norm model)
    assert len(db.zobjects(groups='dev')) == 8  # 8 samples for Z-norm impostor accesses
...
def test_query_dense():
    db = bob.db.bio_filelist.Database(example_dir, probes_filename='for_probes.lst')
...
def test_multiple_extensions():
    ...
    file = bob.db.bio_filelist.models.File("data/model4_session1_sample2", 4)
    ...
    file = bob.db.bio_filelist.models.File("data/model4_session1_sample1", 4)
    try:
