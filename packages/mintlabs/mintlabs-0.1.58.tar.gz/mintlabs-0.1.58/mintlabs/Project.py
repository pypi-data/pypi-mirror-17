...
def show_progress(done, total, finish=False):
    ...
    progress_message = " [{:.2f} %] Uploaded {:.2f} of {:.2f} Mb".format(done / float(total) * 100,
                                                                         done / (bytes_in_mb), total / (bytes_in_mb))
...
class Project:
    ...
    def __init__(self, account, project_id):
        ...
        if type(project_id) == str:
            ...
            project_name = next(iter(filter(lambda proj: proj["id"] == project_id,
                                            account.projects)))["name"]
    ...
    def list_input_containers(self, search_condition="*", limit=1000):
        ...
        content = self._account.send_request("file_manager/get_container_list",
                                             ...
                                             req_headers=req_headers)
        containers = [
            {"patient_secret_name": c["patient_secret_name"], "container_name": c["name"], "container_id": c["_id"]} for
            c in content]
        return containers
    ...
    def download_file(self, container_id, file_name, local_filename=False,
        ...
        if file_name not in self.list_container_files(container_id):
            msg = "File '{}' does not exist in container {}".format(file_name,
                                                                    container_id)
            logging.error(msg)
            return False
    ...
    def start_analysis(self, script_name, in_container_id, analysis_name=None,
        ...
        response = self._account.send_request(
            ...
            req_parameters=post_data)
        return self.__handle_start_analysis_response(post_data, response,
    ...
    def delete_analysis(self, analysis_id):
        ...
        if response.get('success', False):
            return True
        logging.error(response.get('message', 'Could not delete analysis (no more information provided by the server)'))
        return False
    ...
    def __handle_start_analysis_response(self, post_data, response,
        ...
        if response["success"] == 2 and "warning" in response:
            ...
            if not ignore_warnings:
                ...
                logging.error("Call start_analysis with ignore_warnings=True to continue through warnings.")
