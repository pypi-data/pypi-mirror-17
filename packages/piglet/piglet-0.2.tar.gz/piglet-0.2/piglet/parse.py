...
__all__ = ('OpenTag CloseTag OpenCloseTag Comment Text Entity PI Declaration '
           'CDATA html_parser parse_html'.split())
...
class OpenTag(ParseItem):
    ...
    attrs = attr.ib(default=attr.Factory(OrderedDict),
                    convert=OrderedDict)
...
class Attribute(ParseItem):
    ...
    def set_pos(self, pos):
        self.value_pos = pos.advance('{0.name}{0.space1}={0.space2}{0.quote}'
                                     .format(self))
        return super(Attribute, self).set_pos(pos)

    @property
    def source(self):
        return ('{0.name}{0.space1}='
