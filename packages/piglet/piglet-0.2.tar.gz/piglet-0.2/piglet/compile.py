...
__all__ = 'compile_to_source compile_to_ast compile_to_module'.split()
ustr = type(u'')
...
def compile_to_module(srcnode, filename, c=count(), bootstrap=None):
    module_name = 'piglet_tmpl_{}_{}'.format(next(c),
        hashlib.sha256(filename.encode('UTF-8')).hexdigest())
    ast_module = compile_to_ast(srcnode, filename)
    codeob = compile(ast_module, filename, 'exec')
...
class Compiler:
    ...
    def compile_blocknode(self, srcnode, parent, mode='define'):
        ...
        block_name = make_identifier(srcnode.name)
        block_funcname = '__piglet_block_{}'.format(block_name)
        ...
        block_func.body.extend(parse_and_strip('''
            super = __piglet_rt.get_super(__piglet_bases, '{0}')
        ...
        #        yield i
        #
        # We pass **__piglet_extra_blocks into BLOCK as it might have subblocks
        # defined that the caller doesn't know about
        parent.body.append(
    ...
    def compile_extendsnode(self, srcnode, parent):
        parent.body.extend(parse_and_strip(
            '__piglet_parent = __piglet_rt.load(__piglet_template, "{}")\n'
        ...
        block_ids = [make_identifier(n.name)
    ...
    def compile_ifnode(self, srcnode, parent):
        ...
        if srcnode.elsechildren:
            m = Module(body=[])
            for item in srcnode.elsechildren:
    ...
    def compile_choosenode(self, srcnode, parent):
        ...
        for item in srcnode.children:
            if isinstance(item, im.WhenNode):
                comparison = get_comparison('{} is False and {} == ({})'
                                            .format(chooseflag,
                                                    choosevar,
                                                    item.test))
                if_ = If(test=comparison, body=[], orelse=[])
    ...
    def compile_interpolatenode(self, srcnode, parent, mode='yield'):
        value = parse_and_strip('x = {}'.format(srcnode.value))[0].value
    ...
    def annotate_runtime_errors(self, body, imnode):
        ...
        handler = parse_and_strip(
            '__piglet_ctx["__piglet_exc_locations"].append(({!r}, {!r}))\n'
            'raise\n'
            .format(self.filename, imnode.pos.line))
        if not isinstance(body, list):
            body = [body]
        return TryExcept(body=body,
