...
class QuotedString(parse.ParseItem):
    ...
    @property
    def source(self):
        return '%{0.quote}{0.name}{0.quote}'.format(self)
...
def make_tree(parse_result):
    ...
    for item in parse_result:
        if isinstance(item, Statement) and item.is_open_tag():
...
def strip_ws_on_directive_lines(node):
    ...
    for p, n in zip(node.children, node.children[1:]):
        ...
        if isinstance(p, Statement) and isinstance(n, parse.Text):
...
def compile_for(stmt, *args):
    return im.ForNode(each=' '.join(args).strip())


def compile_if(stmt, *args):
    n = im.IfNode(test=' '.join(args).strip())
...
def compile_trans(stmt, message=None):
    return im.TranslationNode(
        message=message, children=[_compile_node(c) for c in stmt.children])


def compile_transname(stmt, name):
    return im.TranslationPlaceholder(
        name=name, children=[_compile_node(c) for c in stmt.children])
