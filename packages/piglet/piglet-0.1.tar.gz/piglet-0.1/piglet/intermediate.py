...
class IfNode(DirectiveNode, ContainerNode):
    ...
    elsechildren = attr.ib(default=attr.Factory(list))
...
class WithNode(DirectiveNode, ContainerNode):
    ...
    def get_pairs(self):
        ...
        values = [tuple(s.strip() for s in str(item).split('=', 1))
                    for item in values]
        return values
...
def _walk_tree(order, n, parent, pos):
    ...
    if order == 'pre':
        yield parent, n, pos
        children = enumerate(n.children)
    else:
        children = reversed(list(enumerate(n.children)))
