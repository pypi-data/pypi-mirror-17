...
directives = [
    ...
    Directive(name='call', attr='function', factory=im.Call.factory),
    ...
    Directive(ns=XMLNS['i18n'],
              name='name',
              attr='name',
              factory=im.TranslationPlaceholder),
]
...
def strip_whitespace(node, strip):
    ...
    if strip:
        ...
        for ix, n in enumerate(node.children):
            ...
            if isinstance(following, (type(None), Element)):
                mo = WS_R.match(n.content)
                if mo:
                    n.content = mo.group(1)

            if isinstance(preceding, (type(None), Element)):
...
def pp_move_else_inside_if(elsenode, parent):
    ...
    takewhile(lambda ix, item: item is not elsenode, siblings_before)

    for ix, item in siblings_before:
        if isinstance(item, im.IfNode):
            item.elsechildren = elsenode.children
            break
    else:
...
def _compile_text(node):
    ...
    for i in items:
        if isinstance(i, (str, ustr)):
            ...
            container.append(im.InterpolateNode.factory(value=i.value, pos=pos, autoescape=(not node.cdata)))
...
def _compile_attr(a, container):
    ...
    if len(items) == 1 and isinstance(items[0], interpolate.Interpolation):
        interp = items[0]
        container.append(
            im.WithNode(
        ...
        for i in items:
            if isinstance(i, (str, ustr)):
                ...
                container.append(im.InterpolateNode(value=i.value, pos=pos))
                pos = pos.advance(i.source)
        outro_text.pos = pos
