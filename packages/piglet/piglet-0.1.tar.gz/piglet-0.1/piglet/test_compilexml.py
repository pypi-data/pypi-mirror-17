...
def test_it_compiles_tag_with_outer_directive_attrs():
    assert c('<div py:if="foo">bar</div>') == [
        im.IfNode(pos=P(1, 13), test='foo', children=[
        ...
        im.ChooseNode(
            ...
            children=[
                im.WhenNode(pos=P(1, 23),
                            test='1',
                            children=[im.TextNode(pos=P(1, 41),
                                                  content='one')]),
                im.WhenNode(pos=P(1, 54),
                            test='2',
                            children=[im.TextNode(pos=P(1, 72),
                                                  content='two')]),
                im.TextNode(pos=P(1, 85), content='<p>some text</p>'),
                im.OtherwiseNode(pos=P(1, 101),
                                 children=[im.TextNode(pos=P(1, 115),
                                                       content='zzz')]),
            ]
        )
    ]
...
def test_it_compiles_if_else():
    ...
    assert c('<py:if test="foo">xxx</py:if>'
        ...
        im.IfNode(
            ...
            elsechildren=[im.TextNode(pos=P(1, 41), content='zzz')]
        )
    ]
...
def test_it_compiles_pycall():
    assert c('<py:call function="func(1)">'
        ...
        im.Call(
            pos=P(1, 1),
            function="func(1)",
            children=[im.CallKeyword(pos=P(1, 29),
                                     name='x',
                                     children=[im.TextNode(pos=P(1, 50),
                                                           content='foo')]),
                      im.CallKeyword(pos=P(1, 66),
                                     name='y',
                                     children=[im.TextNode(pos=P(1, 87),
                                                           content='bar')])]
        )
    ]
...
def test_it_raises_sensible_exceptions_on_malformed_directives():
    try:
        ...
        message = str(e)
        assert message == ("Unrecognized directive 'py:wtf' in element "
...
def test_it_compiles_inline_interpolation():
    assert c('<div>$a</div>') == [im.TextNode(pos=P(1, 1), content='<div>'),
...
def test_it_compiles_conditional_attr_interpolation():
    ...
    assert c('<div class="$foo"></div>') == [

        im.TextNode(pos=P(1, 1), content='<div '),
        im.WithNode(pos=P(1, 6), vars=[('__piglet_tmp', 'foo')],
                    children=[
                        im.IfNode(pos=P(1, 6),
                                  test='__piglet_tmp is not None',
                                  children=[
                                      im.TextNode(pos=P(1, 6),
                                                  content='class="'),
                                      im.InterpolateNode(pos=P(1, 13),
                                                         value='__piglet_tmp'),
                                      im.TextNode(pos=P(1, 17), content='"')
                                    ])]),
        im.TextNode(pos=P(1, 18), content='></div>')]
...
def test_it_processes_strip_whitespace_and_keeps_line_numbering():
    assert c(u'<p py:whitespace="strip"> '
             u'<py:if test="True"> hello! </py:if></p>') == [
                 ...
                 im.IfNode(pos=P(1, 27), test='True', children=[
                     im.TextNode(pos=P(1, 47), content='hello!')
                 ]),
    ...
    assert c(u'<html><py:whitespace value="strip"> '
             ...
             u'</py:whitespace></html>') == [
                 ...
                 im.IfNode(pos=P(1, 40), test='True', children=[
                     im.TextNode(pos=P(1, 60), content='hello!')
                 ]),
...
def test_it_strips_whitespace_between_elements():
    assert c('<p py:whitespace="strip"> <a/> <b/> </p>') == [
        im.TextNode(pos=P(1, 1), content='<p><a/><b/></p>')
    ]


def test_it_compiles_pyattrs():
    assert c('<div class="x" py:attrs="{\'class\': \'y\'}"></div>') == [
        im.TextNode(pos=P(1, 1), content='<div '),
        im.InlineCodeNode(pos=P(1, 1), pysrc="__piglet_attrs = {}"),
        im.InlineCodeNode(pos=P(1, 1), pysrc="__piglet_attrs.update({'class': 'y'})"),
        im.IfNode(pos=P(1, 6), test="'class' not in __piglet_attrs",
                  children=[im.TextNode(pos=P(1, 6), content='class="x" ')]),
        im.ForNode(pos=P(1, 41),
                   each="__piglet_attr_k, __piglet_attr_v in __piglet_attrs",
                   children=[
                       im.TextNode(pos=P(1, 41), content=' '),
                       im.InterpolateNode(pos=P(1, 41), value='__piglet_attr_k'),
                       im.TextNode(pos=P(1, 41), content='="'),
                       im.InterpolateNode(pos=P(1, 41), value='__piglet_attr_v'),
                       im.TextNode(pos=P(1, 41), content='"')]),
        im.TextNode(pos=P(1, 41), content='></div>')
    ]
