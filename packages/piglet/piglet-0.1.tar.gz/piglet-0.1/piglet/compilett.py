...
def compile_for(stmt):
    return im.ForNode(each=''.join(stmt.args).strip())


def compile_if(stmt):
    n = im.IfNode(test=''.join(stmt.args).strip())
    ...
    if children:
        ...
        if isinstance(last, Statement) and last.name == 'else':
            children = children[:-1]
            n.elsechildren = [_compile_node(c) for c in last.children]
...
def _compile_node(node):
    ...
    if isinstance(node, Container):
        ...
        return DIRECTIVES[node.name](node)
