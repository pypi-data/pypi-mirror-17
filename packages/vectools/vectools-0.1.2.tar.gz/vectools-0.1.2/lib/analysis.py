...
from lib.inputoutput import outputvector, vecparse, ParseVectors, _shared_params
import numpy as np
import sys
from sklearn.decomposition import PCA


def confusionmatrix(parser):
...
def runLDA(parser, ordered_file_names_iter, has_title=False, has_row_ids=False):
    """Run an LDA
...
def columnstats(parser):
    ...
    :return:
    """
    parser.add_argument('--infiles', dest='infiles', nargs='+', required=True, help="")
    parser.add_argument('--has_title', dest='has_title', type=bool, required=False, default=False, help="")
    parser.add_argument('--has_row_ids', dest='has_row_ids', type=bool, required=False, default=False, help="")
...
def run_pca(parser):
    """ Preforms principal component analysis (PCA) on a matrix and returns the
    components (Principal axes in feature space, representing the
    ...
    2,20,5000,45,1
    1,24,6000,77,-1
    $ vectortools.py pca -d , matrix.csv
    ...
    """
    parser.add_argument('vector', nargs='?', type=str, help='vector to calculate pca for', default="sys.stdin")
    parser.add_argument('-n', "--components", type=int, help='number of components to keep', default=None)
    _shared_params(parser)
    args = parser.parse_args()

    input_vector = ParseVectors(args.vector, args.column_titles, args.row_titles, delimiter=args.delimiter)
    matrix = input_vector.parse()
    columns = input_vector.col_titles
    if args.components is None or args.components > np.shape(matrix)[1]:
        number_of_components = np.shape(matrix)[1]
    else:
        number_of_components = args.components
    pca = PCA(n_components=number_of_components)
    pca.fit(matrix).transform(matrix)
    cols = list()

    for i in range(number_of_components):
        cols.append("comp_{}".format(i))

    outputvector(pca.components_.T, cols, None)
...
# Closed pattern == x is closed if x if frequenct and no super pattern y > x exists with the same support
# Max-pattern
# Support is the probablity that a transcation contains x and y
# An item is frequent if the support of X in no les than a minsup threshold denoted as sigma
def relativesupport():
    # TODO
    x = 0
...
def pearson_group(parser):
    ...
    parser.add_argument('-g', "--groups", type=str, required=True,
                        help='groups which to calculate the correlation for. There has to be at least two groups (comma seperated)')
    parser.add_argument('-r', '--pearson-correlation-threshold', type=float, default=0.90)
    parser.add_argument('-p', '--p-value-threshold', type=float, default=0.05)
    parser.add_argument('--processes', type=int, default=1)
    parser.add_argument('-n', '--number-of-values-above-zero', help="(default: 5)", type=int, default=5)
    _shared_params(parser,True, False, True, True)
    args = parser.parse_args()

    input_vector = ParseVectors(args.vector, args.column_titles, None, delimiter=args.delimiter, only_apply_on_columns=args.only_apply_on)
    matrix = input_vector.parse()
    ...
    (new_matrix, new_cols) = _calc_correlation(matrix, cols, groups, processes, above_zero, correlation_threshold, p_value_threshold,_pearson_calc)
    input_vector.out(new_matrix, new_cols)


def _calc_correlation(matrix, cols, groups, processes, above_zero, correlation_threshold, p_value_threshold, function, text="pearson_coefficient"):
...
def spearman(parser):
    ...
    parser.add_argument('-g', "--groups", type=str, required=True,
                        help='groups which to calculate the correlation for. There has to be at least two groups (comma seperated)')
    parser.add_argument('-r', '--spearmans-rank-correlation-threshold', type=float, default=0.90)
    parser.add_argument('-p', '--p-value-threshold', type=float, default=0.05)
    parser.add_argument('-n', '--number-of-values-above-zero', help="(default: 5)", type=int, default=5)
...
def summary(parser):
    ...
    """
    parser.add_argument('vector', nargs='?', type=str, help='vector to summarize', default="sys.stdin")
    _shared_params(parser, only_apply_on=True)
    parser.add_argument('--human-readable', action="store_true", help="print human readable output (no vector)", default=None)
...
def minimum(parser):
    ...
    """
    parser.add_argument('vector', nargs='?', type=str, help='vector to calculate minimum for', default="sys.stdin")
    _shared_params(parser, only_apply_on=True)
    args = parser.parse_args()

    input_vector = ParseVectors( args.vector, args.column_titles, args.row_titles, delimiter=args.delimiter, only_apply_on_columns=args.only_apply_on)
...
def maximum(parser):
    ...
    8,50,12500,98,1
    """
    parser.add_argument('vector', nargs='?', type=str, help='vector to calculate maximum for', default="sys.stdin")
    _shared_params(parser, only_apply_on=True)
    args = parser.parse_args()
...
def percentile(parser):
    ...
    parser.add_argument("vector", nargs="?", type=str, help="A vector to calculate the percentile for", default="sys.stdin")
    parser.add_argument("-p", "--percentile", help="Percentile between 0 and 1", metavar="p", required=True, type=float)
...
def sd(parser):
    ...
    """
    parser.add_argument('vector', nargs='?', type=str, help='vector to calculate standard deviation for', default="sys.stdin")
    _shared_params(parser, only_apply_on=True)
    args = parser.parse_args()
    input_vector = ParseVectors(args.vector, args.column_titles, args.row_titles, delimiter=args.delimiter, only_apply_on_columns=args.only_apply_on)
