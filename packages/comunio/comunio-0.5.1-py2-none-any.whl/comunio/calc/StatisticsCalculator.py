...
class StatisticsCalculator(object):
    ...
    def __init__(self, comunio_session, database_manager, xkcd_mode =False):
        u"""
    ...
    def generate_time_graph(self, player, mode):
        u"""
        ...
        x_values = []
        y_values = [] if mode == u"points" else [self.__database_manager.get_player_buy_value(player)]

        smallest_date = datetime.datetime.utcnow()
        i = len(historic_data) - 1
        while i > -1:
            data_point = historic_data[i]
            data_date = datetime.datetime.strptime(data_point[u"date"], u"%Y-%m-%d")
        ...
        image_name = (player + u"-" + mode).replace(u".", u"_").replace(u" ", u"_")
        image_path = os.path.join(os.path.expanduser(u"~"), u".comunio", u"images", image_name)
