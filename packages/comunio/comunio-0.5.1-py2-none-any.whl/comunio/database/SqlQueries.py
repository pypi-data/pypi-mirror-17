...
class SqlQueries(object):
    ...
    def apply_sql_schema(database):
        ...
        database.execute(u"CREATE TABLE IF NOT EXISTS players ("
                         u"name TEXT NOT NULL,"
                         u"position TEXT NOT NULL,"
                         u"value INTEGER NOT NULL,"
                         u"points INTEGER NOT NULL,"
                         u"date TEXT NOT NULL"
                         u");")

        database.execute(u"CREATE TABLE IF NOT EXISTS player_info ("
                         u"name TEXT NOT NULL,"
                         u"buy_value INTEGER NOT NULL,"
                         u"sell_value INTEGER"
                         u");")

        database.execute(u"CREATE TABLE IF NOT EXISTS manager_stats ("
                         u"date TEXT NOT NULL,"
                         u"cash INTEGER NOT NULL,"
                         u"team_value INTEGER NOT NULL"
                         u");")
        database.commit()

    # Inserts
    @staticmethod
    def insert_player_into_players(database, player, date):
        u"""
        ...
        :return:        None
        """
        sql = u"INSERT INTO players (name, value, points, position, date) VALUES(?, ?, ?, ?, ?)"
        database.execute(sql, (player[u"name"], player[u"value"], player[u"points"], player[u"position"], date))

    @staticmethod
    def insert_new_manager_stats_entry(database, date, cash, team_value):
        u"""
        ...
        :return:           None
        """
        sql = u"INSERT INTO manager_stats (date, cash, team_value) VALUES(?, ?, ?)"
        database.execute(sql, (date, cash, team_value))

    @staticmethod
    def insert_player_info(database, name, buy_value, sell_value):
        u"""
        ...
        database.execute(u"INSERT INTO player_info (name, buy_value, sell_value) VALUES(?, ?, ?)",
                         (name, buy_value, sell_value))

    # Updates
    @staticmethod
    def update_player_info(database, name, buy_value, sell_value):
        u"""
        ...
        if buy_value is not None:
            database.execute(u"UPDATE player_info SET buy_value = ? WHERE name = ?", (buy_value, name))
        elif sell_value is not None:
            database.execute(u"UPDATE player_info SET sell_value = ? WHERE name = ?", (sell_value, name))

    # Getters
    @staticmethod
    def get_player_names_with_null_sell_value(database):
        u"""
        ...
        """
        return database.execute(u"SELECT name FROM player_info WHERE sell_value IS NULL").fetchall()

    @staticmethod
    def get_buy_value_of_player(database, name):
        u"""
        ...
        """
        return database.execute(u"SELECT buy_value FROM player_info WHERE name = ?", (name,)).fetchall()[0][0]

    @staticmethod
    def get_player_list_on_date(database, date):
        u"""
        ...
        :return:         the results of the SELECT query, a list of tuples of player information
                         the results are in this order:
                            ...
                            - date
        """
        return database.execute(u"SELECT name, position, value, points, date "
                                u"FROM players WHERE date = ?", (date,)).fetchall()

    @staticmethod
    def get_player_on_date(database, date, name):
        u"""
        ...
        :return:         the results of the SELECT query, a list of tuples of player information
                         the results are in this order:
                            ...
                            - date
        """
        return database.execute(u"SELECT name, position, value, points, date FROM players WHERE date = ? AND name = ?",
                                (date, name)).fetchall()[0]

    @staticmethod
    def get_last_known_assets_values(database):
        u"""
        ...
        """
        assets = database.execute(u"SELECT cash, team_value, MAX(date) FROM manager_stats").fetchall()[0]
        return assets[0], assets[1]

    @staticmethod
    def get_first_recorded_date_of_player(database, name):
        u"""
        ...
        """
        return database.execute(u"SELECT MIN(date) FROM players WHERE NAME = ?", (name, )).fetchall()[0][0]
