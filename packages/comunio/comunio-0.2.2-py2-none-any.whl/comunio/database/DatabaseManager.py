...
class DatabaseManager(object):
    ...
    def __apply_schema(self):
        ...
        player_info_table = u"CREATE TABLE IF NOT EXISTS player_info (" \
                            u"name TEXT NOT NULL," \
                            u"buy_value INTEGER NOT NULL," \
                            u"sell_value INTEGER" \
                            u");"

        manager_stats_table = u"CREATE TABLE IF NOT EXISTS manager_stats (" \
                              u"date TEXT NOT NULL," \
    ...
    def __update_players_table(self):
        ...
        if len(players) == 5:
            raise ReferenceError(u"Player List Unavailable")

        sql = u"INSERT INTO players (name, value, points, position, date) VALUES(?, ?, ?, ?, ?)"
        for player in players:
            self.__database.execute(sql, (player[u"name"],
                                          player[u"value"],
                                          player[u"points"],
                                          player[u"position"],
                                          self.__date))
    ...
    def __update_transfers_from_news(self):
        ...
        for transfer in transfers:
            if transfer[u"type"] == u"bought":
                self.__database.execute(u"INSERT INTO player_info (name, buy_value, sell_value) VALUES(?, ?, NULL)",
                                        (transfer[u"name"], transfer[u"amount"]))
            else:
                self.__database.execute(u"UPDATE player_info SET sell_value = ? WHERE name = ?",
                                        (transfer[u"amount"], transfer[u"name"]))
        self.__database.commit()
    ...
    def __update_transfers_from_unregistered_player(self):
        ...
        player_infos = self.__database.execute(u"SELECT name FROM player_info WHERE sell_value IS NULL").fetchall()

        for player in self.get_players_on_day(0):
            if (player[u"name"],) not in player_infos:
                self.__database.execute(u"INSERT INTO player_info (name, buy_value, sell_value) VALUES(?, ?, NULL)",
                                        (player[u"name"], player[u"value"]))
        self.__database.commit()
        ...
        for player in player_infos:
            ...
            if not is_still_in_team:
                ...
                while market_value is None:
                    ...
                    if day_counter < 15:
                        market_value = self.__database.execute(u"SELECT buy_value FROM player_info WHERE name = ?",
                                                               (player[0],)).fetchall()[0]
    ...
    def update_database(self):
        ...
        today_results = self.__database.execute(u"SELECT * FROM players WHERE date = ?", (self.__date,)).fetchall()
    ...
    def get_players_on_day(self, day = 0):
        ...
        players = []
        database_results = self.__database.execute(u"SELECT * FROM players WHERE date = ?", (date, )).fetchall()
    ...
    def get_player_on_day(self, name, day = 0):
        ...
        date = self.__create_sqlite_date(day)
        try:
            player_info = self.__database.execute(u"SELECT value, position, points "
                                                  u"FROM players WHERE name = ? AND date = ?",
                                                  (name, date)).fetchall()[0]
            return {
                u"name": name,
                u"value": player_info[0],
                u"position": player_info[1],
                u"points": player_info[2]
            }
        except IndexError:
            return None
    ...
    def get_player_buy_values(self):
        ...
        """
        buy_values = {}
        players = self.__database.execute(u"SELECT * FROM player_info WHERE sell_value IS NULL").fetchall()
    ...
    def get_player_buy_value(self, name):
        ...
        """
        return self.__database.execute(u"SELECT buy_value FROM player_info WHERE name = ?", (name, )).fetchall()[0][0]

    def get_last_cash_amount(self):
        u"""
        :return: The last recorded cash amount
        """
        return self.__database.execute(u"SELECT cash, MAX(date) FROM manager_stats").fetchall()[0][0]

    def get_last_team_value_amount(self):
        u"""
        :return: The last recorded team value
        """
        return self.__database.execute(u"SELECT team_value, MAX(date) FROM manager_stats").fetchall()[0][0]
    ...
    def get_historic_data_for_player(self, player):
        ...
        while True:
            ...
            if player_on_day is not None:
                ...
                date = self.__create_sqlite_date(day)
                lowest_date = self.__database.execute(u"SELECT MIN(date) FROM players").fetchall()[0][0]
                if date < lowest_date:
                    break
