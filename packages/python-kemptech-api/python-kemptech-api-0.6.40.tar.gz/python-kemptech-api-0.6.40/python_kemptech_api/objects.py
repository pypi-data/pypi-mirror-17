...
from .exceptions import (
    ...
    AccessControlMissingLoadmasterInfo,
    AccessControlMissingVirtualServiceInfo,
    ValidationError,
    ...
    RangeMaskInvalid, VirtualServiceMissingLoadmasterInfo)
from .generic import BaseKempObject
from .utils import (
    validate_ip,
    validate_port,
    validate_protocol,
...
class VirtualService(BaseKempObject):
    ...
    def _get_base_parameters(self):
        ...
        if self.index is None:
            return super(VirtualService, self)._get_base_parameters()
        else:
    ...
    def create_access_control(self, addvs, addr):
        """AccessControl factory with pre-configured LoadMaster connection."""
        # AccessControl requires the full vs ID parameters
        # in order to operate correctly, so if an index is
        # currently populated in this instance of VirtualService
        # it will instantiate AccessControl with non valid params.
        # Temporarily making index None will bypass this behaviour
        # and pass the required parameters along to AccessControl.
        id = self.index
        self.index = None
        try:
            acl = AccessControl(self.access_info, addvs, addr)
        except AccessControlMissingLoadmasterInfo:
            raise
        finally:
            self.index = id
    ...
    def populate_default_attributes(self, service):
        ...
        if not self._ssl:
            ...
            try:
                self.sslrewrite = sslrewrite[self.sslrewrite]
            except KeyError:
...
class AccessControl(BaseKempObject):
    _API_ADD = "/aclcontrol"
    API_INIT_PARAMS = {
        "ip": "VSAddress",
        "port": "VSPort",
    ...
    def __init__(self, loadmaster_virt_service_info, addvs, addr):

        try:
            self.vsip = loadmaster_virt_service_info["vs"]
        except KeyError:
            raise AccessControlMissingVirtualServiceInfo("vs")
        ...
        try:
            ...
            raise AccessControlMissingLoadmasterInfo("endpoint")

        try:
            self.ip_address = loadmaster_virt_service_info["ip_address"]
        except KeyError:
            raise AccessControlMissingLoadmasterInfo("ip_address")

        super(AccessControl, self).__init__(loadmaster_virt_service_info)

    def __str__(self):
        return 'Access Control on {}'.format(self.vsip)

    def save(self, update=False):
        if not update:
            response = self._get(self._API_ADD,
...
class Rule(BaseKempObject):
    ...
    def populate_default_attributes(self, dictionary):
        ...
        # different tag names. The generic code using _API_TAG
        # isn't usable in this case.
        dictionary = dictionary.popitem()[1]

        for attribute, tag in self._API_DEFAULT_ATTRIBUTES.items():
            self.__setattr__(attribute, dictionary.get(tag, None))
...
class Fqdn(BaseKempObject):
    ...
    def get_sites(self):
        ...
        try:
            ...
            data = get_data(response)
            xml_object = data["fqdn"].get(self._API_TAG, [])
        except KempTechApiException:
            xml_object = []

        obj_list = []

        # If there is no _API_TAG key, build will fail with a
        ...
        if not isinstance(xml_object, list):
    ...
    def build_site(self, site):
        ...
        for parameter, tag in self.API_INIT_PARAMS.items():
...
class Site(BaseKempObject):
    ...
    def save(self, update=False):
        ...
        if is_successful(response):
            response = self._get(self._API_GET, self._get_base_parameters())
            data = get_data(response)
            maps = data["fqdn"].get(self._API_TAG, {})
...
class Certificate(BaseKempObject):
    def __init__(self, loadmaster_info, certname, certfile=None, certpass=None,
                 type=None):
    ...
    def save(self, update=False):
        if self.type == 'cert':
            ...
            response = self._post("/addintermediate", file=self.certfile,
                                  parameters=self._get_base_parameters())
        else:
            pass
        if is_successful(response):
    ...
    def delete(self):
        if self.type == 'cert':
            ...
            response = self._get("/delintermediate",
                                 self._get_base_parameters())
        return send_response(response)
...
class Interface(BaseKempObject):
    ...
    def __init__(self, loadmaster_info, interface):
    ...
    def populate_default_attributes(self, dictionary):
        super(Interface, self).populate_default_attributes(dictionary)

        # Strip additional addresses into a list
        if isinstance(self.additionaladdresses, OrderedDict):
            self.additionaladdresses = self.additionaladdresses['IPaddress']

            if not isinstance(self.additionaladdresses, list):
                self.additionaladdresses = [self.additionaladdresses]
        elif self.additionaladdresses is None:
            self.additionaladdresses = []

        if self.geotraffic == "no":
            self.geotraffic = "0"
    ...
    def set_additionaladdresses(self):
        ...
        old = Interface.get(self, self.interface).additionaladdresses
