...
def parse(name, data):
    ...
    for test in tap_parser.tests:
        ...
        if test.result == 'ok':
            ...
            t.add_error_info(test.yaml['trace'])

        junit_tests.append(t)

    return TestSuite(name, junit_tests)


def main():
    desc = "Converts a TAP13 file to jUnit"
    parser = argparse.ArgumentParser(desc)
    parser.add_argument('--input', '-i', type=argparse.FileType('r'), nargs='?',
                        help='path to tap13 file')
    parser.add_argument('--output', '-o', type=argparse.FileType('w'),
                        nargs='?', help='destination')
    ...
    result = parse(args.input.name.rstrip('.tap'), data)
    try:
