...
def _query_single(parent_id, q, creds):
    r = rpc.call("anvil.private.google.drive.list_files", parent_id, q, creds, max_results=1)
...
def wrap_item(f, creds):
    if f == None:
        ...
        return google.sheets.get_sheet(f["id"], creds)
    else:
        return File(f, creds)
...
def create_item_multipart(title, folder, content, creds):

    return wrap_item(rpc.call("anvil.private.google.drive.create_item_multipart", title, folder.id, content, creds), creds)
...
class File(DriveItem,rpc.LazyMedia):
    ...
    def __getattr__(self, attr_name):
        if attr_name == "revisions":
            ...
            return anvil.Media.__getattr__(self, attr_name)
        else:
    ...
    def list_revisions(self):
        ...
        return wrap_gen(_list_gen(request_page_fn), FileRevision, self.creds);
...
class Folder(DriveItem):
    ...
    def list_files(self):
        ...
        def request_page_fn(page_token):
            return rpc.call("anvil.private.google.drive.list_files", self.id, q, self.creds, page_token=page_token)

        return wrap_gen(_list_gen(request_page_fn), File, self.creds);
    ...
    def list_folders(self):
        ...
        return wrap_gen(_list_gen(request_page_fn), Folder, self.creds);
    ...
    def get(self, title):
        ...
        return wrap_item(_query_single(self.id, q, self.creds), self.creds)
    ...
    def create_file(self, title, content_bytes = None):
        if content_bytes:
            return create_item_multipart(title, self, content_bytes, self.creds)
        else:
            return create_item_simple(title, self, "text/plain", self.creds)
...
description: |
  ...
  folder = google.drive.get_user_files()

  for f in folder.list_files():
    print f["title"]
  ```

  To read and write files in a user's Google Drive, you must first call `google.drive.login()`, which will ask the user for permission to access their files. This is different from calling
  `google.auth.login()`, which only asks for their email address.

  Once a user has logged into your app using `google.drive.login()` you can read and write files in their Google Drive.

  You can get the top-level folder containing all the files in their drive by calling `google.drive.get_user_files()`.

  You will need to get a client ID and secret from the
  <a href="https://developers.google.com/api-client-library/python/guide/aaa_apikeys" target="_blank">Google Developer Console <i class="fa fa-external-link"></i></a>
  if you want to use `google.drive.login()`. This client ID and secret must be pasted
...
def login():
    return google.auth.login(google_api_scopes)
...
class AppFilesCollection:
    ...
    def __getattr__(self, python_name):
        id = None
        for df in google.get_config().get("app_files", []):
