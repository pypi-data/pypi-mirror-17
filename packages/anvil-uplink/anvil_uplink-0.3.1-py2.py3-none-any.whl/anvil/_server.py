...
class LiveObjectProxy(anvil.LiveObject):
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            raise AnvilWrappedError(e.error_obj)
...
def reconstruct_live_object(d):
    ...
    for k,v in d.get("itemCache", {}).iteritems():
...
def serialise_val(v):
    if isinstance(v, (numbers.Number, bool, str, unicode)) or v is None:
        return {
            "type": ["Primitive"],
            "value": v
        }
...
def serialise_live_object(obj):
    ...
    for k,v in obj.get("itemCache", {}).iteritems():
