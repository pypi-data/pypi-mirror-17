...
def ajaxloaded(decorated):
    def display(w, value={}, **kw):
        ...
        return HTML(HTML.div('', id=form_id), 
                    HTML.script(HTML.literal('''tgext_ajaxforms.jQuery(document).ready(function() {{
tgext_ajaxforms.jQuery('#{0}').load('{1}', {2});
...
function {0}_on_demand(where, params) {{
    tgext_ajaxforms.jQuery(where).load('{1}', params);
    ...
    if is_tw2_form(decorated):
        decorated.resources.extend([TW2.jquery_js, TW2.jquery_form_js, TW2.ajaxforms_js])
        decorated.display = classmethod(display)
        decorated.on_demand = classmethod(on_demand)
    else:
        decorated.javascript.extend([TW1.jquery_js, TW1.jquery_form_js, TW1.ajaxforms_js])
