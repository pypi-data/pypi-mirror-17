...
class Connection(object):
    ...
    def download(self, full_title, data):
        ...
        try:
            ...
            try:
                html_sub_download = BeautifulSoup(sub_download_page.text, "html.parser")
                sub_id = get_id_with_reg(html_sub_download.find_all(
                    ...
                    for title in titles:
                        try:
                            ...
                            if sub_data["format"] == data["format"] and (sub_data["screen_size"] == data[
                                "screen_size"] or sub_data["video_codec"] == data["video_codec"]) and \
                                    title.parent.find_previous("div").find_previous("div").find("img")[
