...
def _discoro_server_coro_proc():
    ...
    sys.path.insert(0, os.path.join(_discoro_dest_path, '..'))
...
def _discoro_server_process(_discoro_config, _discoro_mp_queue):
    import os
    import sys
    import time
    import traceback
...
def _discoro_spawn(_discoro_config, _discoro_id_ports, _discoro_mp_queue,
                   _discoro_computation, _discoro_pipe):
    import os
    import shutil
    import signal
    ...
    for i in range(len(_discoro_id_ports)):
        ...
        asyncoro.logger.info('discoro server %s started with PID %s',
            ...
            if msg:
                ...
                if req == 'server_setup':
                    ...
                    if (isinstance(client, asyncoro.Coro) and isinstance(cpus, int) and
                        ...
                        params = (_discoro_config, id_ports, _discoro_mp_queue, computation,
                                  _discoro_recv_pipe)
                    ...
                    if auth == cur_computation_auth:
                        ...
                        if _discoro_spawn_proc:
                            ...
                            for i in range(10):
                                if _discoro_send_pipe.poll(1):
                                    closed = _discoro_send_pipe.recv()
                                    _discoro_spawn_proc = None
                                    break
                                if i == 5:
                                    for server in _discoro_servers:
                                        if server.coro:
                                            server.coro.terminate()
