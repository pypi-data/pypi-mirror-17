...
import bob.core
import logging
logger = logging.getLogger("bob.db.base")
...
def read_annotation_file(file_name, annotation_type):
  ...
  with open(file_name, 'r') as f:
    ...
    if str(annotation_type) == 'eyecenter':
      ...
      # multiple lines, no header line, each line contains annotation and position or single value annotation
      for line in f:
        positions = line.split()
        if len(positions) == 3:
          ...
          logger.error("Could not interpret line '%s' in annotation file '%s'", line, file_name)
