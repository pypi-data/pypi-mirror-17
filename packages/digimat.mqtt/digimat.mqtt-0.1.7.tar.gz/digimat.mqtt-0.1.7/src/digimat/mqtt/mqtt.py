...
class MQTTPayloadAESCipher(MQTTPayloadCipher):
    ...
    def onEncryptData(self, data):
        return self._cipher.encrypt(data, False)

    def onDecryptData(self, data):
        return self._cipher.decrypt(data, False)
...
class MQTTPayloadAESCipherWithHashIntegrity(MQTTPayloadCipher):
    ...
    def onDecryptData(self, data):
        print data
        if self._hashCache is not None:
            h=self.hash_sha224(data)
            try:
                if self._hashCache[h]:
                    print "DEBUG: potential replay attack!"
                    return None
            except:
                self._hashCache[h]=1
        ...
        if h and h==data['hash']:
            ...
            if self._maxLagAllowed>0:
                try:
                    tlag=abs(time.time()-data['ts'])
                    if tlag>self._maxLagAllowed:
                        print "DEBUG: Potential replay attack blocked (%d)" % tlag
                        return None
                except:
                    pass
            return data['data']
