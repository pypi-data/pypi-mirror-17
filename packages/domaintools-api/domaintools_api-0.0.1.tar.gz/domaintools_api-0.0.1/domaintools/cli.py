...
def run():
    ...
    parser.add_argument('-v', '--version', action='version', version='DomainTools CLI API Client {}'.format(version))
    ...
    for api_call in API_CALLS:
        ...
        for argument_name, default in reversed(list(zip_longest(reversed(spec.args or []),
            ...
            if argument_name == 'self':
                ...
                subparser.add_argument('--{}'.format(argument_name.replace('_', '-')), dest=argument_name,
    ...
    if not user or not key:
        try:
            ...
            print('Credentials are required to perform API calls.')
            sys.exit(1)
