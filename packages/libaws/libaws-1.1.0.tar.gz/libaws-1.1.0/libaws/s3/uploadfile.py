...
def main():
    ...
    #上传分块个数，默认为6
    parser.add_argument("-n", "--num", type=int, dest="part_num",help='part num of file',default=config.DEFAULT_PART_NUM)
    ...
    if os.path.isdir(file_path):
        ...
        for child_file_path in iter_path(file_path):
            ...
            if key is not None:
                upload_config.key = os.path.join(key,upload_config.key)
