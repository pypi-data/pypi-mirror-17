...
class IOLooper(object):
    ...
    def update_download_progress(self,down_cell,s3_download_db):
        ...
        if s3_download_db.get_range_download_size(down_cell.range_id) == s3_download_db.get_range_size(down_cell.range_id):
            return
        #计算下载进度
...
class MultiPartDownloader(object):
    ...
    def download(self,s3_downloader,body,callback=None):
        '''
            下载分块
        '''
        ...
        for chunk in iter(lambda:body.read(block_size),b''):
            ...
            #构建最小下载单元对象
            download_cell = DownloadCell(start_byte,chunk_length,chunk,self.download_range_id,range_id)
